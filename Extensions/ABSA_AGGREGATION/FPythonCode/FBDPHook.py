...
History:
Date       JIRA Number     Who             What
2019-09-05 FAU-359         Amit Kardile    Added exercise_trade hook to let exercise/ assign
                                           script generate trades in FO Confirmed status(which
                                           traders have access to). The exercise assign script
                                           otherwise implements the core
                                           FInstrument.CalculatePosition() api which returns
                                           BO Confirmed trades due to FBDPHook.set_trade_field_status hook
2021-01-20 FAPE-288        Marian Zdrazil  Added exclusion hook to keep trades on PRIME BIDVEST NOTE
                                           from getting aggregated. Checking the instrument type first,
                                           then if the instrument ID is PRIME BIDVEST NOTE. If it is not,
                                           we are going to apply the exclude_dividend_suppression_trades hook,
                                           as usual, to exclude trades with dividend suppression.
----------------------------------------------------------------------------"""
...
def set_trade_field_status(trades, default):
    """
    Set the aggregated trade status to BO Confrimed.
    """
    if (trades):
        return "BO Confirmed"
    else:
...
def exclude_BIDVEST_note_hook(trade):
    # Exclude all trades done on PRIME BIDVEST NOTE
    if trade.insaddr.instype in ['Stock']:
        if trade.insaddr.insid == 'PRIME BIDVEST NOTE':
            return 1
        else:
            # Exclude all dividend suppression trades
            return exclude_dividend_suppression_trades(trade)
    else:
        # Exclude all dividend suppression trades
        return exclude_dividend_suppression_trades(trade)

def exclude_bonds_hook(trade):
    from exclude_bonds import EXCLUDE_POSITIONS

    portfolio = trade.prfnbr.prfid
    instrument = trade.insaddr.insid
    exclude = False
...
def exclude_dividend_suppression_trades(trade):
    payments = acm.FPayment.Select('trade=%s type="%s"' % (trade.trdnbr, 'Dividend Suppression'))
    if len(payments) > 0:
        return 1
    else:
        return 0

def exclude_sales_credit_non_african_bonds(trade):
    cl_name = 'AfricaBondAggregationCurrencies'

    if exclude_sales_credit(trade) == 1 or exclude_trades_with_instrument_currency(trade, cl_name) == 1:
        return 1
    return 0

def exclude_sales_credit_non_african_bills(trade):
    cl_name = 'AfricaBillAggregationCurrencies'

    if exclude_sales_credit(trade) == 1 or exclude_trades_with_instrument_currency(trade, cl_name) == 1:
        return 1
    return 0

def exclude_sales_credit(trade):
    if ((trade.sales_credit is None or trade.sales_credit == 0.0) and
            trade.add_info('Sales_Credit2') == "" and
            trade.add_info('Sales_Credit3') == "" and
            trade.add_info('Sales_Credit4') == "" and
            trade.add_info('Sales_Credit5') == ""):
        return 0
    return 1

def exclude_open_collateral_trades(trade):
    today = acm.Time().DateNow()
    if trade.TradeCategory() == 'Collateral' and trade.ReAcquireDay() >= today:
        return 1
    return 0

def exclude_trades_with_instrument_currency(trade, cl_name):
    # Exclude trades of instruments with currencies not in choice list.
    currencies = [choice.Name() for choice in acm.FChoiceList.Select('list={}'.format(cl_name))]

    if trade.insaddr.curr.insid not in currencies:
        return 1
    return 0
...
'''Required in Exercise Assign script. Refer to extension history at the top for more details.'''
def exercise_trade(ins, trade):
    trade.status = 'FO Confirmed'
    trade.text2 = 'AUTOCLOSE'
    return trade
...
#----------------------------------------------------------------------------"""
def RiskFactorNameHandler(riskClass, rfNames, rfValues):
    ...
    if riskClass == 'FX':
        if 'Volatility' not in rfNames:
            termCurrencyIndex = rfNames.index('Term Currency')
