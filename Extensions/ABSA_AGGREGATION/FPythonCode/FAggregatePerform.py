...
HISTORY
    ...
    25/09/2020   FAPE-464   Marcus Ambrose   Short sell restricted bonds workaround.
...
class FAggregatePerform(FBDPValidate):
    ...
    def perform_aggregation(self, dictionary):
        ...
        short_sell_bonds = self._get_forbidden_short_sell_bonds(filters)
        if short_sell_bonds: # check this check on sets
            self.update_bonds_short_sell(short_sell_bonds, 'Allowed')

        result = acm.AggregateTrades(day, log_agg, poll_agg,
                useSelectedFundingDay, filters, bypassTradeValidation)

        if short_sell_bonds:
            self.update_bonds_short_sell(short_sell_bonds, 'Forbidden')
    ...
    @staticmethod
    def _get_forbidden_short_sell_bonds(filters):
        bond_ins = set()
        for trd_filter in filters:
            filter_obj = acm.FTradeSelection[trd_filter]
            for condition in filter_obj.FilterCondition().AsList():
                if str(condition[2]) == 'Instrument.Type':
                    bond_ins = bond_ins.union({ins for ins in filter_obj.Instruments() if
                                               ins.InsType() == 'Bond' and ins.ShortSell() == 'Forbidden'})

        return bond_ins

    @staticmethod
    def update_bonds_short_sell(bond_instruments, new_status):
        if bond_instruments:
            acm.BeginTransaction()
            try:
                for bond in bond_instruments:
                    bond.ShortSell(new_status)
                    bond.Commit()
                acm.CommitTransaction()
            except Exception as e:
                acm.AbortTransaction
                Logme()('Failed to update bond ShortSell attribute to {}: {}'.format(new_status, e), 'ERROR')
