...
DEFAULTTRADER = acm.User()
MIDASDEFAULTPARTY = acm.FParty['MIDAS DUAL KEY']
...
TRADE_SYSTEM = 'FAR'
TRADE_SYSTEM_CFR = 'MIDAS CFR'
TRADE_SYSTEM_OTHER = 'OTHER'
_SUBDIRS = ('Before', 'After', 'Compare')
global ABSA_ADDITIONAL_INFO
ABSA_ADDITIONAL_INFO = {}

class AdditionalInfo():
    def __init__(self):
        self.optionalKey = None
        self.insOverride = None
        self.sourceCtpyName = None
        self.sourceSystem = None
        self.sourceTradeId = None
        self.sourceTradeType = None
        self.sourceTrader = None
    
    def set_optionalKey(self, optionalKey):
        self.optionalKey = optionalKey
        
    def set_insOverride(self, insOverride):
        self.insOverride = insOverride
    
    def set_sourceCtpyName(self, sourceCtpyName):
        self.sourceCtpyName = sourceCtpyName
        
    def set_sourceSystem(self, sourceSystem):
        self.sourceSystem = sourceSystem
        
    def set_sourceTradeId(self, sourceTradeId):
        self.sourceTradeId = sourceTradeId
    
    def set_sourceTradeType(self, sourceTradeType):
        self.sourceTradeType = sourceTradeType
    
    def set_sourceTrader(self, sourceTrader):
        self.sourceTrader = sourceTrader

def setABSAAdditionalInformation():
    global ABSA_ADDITIONAL_INFO
    Logme()('Setting ABSA Additional Information')
    for trdnbr in ABSA_ADDITIONAL_INFO.keys():
        ABSAAdditionalInfo = ABSA_ADDITIONAL_INFO[trdnbr]
        Logme()(trdnbr)
        Logme()('\t OptionalKey: %s' %ABSAAdditionalInfo.optionalKey)
        Logme()('\t InsOverride: %s' %ABSAAdditionalInfo.insOverride)
        Logme()('\t Source_Ctpy_Name: %s' %ABSAAdditionalInfo.sourceCtpyName)
        Logme()('\t Source_System: %s' %ABSAAdditionalInfo.sourceSystem)
        Logme()('\t Source_Trade_Id: %s' %ABSAAdditionalInfo.sourceTradeId)
        Logme()('\t Source_Trade_Type: %s' %ABSAAdditionalInfo.sourceTradeType)
        Logme()('\t Source_Trader: %s' %ABSAAdditionalInfo.sourceTrader)
        
    acm.PollDbEvents()
    for trdnbr in ABSA_ADDITIONAL_INFO.keys():
    	aggTrade = acm.FTrade[trdnbr]
	
	ABSAAdditionalInfo = ABSA_ADDITIONAL_INFO[trdnbr]
	
	if ABSAAdditionalInfo.optionalKey:
	    aggTrade.OptionalKey = ABSAAdditionalInfo.optionalKey
	
	if ABSAAdditionalInfo.insOverride:
	    aggTrade.AdditionalInfo().InsOverride(ABSAAdditionalInfo.insOverride)
	
	if ABSAAdditionalInfo.sourceCtpyName:
	    aggTrade.AdditionalInfo().Source_Ctpy_Name(ABSAAdditionalInfo.sourceCtpyName)
	
	if ABSAAdditionalInfo.sourceSystem:
	    aggTrade.AdditionalInfo().Source_System(ABSAAdditionalInfo.sourceSystem)
	
	if ABSAAdditionalInfo.sourceTradeId:
	    aggTrade.AdditionalInfo().Source_Trade_Id(ABSAAdditionalInfo.sourceTradeId)
	
	if ABSAAdditionalInfo.sourceTradeType:
	    aggTrade.AdditionalInfo().Source_Trade_Type(ABSAAdditionalInfo.sourceTradeType)
	
	if ABSAAdditionalInfo.sourceTrader:
	    aggTrade.AdditionalInfo().Source_Trader(ABSAAdditionalInfo.sourceTrader)
	...
	aggTrade.Commit()
	    ...
	    Logme()('ERROR: Could not set the ABSA Information on trade %i: %s' %(trdnbr, str(e)))
    ABSA_ADDITIONAL_INFO = {}
...
class FxAggregation(FFxCommon.FxGroupingProcess):
	...
	self.minAggAmount = args['minAggAmount']
                ...
                for (node, self.attributes) in nodes:
                    ...
                    if not self.deaggregate:
                        if trades is None:
                            ...
                            nbrOfTrades = len(trades)
                            
                        if nbrOfTrades < self.minAggAmount:
                            Logme()('Number of trades is less than the minimum number of trades to aggregate. No aggregation will take place.')
                        else:
                            self.aggregateBatch(trades, currPair)
                    else:
                        self.aggregateBatch(trades, currPair)
                        
                    setABSAAdditionalInformation()
            diffPL(obj)
    ...
    def getCashAggregationInfoObjects(self, node, trades, currPair):
        pair = ''
        if 'CurrencyPair' in self.attributes:
            pair = self.attributes['CurrencyPair']
        elif 'PositionOrCurrencyPair' in self.attributes:
            pair = self.attributes['PositionOrCurrencyPair']

        if 'No Currency Pair' in pair or \
           'No CurrencyPair' in pair:
            aggInfoDic = {}
            for t in trades:
                curr = t.Currency()
    ...
    def aggregatePosition(self, node, tradingPort, aggInfo):
        ...
        try:
            if trades and len(trades) > 0:
                midasSettlement = trades[0].MidasSettlement()
                insOverride = trades[0].InsOverride()
        except:
            midasSettlement = False
	    insOverride = None
        ...
        if not (abort):
            if not self.Testmode:
	    	if aggTrade.Oid() in ABSA_ADDITIONAL_INFO.keys():
		    ADDITIONAL_INFO  = ABSA_ADDITIONAL_INFO[aggTrade.Oid()]
		else:
	    	    ADDITIONAL_INFO = AdditionalInfo()
		if tradingPort and tradingPort.Name().__contains__('MIDAS'):
        	    ADDITIONAL_INFO.set_sourceCtpyName('CASH')
                    ADDITIONAL_INFO.set_sourceSystem('Midas')
	            ADDITIONAL_INFO.set_sourceTradeId('CASH')
	            ADDITIONAL_INFO.set_sourceTradeType('CA')
	            ADDITIONAL_INFO.set_sourceTrader('CASH')
                    
              	if insOverride not in (None, ''):
                    ADDITIONAL_INFO.set_insOverride(insOverride)
                
		tradeSystem = self.getTradeSystem(tradingPort, midasSettlement)
                if tradeSystem != TRADE_SYSTEM_OTHER:
                    ADDITIONAL_INFO.set_optionalKey('%i|%s|0' %(aggTrade.Oid(), tradeSystem))
		
		ABSA_ADDITIONAL_INFO[aggTrade.Oid()] = 	ADDITIONAL_INFO
    ...
    def createNewAggTrade(self, tradingPort, aggInfo):
        ...
        if tradingPort and tradingPort.Name().__contains__('MIDAS'):
            trade.Acquirer(MIDASDEFAULTPARTY)
        else:
    ...
    def getTradeSystem(self, tradingPort, midasSettlement):
        if tradingPort:
            if tradingPort.Name().__contains__('MIDAS'):
                return TRADE_SYSTEM_CFR
            elif midasSettlement == True:
                return TRADE_SYSTEM
            else:
                return TRADE_SYSTEM_OTHER
        return ''
