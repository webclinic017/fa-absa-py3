...
class LiveInstTrds:
    ...
    def _checkValidTrdPosition(self, inst, posName, preservePL, calcResult):
        ...
        if useCurrencyDependency:
            ...
            # No cash posting trades generated.
            if not calcResult[0]:
                return True
            trd = calcResult[0][0]
            trdPosition = trd.Quantity()
        ...
        if not FBDPCommon.eps_compare(trdPosition):
            self.log.logWarning('Non-Zero Position in %s. Ingore to perform on this position.' % posName)
            return False
        ...
        try:
            for portId in list(trdsInPortf.keys()):
                ...
                for calcResult in calcPosOnGrouper:
                    ...
                    useCurrencyDependency = len(calcResult[0]) > 1
                    if not calcResult[0] and dependencyTrds:
                        csp_trades.append([None, dependencyTrds])
                    else:
                        for trd in calcResult[0]:
    ...
    def _createCSPTrade(
        ...
        t.aggregate = 1
        t.type = 'Cash Posting'
        if len(insid) > MAX_TEXT_FIELD_LENGTH:
            insidstr = insid[:MAX_TEXT_FIELD_LENGTH - 1] + '$'
            t.text1 = insidstr
        else:
            t.text1 = insid
        if len(prfid) > MAX_TEXT_FIELD_LENGTH:
            pidStr = prfid[:MAX_TEXT_FIELD_LENGTH - 1] + '$'
            t.text2 = pidStr
        else:
            t.text2 = prfid
        t.quantity = 0.0
...
class ArchivedInstTrds:
    ...
    def GetArchivedTrdsAndItsCPTrds(self, execParam, getCSPTrds):
        ...
        sql_archivedTrades = sql_archivedTrades.replace("[", "(")
        sql_archivedTrades = sql_archivedTrades.replace("]", ")")
        ...
        if not getCSPTrds:
            cpAndArchiveTrds = _CPAndArchiveTrds(
                cpTrds=None, arcTrds=selectedTrds)
            return [cpAndArchiveTrds]

        # TODO for store folder
        #Find the cashposting trade.
        sql_cashpostingTrades = (
            "select distinct aggregate_trdnbr from trade "
            "where insaddr = %s " % self.inst)\
            + sql_ports + sql_allowed_trades

        sql_cashpostingTrades = sql_cashpostingTrades.replace("[", "(")
        sql_cashpostingTrades = sql_cashpostingTrades.replace("]", ")")

        cpTrades = FBDPCommon.get_result_in_list(
                    ael.dbsql(sql_cashpostingTrades))

        cpTrades = [ x for x in cpTrades if x != 0]
        cpArcTrdsList = []
        if len(cpTrades):
            ...
            self.log.logInfo(
                'we can not find any CSP trades associated with '
                'the selected trades, de-archive the selected trade anyway.')
            cpAndArchiveTrds = _CPAndArchiveTrds(
                    cpTrds=None, arcTrds=selectedTrds)
            cpArcTrdsList.append(cpAndArchiveTrds)
