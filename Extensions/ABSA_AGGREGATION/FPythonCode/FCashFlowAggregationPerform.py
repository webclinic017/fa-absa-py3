...
class _CashFlowAggregationBase(FBDPPerform.FBDPPerform):
    ...
    @staticmethod
    def _infraGetNonArchivedConfOidListByCashFlowOid(cfOid, requestedAggDate):
        q = 'SELECT seqnbr FROM confirmation WHERE cfwnbr = {0} and archive_status = 0 and creat_time < {1}'.format(cfOid, requestedAggDate)
        return [row[0] for row in ael.dbsql(q)[0]]

    @staticmethod
    def _infraGetNonArchivedSettlOidListByCashFlowOid(cfOid, requestedAggDate):
        q = 'SELECT seqnbr FROM settlement WHERE cfwnbr = {0} and archive_status = 0 and creat_time < {1}'.format(cfOid, requestedAggDate)
        return [row[0] for row in ael.dbsql(q)[0]]
    ...
    @staticmethod
    def _infraGetConfirmationOidListByResetOid(rstOid, requestedAggDate):
        q = 'SELECT seqnbr FROM confirmation WHERE reset_resnbr = {0} and archive_status = 0 and creat_time < {1}'.format(rstOid, requestedAggDate)
        return [row[0] for row in ael.dbsql(q)[0]]
    ...
    def __findAlignedAggDate(self, leg, requestedAggDate, firstRACFPayDate):
        ...
        candidateAlignedAggDates = list(set([cf.StartDate() for cf in
                periodOverlappedCFs]))
    ...
    def __inspectInstrumentLeg(self, leg, runParam, insProcParam,
        ...
        # Inspect cash flows
        allCFs = [cf for cf in leg.CashFlows()]
        for cf in allCFs:
            # Check the cash flow.
    ...
    def __inspectInstrumentLegCashFlowReset(self, rst, runParam, insProcParam,
        ...
        cfmOidList = self._infraGetConfirmationOidListByResetOid(rst.Oid(), runParam.requestedAggDate)
        if cfmOidList:
    ...
    def __inspectInstrumentLegCashFlowNoConfirmationAttached(self, cf,
        ...
        cfmOidList = self._infraGetNonArchivedConfOidListByCashFlowOid(cf.Oid(), runParam.requestedAggDate)
        if cfmOidList:
    ...
    def __inspectInstrumentLegCashFlowNoClosedSettlementAttached(self, cf,
        ...
        stmOidList = self._infraGetNonArchivedSettlOidListByCashFlowOid(cf.Oid(), runParam.requestedAggDate)
        if stmOidList:
            errMsg = ('Cash flow {0} of Leg {1} of Instrument {2} has '
...
class _CashFlowAggregation(_CashFlowAggregationBase):
    ...
    def __calculateRatesCashFlowSettledInterestList(self, acmCFList,
        ...
        for cf in acmCFList:
            try:
                si = cf.Calculation().Projected(spaceCollection, siParams['trade']).Number()
            except Exception as e:
