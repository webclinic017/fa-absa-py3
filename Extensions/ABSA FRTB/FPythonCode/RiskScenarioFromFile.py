...
def CreateScenario(scenDict, dictExtra) :
    ...
    if not fileData:
        ...
        for collection in collectionsList:
            absolute_strikes = {}
            relative_strikes = []#moneyness
            vol_surface = None
            processed_vol_surface = {}
            if collection.RiskFactorType() == 'Volatility' and "Equity" in collection.AddInfos().AsString():
                for dimension in collection.RiskFactorDimensions():
                    if dimension.DimensionId() == 'Strike':
                        strikebucket_dfn = dimension.CoordinatesSource().StrikeBucketsDefinition()
                        for strikebucket in strikebucket_dfn.StrikeBucketDefinitions():
                            relative_strikes.append(strikebucket.StrikeMid())
  
            #do a lookup and derive absolute strikes from relatives:
            for riskFactorInstance in collection.RiskFactorInstances():
                vol_surface_handle =""               
                if  'MDT_AA_Format' in  collection.RiskFactorSetup().RiskFactorPropertySpecifications().AsString():
                    cords = riskFactorInstance.RiskFactorCoordinates()       
                    
                    if collection.RiskFactorType() == 'Volatility' and "Equity" in collection.AddInfos().AsString():
                        for coordinate in cords:
                            coord_value = str(coordinate.CoordinateValue())                          
                            riskfactorDimId = coordinate.RiskFactorDimensionUniqueId()
                            riskFactorDimension = acm.FRiskFactorDimension[riskfactorDimId]
                            #only go through this loop to set moneyness to strike dict
                            if riskFactorDimension.DimensionId() == "Volatility Surface" and coord_value not in processed_vol_surface.keys():

                                vol_surface = acm.FVolatilityStructure[coord_value]
                                if vol_surface:
                                    vol_surface_handle = coord_value
                                    points = vol_surface.Points().SortByProperty("ActualExpiryCoordinate")
                                    ins = None
                                    for point in points:
                                        if not ins:
                                            if point.Benchmark() and point.Benchmark().ValuationUnderlying():
                                                ins = point.Benchmark().ValuationUnderlying()                                           
                                    #print('underlying ins')
                                    if ins:
                                        get_last_price = acm.FPrice.Select('instrument="%s" and market="SPOT_MID"' % ins.Name())
                                        last_mid = get_last_price
                                        if last_mid:
                                            for moneyness_term in relative_strikes:
                                                absolute_strike = round((moneyness_term * last_mid.Last().Last()), 2) #moneyness=k/s on current rw extract, incoming scenarios
                                                absolute_strikes[moneyness_term] = absolute_strike
                                            processed_vol_surface[coord_value] = copy.deepcopy(absolute_strikes)
                                            
            #print(processed_vol_surface)
            for riskFactorInstance in collection.RiskFactorInstances():
                #if  riskFactorInstance.RiskFactorCollection().AdditionalInfo().VaR_AA_Scenarios() == True: 
                if  'MDT_AA_Format' in  collection.RiskFactorSetup().RiskFactorPropertySpecifications().AsString():           
                    externalId = acm.FSymbol(acm.RiskFactor.RiskFactorExternalId(riskFactorInstance))
                    if collection.RiskFactorType() == 'Volatility':# and applicable:
                        cords = riskFactorInstance.RiskFactorCoordinates()
                        strike_coord= None
                        vol_surface = None                        
                        for coordinate in cords:
                            ...
                            riskfactorDimId = coordinate.RiskFactorDimensionUniqueId()
                            riskFactorDimension = acm.FRiskFactorDimension[riskfactorDimId]
                            if riskFactorDimension.DimensionId() == "Volatility Surface":#store handle to vol surface name to use in lookup of strike in dict
                                vol_surface = acm.FVolatilityStructure[coord_value]
                            if riskFactorDimension.DimensionId() == "Strike":#pick strike coordinate object handle 
                                strike_coord = coordinate
                        
                        if strike_coord and vol_surface and vol_surface.Name() in processed_vol_surface.keys():
                            if strike_coord.IsModified():
                                strike_coord.Undo()
                            coord_value=str(strike_coord.CoordinateValue())
                            look_up_value = float(coord_value)
                            strike_coord.CoordinateValue(processed_vol_surface[vol_surface.Name()][look_up_value])#replace moneyness with absolute strike
                            
                        if vol_surface.RiskType() == "IR Vol" and vol_surface.StructureType() == "Benchmark":
                            if strike_coord.IsModified():
                                strike_coord.Undo()
                            coord_value = float(str(strike_coord.CoordinateValue()))
                            adjusted_strike = coord_value * 100.0
                            strike_coord.CoordinateValue(adjusted_strike)#adjust IR strike
                   
                    if str(externalId).find("Par CDS Rate") != -1: #Par CDS Rate lookup change partyname to party ID
                        IssuerId = str(externalId).split("|")[0].split(".")[-1]
                        IssuerName = acm.FParty[IssuerId].Name()
                        externalId = str(externalId).replace(str(IssuerId), str(IssuerName), 1)#replace only first occurance in case IssuerId is same as time bucket  
                
                    if fileData.HasKey(externalId):                       
                        rf = acm.RiskFactor.RiskFactorFromRiskFactorInstance(riskFactorInstance, externalId, riskFactorCreatorCache, dateToday)
                        riskFactors.append(rf)

                        
                    cords = riskFactorInstance.RiskFactorCoordinates() 
                    for coordinate in cords: #Now reset strike to originil Moneyness numbers, leave in consistent state
                        riskfactorDimId = coordinate.RiskFactorDimensionUniqueId()
                        riskFactorDimension = acm.FRiskFactorDimension[riskfactorDimId]
                        if riskFactorDimension:
                            if riskFactorDimension.DimensionId() == "Strike" and coordinate.IsModified():#check riskfactorInstance coordinate value
                                coordinate.Undo()                   
