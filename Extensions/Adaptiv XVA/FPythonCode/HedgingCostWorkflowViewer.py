...
#__src_file__ = "extensions/cva/adaptiv_xva/./etc/HedgingCostWorkflowViewer.py"
import acm
import FUxCore

STATE_CHART_HEDGINGCOST            = 'HedgingCostWorkflow'
STATE_PENDING_HEDGINGCOST          = 'Pending HedgingCost'
COLUMN_ID_LIST             = ['Trade Instrument', 'Trade Counterparty', 'Trade Acquirer', 'Trade Trader', 'Trade Portfolio', 'Trade Status']

def HedgingCostWorkflowState(trade):
    stateChart = acm.FStateChart[STATE_CHART_HEDGINGCOST]
...
class HedgingCostWorkflowViewer (FUxCore.LayoutPanel):
    def __init__(self):
        self.m_stateChart = acm.FStateChart[STATE_CHART_HEDGINGCOST]
    ...
    def UpdateAdHocPortfolio(self):
        ...
        currentResultSet.AddAll(self.m_tradesInPendingHedgingCost)
        ...
        self.m_tradesInPendingHedgingCost.AddAll(updatedResultSet)
        self.m_tradesInPendingHedgingCost.RemoveAll(shouldBeRemoved)
    ...
    def InsertAdHocPortfolioInTradeSheet(self):
        adHocPortfolio = acm.FAdhocPortfolio(self.m_tradesInPendingHedgingCost)
        adHocPortfolio.Name("HedgingCost Trades")
        treeProxy = self.m_tradeSheet.GridBuilder().InsertItem(adHocPortfolio)
        treeProxy.Expand(True)
        treeProxy.ApplyGrouper(acm.FAttributeGrouper("Trade.HedgingCostWorkflowState"))
    ...
    def HandleCreate(self):
        ...
        self.m_tradesInPendingHedgingCost = acm.FDependentArray()
        
        self.Owner().AddDependent(self)
        self.m_queryResult.AddDependent(self)
        self.m_tradesInPendingHedgingCost.AddDependent(self)
...
def OnCreate(eii):
    ...
    myPanel = HedgingCostWorkflowViewer()
    basicApp.CreateCustomDockWindow(myPanel, 'HedgingCostWorkflowViewer', 'HedgingCost Workflow Viewer', 'Bottom', None, True, False);
