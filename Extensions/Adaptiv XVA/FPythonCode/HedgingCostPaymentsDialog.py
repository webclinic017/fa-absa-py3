...
#__src_file__ = "extensions/cva/adaptiv_xva/./etc/HedgingCostPaymentsDialog.py"
from __future__ import print_function
import acm
import FUxCore
from HedgingCostUtils import HedgingCostStateChartConstants
import HedgingCostHooksHelper
import traceback
import sys
...
class HedgingCostPaymentsDialog( FUxCore.LayoutDialog ):
    ...
    def CreateLayout( self ):
        ...
        b.      BeginHorzBox()
        b.              BeginVertBox()
        b.                      AddLabel('serverAmountLabel', 'Hedging Cost ( XYZ ) ')
        self.                   m_currBinding.BuildLayoutPart(b, 'Currency')
        b.                      AddLabel('clientAmountLabel', 'Hedging Cost ( XYZ ) ')
        b.              EndBox()
        b.              BeginVertBox()
    ...
    def HandleCreate( self, dlg, layout ):
        ...
        try:
            ...
            self.m_serverCurrName, self.m_serverAmountCalcValue = self._suggestHedgingCost()
            self.m_dlg = dlg
            
            dlg.Caption( 'Hedging Cost Charge' )
    ...
    def UpdateControls(self):
        ...
        serverCurrLabel = 'Hedging Cost ( %s )' % self.m_serverCurrName
        ...
        clientCurrLabel = 'Hedging Cost ( %s )' % self.m_currObject.Name()
    ...
    def _suggestHedgingCost( self ):

        hedging_costCurr =''
        hedging_amount = None
        try:
            denomValue = HedgingCostHooksHelper.SuggestedHedgingCost(self.m_trade)
            hedging_costCurr = str(denomValue.Unit())
            hedging_amount = denomValue.Number()
        except Exception as e:
            print('Exception:', e)
            hedging_costCurr = self.m_trade.CreditBalance().Currency().Name()
            hedging_amount = 0.0
        
        return (hedging_costCurr, hedging_amount)

    def ShowHedgingCostPaymentsDialog( self, trade, notes='' ):
        self.m_trade      = trade
        self.m_notes      = notes
...
class HedgingCostReRequestPaymentsDialog( FUxCore.LayoutDialog ):
    def __init__(self, shell):
        self.m_shell    = shell
    ...
    def HandleCreate( self, dlg, layout ):
        dlg.Caption( 'Request Hedging Cost Charge' )
    ...
    def ShowHedgingCostReRequestPaymentsDialog( self ):
        builder = self.CreateLayout()
        acm.UX().Dialogs().ShowCustomDialogModal( self.m_shell, builder, self )
