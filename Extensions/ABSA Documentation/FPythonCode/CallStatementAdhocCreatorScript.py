"""--------------------------------------------------------------------------------------------------------------------------------------
MODULE
    CallStatementAdhocCreatorScript.
    
DESCRIPTION
    This module contains an AEL main script used for the adhoc generation 
    of a call deposit statement for an acquirer and counterparty.
...
def _create_ael_variable_handler():
    ...
    ael_variable_handler.add(
        ...
        mandatory=True,
        multiple=False,
        alt='The trades to generate an adhoc statement for.'
    )
    # Period Type.
    ael_variable_handler.add(
        name='period_type',
...
ael_gui_parameters = {
    'windowCaption': 'Create Adhoc Call Statement',
    'runButtonLabel': '&&Create',
    'runButtonTooltip': 'Create Adhoc Call Statement',
...
def ael_main(ael_parameters):
    ...
    """
    try:
        trade = ael_parameters['trade']
        from_date, to_date = _get_date_range(ael_parameters)
        StatementGeneral.validate_statement_date_range(from_date, to_date)
        if _should_create_adhoc_statement(trade, from_date, to_date):
            CallStatementConfirmationCreator().create_adhoc_statement_confirmation(
                trade, from_date, to_date)
...
def _should_create_adhoc_statement(trade, from_date, to_date):
    """
    Determine whether or not to create the adhoc statement.
    """
    existing_statement_confirmations = CallStatementGeneral \
        .get_existing_statement_confirmations(trade, from_date, to_date)
...
def _ensure_recreate_statement_permitted():
    ...
    if acm.User().IsAllowed(operation_name, 'Operation'):
        return
    error_message = "A call deposit statement for the specified "
...
def _confirm_recreate_statement(existing_statement_confirmations):
    """
    Confirm whether or not to create an additional call deposit
    statement when statement confirmations exist for the same
    criteria.
    """
    message = None
    if len(existing_statement_confirmations) == 1:
        message = "An existing call deposit statement was "
        message += "found for the specified criteria:\n\n"
    else:
        message = "Existing call deposit statements were "
        message += "found for the specified criteria:\n\n"
