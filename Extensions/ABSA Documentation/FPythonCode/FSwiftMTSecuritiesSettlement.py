""" Compiled: 2018-12-05 17:44:26 """

#__src_file__ = "extensions/swift/etc/FSwiftMTSecuritiesSettlement.py"
"""---------------------------------------------------------------------------------------------------------------------
MODULE
    FSwiftMTSecuritiesSettlement 

DESCRIPTION
    Module that implements the functionality required for generating 54x series of messages
...
------------------------------------------------------------------------------------------------------------------------
05-04-2019                      Tawanda Mukhalela       Rayan Govender          Added imprecise rounding to security 
                                                                                nominals settlement amount.
09-05-2019      FAOPS-505       Hugo Decloedt           Rayan Govender          Fixed swift message bug.
07-05-2020                      Cuen Edwards            Wandile Sithole         Replaced hard-coded rounding rules for 
                                                                                KES with call to FSwiftUtils.
03-08-2020      FAOPS-852       Tawanda Mukhalela       Wandile Sithole         Added check to always return Settlement
                                                                                value day on TRAD for SARB Transfers
------------------------------------------------------------------------------------------------------------------------
"""
import acm
import FSwiftMTBase
...
def GetTradeDatetimeDate(settlement):
    ...
    return _get_trade_date(allDates, settlement)


def _get_trade_date(allDates, settlement):
    """
    Gets the Trade Date based of SARB Tranfer field or Trade Value Days
    """
    trade_area = 'SARB Transfer'
    if not settlement.Trade():
        for netted_settlement in settlement.Children():
            if not netted_settlement.Trade():
                continue
            if netted_settlement.Trade().OptKey1() is None:
                continue
            if netted_settlement.Trade().OptKey1().Name() != trade_area:
                continue
            return settlement.ValueDay()

    elif settlement.Trade().OptKey1() and settlement.Trade().OptKey1().Name() == trade_area:
        return settlement.ValueDay()
...
def GetDescriptionOfSecurity(settlement):
    assert settlement.Instrument(), "Settlement has no security instrument or trade referenced by the settlement " \
                                    "has no instrument"
    product_type = settlement.Instrument().ProductTypeChlItem()
    if product_type:
...
def GetMandatoryIndicator(settlement):
    if settlement.Instrument() is not None:
...
def ParentOrAllChildrenHaveInstrumentType(settlement, instrumentType):
    ...
    if instrumentType == InsType.COLLATERAL:
        if settlement.Trade() is not None:
...
def GetRelatedRef(settlement):
    related_settlement = settlement.Children()[0]
    related_settlement_message = GetRelatedEntityMessage(related_settlement)
    field = '20C'
    field_value = GetFieldValue(related_settlement_message, field)
    start = field_value.find("FAS-")

    return field_value[start:]
...
def GetCurrencyCode(settlement):
    ...
    if settlement.Type() == SettlementType.SECURITY_DVP:
        ...
        return settlement.Trade().Currency().Name()
...
def GetApplicablePartyDetails(option, partyDetails):
    ...
    for partyInfo in partyDetails:
        applicablePartyInfo = dict.fromkeys(['countrycode', 'bic', 'name', 'address', 'safekeepingaccount',
                                             'datasourcescheme', 'partyproprietarycode'], '')
