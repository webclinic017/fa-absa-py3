...
'''
Here is how to write a custom hook that uses a different parallelization strategy:

def custom_receiver_selection(mbfObject, fObject, receivers):
    # Any strategy to select one of the receivers
    return receivers[0]

def sender_modify(mbfObject, subject):
    from FRoutingAmbaHook import do_sender_modify
    return do_sender_modify(mbfObject, subject, custom_receiver_selection)
'''
