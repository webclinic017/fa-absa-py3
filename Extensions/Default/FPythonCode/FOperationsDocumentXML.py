...
class FOperationsDocumentXML(object):
    ...
    def ExecuteAcmCode(self, xmlNode, fObject, sourceRecord):
        if not list(filter(xmlNode.hasAttribute, ['method', 'function', 'acmLoop', 'eval'])):
            return fObject
    ...
    def ProcessAcmCode(self, originalXml, originalNode, fObject, sourceRecord):
        ...
        for i in originalNode.getElementsByTagName("acmCode"):
            try:
                ...
                OneUpParent, _  = opsxmldom.GetAncestors(i)
                errorMsg = "Exception occurred when processing tag {}: {}".format(OneUpParent.tagName, str(e))
                e.args = (errorMsg,)
                raise
    ...
    def ProcessTemplate(self, template, sourceRecord):
        try:
            ...
            errorMsg = "Could not generate XML from template: {}".format(str(e))
            e.args = (errorMsg,)
            raise
    ...
    def GenerateXml(self, template, sourceRecord, overrideXML = None):
        templateXML = self.ProcessTemplate(template, sourceRecord)
