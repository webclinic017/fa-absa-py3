...
#__src_file__ = "extensions/swift/etc/FSwiftCheckBic.py"
"""----------------------------------------------------------------------------
MODULE
    FSwiftCheckBic - Module that includes SWIFT related functions.

    (c) Copyright 2005 SunGard FRONT ARENA. All rights reserved.

DESCRIPTION
    This module implements syntax checkning according to the definition of the
    SWIFT BIC code.

    FValidation can be implemented in the following way
    import ael, FSwiftCheckBic

    def validate_transaction(transaction_list, *rest):
        for (o, op) in transaction_list:
            if o.record_type == "Party" and op not in ('Delete'):
                if FSwiftCheckBic.check_party_bics(o) == 0:
                    raise Exception("FValidation:Invalid Party BIC code(s)")
    ...
    select
      p.ptyid 'Party',
      p.swift 'Party.swift',
      ael_s(p,'FSwiftCheckBic.check_party_bics',1,1,1) 'Validation',
      ael_s(p,'FSwiftCheckBic.get_party_aliases',1) 'SWIFT Party Alias',
      ael_s(p,'FSwiftCheckBic.check_party_bics',2,1,1) 'Validation'
    from party p
    order by 3,5
...
def check_bic_code(party, partyalias=None, mode=1, noLog=0):
    ...
    Example of a valid BIC: NDEASEGGYYY
    '''
    #BANKCCLLO
    ...
    #AAAAAABb
    ERR5 = "Err5: Only capital letters and numbers are allowed in the location \
            code"

    #AAAAAA21333, AAAAAA2133A
    ERR6 = "Err6: Optional branch code must be 3 characters long"

    ERR7 = "Err7: Only capital letters and numbers are allowed in the Optional \
            branch code"
    ...
    log_str = '%s (mode: %s)' % (info_str, modeStr)
    if noLog != 1:
        ael.log(log_str)

    if not partyalias and mode == 2:
        ael.log('No partyalias, mode 2, Ok returned')
        return 1

    if not party:
        ael.log('No Party deployed to FSwiftCheckBic.check_bic_code')
        return 0
    else:
        ptyid = party.ptyid

    if noLog != 1:
        log_str = "Party %s has %s = %s" % (ptyid, modeStr, partyalias)
        ael.log(log_str)
...
def check_party_bics(party, mode=0, asqlMode=0, noLog=1):
    '''
    party    - party entity
...
def get_party_aliases(party, asqlMode=0, noLog=1):
    ...
    if len(swift_pas) > 1:
        if noLog != 1:
            ael.log('Party %s has %d SWIFT aliases, first will be used.' % \
                    (party.ptyid, len(swift_pas)))
...
def test():
    ...
    test_bics = ['BANKCCLL', 'BaNKCCLL', 'BA1KCCLL', 'Ba1KCCLL', 'BAN@CCLL',
                 'BANKcCLL', 'BANKC1LL', 'BANK*CLL', 'BANKCC2L', 'BANKCCLa',
                 'BANKCC33', 'BANKCCLLXX1', 'BANKCCLLXc1', 'BANKCCLLcXX',
                 'BANKCCLL1XX', 'BANK', 'BANKBANBB']
...
def test_all_parties():
    '''Test function for all parties, both SWIFT Party alias and party.swift is
    validated.'''
