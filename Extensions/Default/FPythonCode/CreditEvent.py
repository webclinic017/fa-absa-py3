...
class CreditEventHandler( object ):
    ...
    def PayDate(cls, task):
        instrument = task.At('instrument')
        insEndDate = instrument.MainLeg().EndDate()
        eventDate = cls.PARAMS.At('settlementDate')
        if instrument.CreditEventSpec().Settlement() == 'Event' or acm.Time.DateDifference(eventDate, insEndDate) > 0:
            payDate = eventDate
        else:
            payDate = insEndDate
        return payDate
    PayDate=classmethod(PayDate)
            ...
            for tradeTask in task.At('trades'):
                ...
                if position:
                    if recovery:
                        ...
                        createInsLink = False
                        payDate = cls.PayDate(task)
                        cls.PerformRecoveryPayment( payDate, tradeTask, businessEvent )
    ...
    def GetOngoingCashFlow(cls, ins, defaultDate):
        cashFlow = None
        for leg in ins.Legs():
            if leg.LegType() == "Fixed" or leg.LegType() == "Float":
...
class CreditEventDialog( FUxCore.LayoutDialog ):
    ...
    def __init__( self, dataDict ):
        ...
        # this if statement is needed by unit tests which are run from a non GUI client
        if acm.Class() != acm.FACMServer:
            self.InitControls()
    
    """------------------ Processing ------------------"""
    
    def PerformCreditEvent( self ):
        if self.m_testmode or acm.UX().Dialogs().MessageBoxOKCancel(self.m_fuxDlg.Shell(), 3, 'Are you sure you want process this Credit Event? Entities will be created and modified.') == 'Button1':
            try:
...
def PerformCreditEventInsDef(eii):
