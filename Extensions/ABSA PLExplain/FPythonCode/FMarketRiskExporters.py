...
class PLExplainExporter(FMarketRiskExporterBase):
    ...
    riskfactorColumnsMap = {"InterestRate": ["Interest Rate Yield Delta Bucket Vertical", "Interest Rate Yield Gamma Bucket Vertical"],
                            ...
                            "InstrumentSpread": ["Instrument Spread Delta PLE", "Instrument Spread Gamma PLE"],
                            "Benchmark": ["Benchmark Delta PLE", "Benchmark Gamma PLE"],
                            "CommodityPrice": ["Portfolio Delta Implicit Commodity", "Portfolio Gamma Implicit Commodity"],
                            "Credit": ["Credit Par Delta", "Credit Gamma"],
                            "FXRate": ['Portfolio Profit And Loss Explain FX Per Risk Factor PLE'],
    ...
    def createColumns(self):
        columnConfigurations = []
        needYc = len(set(list(self.params.riskfactors)).intersection(set(["InterestRate", "InstrumentSpread"]))) != 0
        needbenchmarkYc = len(set(list(self.params.riskfactors)).intersection(set(["Benchmark"]))) != 0
        yieldCurves = self.getYieldCurves("All Yield Curves Name", list(self.params.inputYCs)) if needYc else []
        inflationCurves = self.getYieldCurves("All Inflation RF Curves Name", list(self.params.inputYCs)) \
        ...
        benchmarkCurves = self.getYieldCurves("All Benchmark RF Curves Name", list(self.params.inputYCs)) if needbenchmarkYc else []
        basisCurves = [y for y in yieldCurves if acm.FAttributeSpreadCurve[y] != None]
        creditCurves=[]
        if creditCurvesL:
            creditCurves = list(set([n.split(',')[0] for n in creditCurvesL]))
        rfCurveMap = {"Benchmark":benchmarkCurves, "Inflation": inflationCurves, "Credit":creditCurves}
        for rf in self.params.riskfactors:
            if rf in ["Benchmark", "Inflation", "Credit"]:
                ...
                columnConfigurations = self.createInterestRateColumns(columnConfigurations, basisCurves)
            elif rf == "Volatility":
