...
class FReportParametersBase( object ):
    ...
    def getObject(self, object, className):
        ...
        elif className == 'FStoredASQLQuery':
            return self.getStoredASQLQuery(object)
    ...
    def getStoredASQLQuery(self, qf):
        res = acm.FStoredASQLQuery[qf]
        if not res:
            msg = 'Query folder ' + str(qf) + ' does not exist!'
            logger.ELOG(msg)
            raise Exception(msg)
        return res
    ...
    def setPerformanceStrategy(self, performanceStrategy):
        if not performanceStrategy in self.getPerformanceStrategies():
            raise Exception("Invalid performance strategy: " + str(performanceStrategy) + \
                " valid choices = " + str(self.getPerformanceStrategies()) )
