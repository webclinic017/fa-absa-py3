...
MODULE
    ...
    RTMPortfolioSwapUtil

    (c) Copyright 2010 SunGard FRONT ARENA. All rights reserved.

DESCRIPTION

    Utility functions used by the RTMPortfolioSwap functions
...
def GetRedemptionCashFlowFromLeg(callDepositLeg, raiseException = True, isAel = True):
    ...
    return redemptionCF
#----------------------------------------------------------------------------
def GetSingleTrade(portfolioSwap, raiseException = False):
...
def CalculatePerSecurityColumnValues(portfolio, columnIds, globalSimulations):
    ...
    orNode.AddAttrNode('Instrument.InsType', 'EQUAL', 'Stock')
    orNode.AddAttrNode('Instrument.InsType', 'EQUAL', 'ETF')
    calcSpace.InsertItem(query)
