...
import RTMPortfolioSwap
reload(RTMPortfolioSwap)
...
class PortfolioSwapVariables(FBDPGui.AelVariables):
    def __init__(self):
        ...
        self.ael_variables = [
            ...
            ['fixingMode', 'Single fixing day_Fixing', 'int', [0, 1], 1, 0, 0, ttFixingMode, self.cBFixingMode],
            ...
            ['generateDividendLegs', 'Generate dividend legs', 'int', [0, 1], 1, 0, 0]]
...
def ael_main(dictionary):
    ...
    if ValidateInput(dictionary):
        ...
        spreadShort = dictionary["spreadShort"]
        payOrReceive = dictionary["payOrReceive"]
        generateDividendLegs = dictionary["generateDividendLegs"]
        ...
        pfsParameters = Util.PortfolioSwapParameters({"portfolioSwap" : pfSwap, \
                                                ...
                                                "generateDividendLegs": generateDividendLegs, \
                                                "paramDict" : dictionary})

        RTMPortfolioSwap.AssertPaymentTypes(pfsParameters)
    
        if "Open End" == pfSwap.OpenEnd() and \
                not RTMPortfolioSwap.CheckEndReturnResets(pfsParameters, resetStartDate):
        ...
        if terminate:
            RTMPortfolioSwap.TerminatePortfolioSwap(pfsParameters)
        else:
            if regenerate:
                RTMPortfolioSwap.GeneratePortfolioSwap(pfsParameters, resetEndDate)
            else:
                RTMPortfolioSwap.RestorePortfolioSwapEndDate(pfsParameters, resetEndDate)
                RTMPortfolioSwap.AddNewLegs(pfsParameters, resetStartDate, resetEndDate)
                RTMPortfolioSwap.AddNewCashFlows(pfsParameters, resetStartDate, resetEndDate)

                RTMPortfolioSwap.ClearResetsFromDateUntilEnd(pfsParameters, resetStartDate)
                RTMPortfolioSwap.ClearCashFlowsFromDateUntilEnd(pfsParameters, resetStartDate)
                #RTMPortfolioSwap.ClearPaymentsFromDateUntilEnd(pfsParameters, resetStartDate)
                
                RTMPortfolioSwap.GenerateEndReturnResets(pfsParameters, resetStartDate)
            acm.PollDbEvents()

            RTMPortfolioSwap.PerformDailyUpdatesInPeriod(pfsParameters, resetStartDate, resetEndDate)
            ...
            if not acm.Time.DateDifference(resetStartDate, pfSwap.StartDate()) and \
                ...
                RTMPortfolioSwap.TransferPreviousPortfolioSwapData(pfsParameters, resetStartDate)
