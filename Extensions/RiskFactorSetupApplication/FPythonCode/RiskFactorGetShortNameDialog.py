...
def Show(shell, caption, label, initialText, maxLength=-1, checkAlphaNum=False, names = None):
    customDlg = RiskFactorGetShortNameDialog()
...
class RiskFactorGetShortNameDialog (FUxCore.LayoutDialog):
    def __init__(self):
        self.m_okButton = 0
        self.m_edit = 0
        self.m_initialText = ''
    ...
    def HandleApply( self ):
        ...
        if self.m_names != None:
            if s in self.m_names :
                acm.UX().Dialogs().MessageBoxInformation(self.m_fuxDlg.Shell(), 'Name already exists')
                ret = None
