...
#__src_file__ = "extensions/RegulatoryInfo/../SwiftIntegration/RegulatoryInfo/General/FRegulatoryInfoAMB.py"
"""------------------------------------------------------------------------
MODULE
    FRegulatoryInfoAMB -
DESCRIPTION:
...
acm_verison = integration_utils.get_acm_version_override()
...
class RegulatoryInfoAMB(object):
    def __get_msg_dict(self, col_name, col_value, attr_dict):
        msgDict = None
        if acm_verison >= 2016.4:
            adm_col_name = None
            if acm_verison >= 2017.2 and (attr_dict[col_name] in FRegulatoryUtils.add_info_dict_17_2):
                adm_col_name = adm_column_name[attr_dict[col_name]]
            elif acm_verison >= 2017.1 and (attr_dict[col_name] in FRegulatoryUtils.add_info_dict_17_1):
                adm_col_name = adm_column_name[attr_dict[col_name]]
            elif acm_verison >= 2016.5 and (attr_dict[col_name] in FRegulatoryUtils.add_info_dict_16_4 or attr_dict[col_name] in FRegulatoryUtils.add_info_dict_16_5):
                adm_col_name = adm_column_name[attr_dict[col_name]]
            elif acm_verison >= 2016.4 and (attr_dict[col_name] in FRegulatoryUtils.add_info_dict_16_4):
                adm_col_name = adm_column_name[attr_dict[col_name]]
            if adm_col_name == 'CLR_HOUSE_PTYNBR.PTYID' and acm_verison == 2016.4:
    ...
    def reg_info_to_amb(self, col_name, col_value, attr_dict, api_call, parent_col_name = None, parent_col_value = None, parent_required = True, src_currency = None, dest_currency = None, market = 'ECBFIX'):
        ...
        msgDict = None
        if not attr_dict.has_key(col_name):
        ...
        if attr_dict[col_name] == 'FUNCTION': # Means: just mapping is not enough, call fn
            ...
            if col_name in attr_bool_enum_validate_17_2 and acm_verison >= 2017.2:
                col_value = FRegulatoryUtils.validate_enum_bool(col_value, col_name)
            elif col_name in attr_bool_enum_validate_17_1 and acm_verison >= 2017.1:
                col_value = FRegulatoryUtils.validate_enum_bool(col_value, col_name)
                ...
                if col_name == 'TradingTerminationDate':
                    ...
                    if acm_verison >= 2017.2 and (col_name in adm_trade_reg_fields or col_name in adm_ins_reg_fields) and attr_dict[col_name] in FRegulatoryUtils.add_info_dict_17_2:
                        pass
                    elif acm_verison >= 2017.1 and (col_name in adm_trade_reg_fields or col_name in adm_ins_reg_fields) and attr_dict[col_name] in FRegulatoryUtils.add_info_dict_17_1:
                        pass
                    else:
                ...
                if attr_party_type_validate.has_key(col_name):
        ...
        if not msgDict and attr_alias_dict.has_key(col_name):
        ...
        if not msgDict:
            ...
            if msgDict.has_key('ADDITIONALINFO') and acm_verison >= 2016.5 and \
            ...
            if col_name in adm_trade_reg_fields:
                if not msgDict.keys()[0] == 'TRADEREGULATORYINFO':# this is created in the case of Waiver and should not get created again
                    parent = 'TRADEREGULATORYINFO'
            elif col_name in adm_ins_reg_fields:
                if not msgDict.keys()[0] == 'INSTRREGULATORYINFO':# this is created in the case of Commodity classification and should not get created again
                    parent = 'INSTRREGULATORYINFO'
            if parent and (not msgDict.has_key('ADDITIONALINFO')):
                msgDict = {parent : msgDict}
        return msgDict
    ...
    def tradeRegInfo2amb(self, col_name, col_value, parent_col_name = None, parent_col_value = None, parent_required = True):
        ...
        if parent_required and ((col_name not in ['ShortSellIndicator'] and acm_verison >= 2016.5 ) or (acm_verison < 2016.5)):
...
class AMBARegIntoToAddInfoConverter(object):
    def __init__(self, amba_message):
        ...
        self.__adm_vs_addinfo_dict  = dict(list(zip(adm_column_name.values(), adm_column_name.keys())))
        ...
        self.__addinfo_vs_trd_api_dict = dict(list(zip(trade_attr_dict.values(), trade_attr_dict.keys())))
        self.__addinfo_vs_ins_api_dict = dict(list(zip(instrument_attr_dict.values(), instrument_attr_dict.keys())))
    ...
    def insert_into_amba_message(self, amba_message, insert_dict):
        ...
        if not self.__parent_node:
            self.__parent_node = insert_dict.keys()[0]
            if self.__parent_node not in ['INSTRREGULATORYINFO', 'TRADEREGULATORYINFO']:
                self.__child_reg_node = insert_dict[insert_dict.keys()[0]].keys()[0]
            else:
        ...
        if self.__parent_node in ['INSTRREGULATORYINFO', 'TRADEREGULATORYINFO']:
            self.__child_reg_node = self.__parent_node
        val = None
        if insert_dict.keys() and insert_dict.keys()[0] == self.__parent_node:
            val = insert_dict[insert_dict.keys()[0]]
        else:
            vals = insert_dict[orig_parent]
            if vals.keys() and vals.keys()[0] == self.__parent_node:
                val = vals[vals.keys()[0]]
        mb_msg = None
        if val.keys()[0] == 'ADDITIONALINFO':
            bFound = False
            add_key = None
            add_val = None
            if mb_node.mbf_find_object('ADDITIONALINFO', 'MBFE_BEGINNING') :
                ...
                if child_handleVal.mbf_get_value() == val.values()[0]['ADDINF_SPECNBR.FIELD_NAME']:#TESTED
                    bFound = True
                    add_info_node_val = self.get_tagvalue(handleVal, 'VALUE')
                    if add_info_node_val <> val.values()[0]['VALUE']:
                        handleVal.mbf_replace_string('VALUE', str(val.values()[0]['VALUE']))
            if not bFound:#TESTED
                mb_msg = mb_node.mbf_start_list(val.keys()[0])
                for add_info_val in val.values()[0]:
                    mb_msg.mbf_add_string(add_info_val, str(val.values()[0][add_info_val]))
                mb_msg.mbf_end_list()
        else:
            if not mb_node.mbf_find_object(self.__child_reg_node, 'MBFE_BEGINNING') and self.__child_reg_node != self.__parent_node:
                ...
                if self.__child_reg_node == self.__parent_node:#it means we are directly passing the Reg object
                    add_key = val.keys()[0]
                    add_val = val.values()[0]
                else:
                    add_key = val.values()[0].keys()[0]
                    add_val = str(val.values()[0][val.values()[0].keys()[0]])
                mb_msg.mbf_add_string(add_key, str(add_val))
                if self.__child_reg_node == 'INSTRUMENTALIAS':
                    mb_msg.mbf_add_string(val.values()[0].keys()[1], str(val.values()[0][val.values()[0].keys()[1]]))
                mb_msg.mbf_end_list()
            else:
                location = None
                key_node = None
                ...
                if tagExists:
                    ...
                    if self.__child_reg_node == self.__parent_node:
                        key_node = val.keys()[0]
                        node_value = val.values()[0]
                    else:
                        key_node = val.values()[0].keys()[0]
                        node_value = str(val.values()[0][val.values()[0].keys()[0]])
                    if key_node == 'ADDITIONALINFO':
                        key_vals = val.values()[0][val.values()[0].keys()[0]].keys()
                        key_val = val.values()[0][val.values()[0].keys()[0]].keys()[0]
                        val_val = val.values()[0][val.values()[0].keys()[0]]['ADDINF_SPECNBR.FIELD_NAME']
                        while tagExists:
                            ...
                            if add_info_node_name == val_val:
                                bFound = True
                                if add_info_node_val <> val.values()[0][val.values()[0].keys()[0]]['VALUE']:
                                    tagExists.mbf_replace_string('VALUE', str(val.values()[0][val.values()[0].keys()[0]]['VALUE']))
                                break
                            else:
                                tagExists = location.mbf_next_object()
                        if not bFound:
                            location = location.mbf_start_list(val.values()[0].keys()[0])
                            location.mbf_add_string(key_vals[0], str(val.values()[0][val.values()[0].keys()[0]].values()[0]))
                            location.mbf_add_string(key_vals[1], str(val.values()[0][val.values()[0].keys()[0]].values()[1]))
                    else:
                        location.mbf_replace_string(key_node, str(node_value))
                        if len(key_nodes) > 1:
                            if self.__child_reg_node != self.__parent_node:
                                tagExists = location.mbf_find_object(val.values()[0].keys()[1], 'MBFE_BEGINNING')
                                if tagExists:
                                    location.mbf_replace_string(val.values()[0].keys()[1], str(val.values()[0].values()[1]))
                                else:
                                    location.mbf_add_string(val.values()[0].keys()[1], str(val.values()[0].values()[1]))
                            else:
                                tagExists = location.mbf_find_object(val.keys()[1], 'MBFE_BEGINNING')
                                if tagExists:
                                    location.mbf_replace_string(val.keys()[1], str(val.values()[1]))
                                else:
                                    location.mbf_replace_string(val.keys()[1], str(val.values()[1]))
                                
                else:
                    if self.__child_reg_node == self.__parent_node:
                        location.mbf_add_string(val.keys()[0], str(val.values()[0]))
                        if len(key_nodes) > 1:
                            location.mbf_add_string(val.keys()[1], str(val.values()[1]))
                    else:
                        try:
                            key_vals = val.values()[0][val.values()[0].keys()[0]].keys()
                            location = location.mbf_start_list(val.values()[0].keys()[0])
                            ...
                            key_vals = val.values()[0].keys()
                            for each_key in val.values()[0].keys():
                                location.mbf_add_string(each_key, str(val.values()[0][each_key]))
    ...
    def create_add_info_tags(self, amba_node, each_element):
        ...
        if tagExists:
            ...
            if self.__linked_reg_attr_to_delete.has_key(each_element):
            ...
            if self.__acm_version_lookup.has_key(self.__acm_version):
            ...
            if b_create_add_info or b_change_reg_name:
                ...
                if b_change_reg_name:
                    ...
                    if key == 'regClearingIsMandatory':
                        key = 'regClearingIsMandat'
                    if self.__addinfo_vs_ins_api_dict.has_key(key):
                ...
                print "not creating AddInfo for %s as ADM column exists for this attribute in version %s"%(each_element, self.__acm_version)
        ...
        for each_reg in self.__all_regs:
            ...
            if amba_node:
                ...
                if each_reg in ['TRADE.TRADEREGULATORYINFO', 'TRANSACTION.TRADE.TRADEREGULATORYINFO']:
                    ...
                    if mb_node:
                        print "There are no RegulatoryInfo related attributes on the trade"
                    else:
                        self.__parent_node = 'TRADE'
                        self.__child_reg_node = 'TRADEREGULATORYINFO'
                if each_reg in ['INSTRUMENT.INSTRREGULATORYINFO', 'TRANSACTION.INSTRUMENT.INSTRREGULATORYINFO']:
                    ...
                    if mb_node:
                        print "There are no RegulatoryInfo related attributes on the instrument"
                    else:
                        self.__parent_node = 'INSTRUMENT'
                        self.__child_reg_node = 'INSTRREGULATORYINFO'
