...
#__src_file__ = "extensions/RegulatoryInfo/../SwiftIntegration/RegulatoryInfo/RegulatoryInfoLib/FInstrumentClassification.py"
import acm, ael
...
class FInstrumentClassification(object):
    ...
    def CurrSwap(self):
        ...
        if all(map(lambda leg: leg.LegType() == 'Fixed', legs)):
        ...
        elif any(map(lambda leg: leg.LegType() == 'Fixed', legs)) and any(
                map(lambda leg: leg.LegType() == 'Float', legs)):
            rts2_instype = 'Interest Rate Derivatives'
        ...
        elif all(map(lambda leg: leg.LegType() == 'Float', legs)):
            rts2_instype = 'Interest Rate Derivatives'
    ...
    def Future_Forward(self):
        ...
        if self.__instrument.Underlying() and self.__instrument.Underlying().InsType() == 'CurrSwap':
            ...
            for leg in self.__instrument.Legs():
                ...
                if all(map(
                        lambda ins: ins.Instrument().InsType() in ['Stock', 'EquityIndex', 'ETF', 'Depositary Receipt',
                                                                   'Dividend Point Index'], instrument_map)):
                    rts2_instype = 'Equity Derivatives'
        ...
        if underlying and underlying.InsType() in ['Commodity', 'Commodity Variant', 'Commodity Index']:
            try:
                ...
                print "RegulatorySupport package needs to be installed to use this functionality"
        ...
        if self.__instrument.Underlying():
            ...
            if self.__instrument.Underlying().InsType() in ['Commodity', 'Commodity Variant', 'Commodity Index']:
                ...
                if all(map(
                        ...
                        self.__instrument.Underlying().InstrumentMaps())):
                    rts2_instype = 'Equity Derivatives'
                    rts2_subtype = 'Other equity derivatives'
                    fpml_code = 'Other'
        return rts2_instype, rts2_subtype, fpml_code
        ...
        for leg in self.__instrument.Legs():
            if leg.LegType() == 'Total Return' and leg.FloatRateReference():
                if leg.FloatRateReference().InsType() in ['Commodity', 'Commodity Variant', 'Commodity Index']:
                    ...
                    rts2_subtype = 'Swaps'
                    fpml_code = 'Swap'
                elif leg.FloatRateReference().InsType() == 'Combination' and all(map(
                        lambda ins: ins.Instrument().InsType() in ['Stock', 'EquityIndex', 'ETF', 'Depositary Receipt',
                                                                   'Dividend Point Index'],
                        leg.FloatRateReference().InstrumentMaps())):
                    rts2_instype = 'Equity Derivatives'
    ...
    def __instrument_classification(self):
        ...
        try:
            ...
            if instrument_type in ['Combination', 'Commodity', 'Curr', 'Commodity_Index', 'Commodity_Variant',
                ...
                print "Could not infer the instrument classification", e
