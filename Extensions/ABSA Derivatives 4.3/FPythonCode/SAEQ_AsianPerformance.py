...
#Developer:              Zaakirah kajee, Stephen Zoio, Paul Jacot-Guillarmod

#CR Number:              275496, 517213, 527888, 550450
...
class AsianPerformance:
    def __init__(
        ...
        volmap = self.getVols(valuationDate, instrumentOid, exoticEvents, mod_strike, volatilityStructure)
            ...
            if e.Type() == type:
                ...
                if acm.Time.DateDifference(d, valuationDate) >= 0 or (valuationDate > acm.Time().DateToday() and d >= acm.Time().DateToday()):
                    ...
                    if valuationDate > acm.Time().DateToday() and d < valuationDate:
                        certain = True
                        underlyingSpotPrice = fixedUnderlyingSpotPrice
                    
                    if d == acm.Time().DateToday():
                        certain = True
                    
                    val = (underlyingSpotPrice - divpv)*exp(timetocarry*carrycost)
    ...
    def getVols(self, valuationDate, instrumentOid, exoticEvents, strikePrice, volatilityStructure):
        v = acm.FDictionary()
