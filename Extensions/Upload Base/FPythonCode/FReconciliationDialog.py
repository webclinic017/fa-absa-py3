...
#__src_file__ = "extensions/business_data_upload/../reconciliation/etc/FReconciliationDialog.py"
"""--------------------------------------------------------------------------
MODULE
    FReconciliationDialog
...
class ReconciliationDialog(FRunScriptGUI.AelVariablesHandler):
    ...
    def __init__(self, namespace=None):
        ...
        variables.extend(self.getFormatDateTime())
        #tab=Logging
    ...
    def getParameters(ael_params):
        if ael_params['FormatDateTime']:
            file_name  = str(ael_params['Filename'].SelectedFile())
            utc = ael_params['FormatDateTimeUTC']
            file_name = setDateFormats(file_name, utc)
            ael_params['Filename'].SelectedFile(file_name)
        paramClass = collections.namedtuple('ReconciliationParameters', ael_params.keys())
    ...
    def getOverrideReRunCheck(self):
        ...
        return (
            ('ForceReRun', '_'.join((label, tab)), 'string', ["1", "0"], "0", 0, 0, tooltip),
            )
    ...
    def toggle_utc_ctrl(self, index, fieldArray):
        updateIndex = index + 1
        if fieldArray[index] == "1":
            self.ael_variables[updateIndex][9] = 1
        else:
            self.ael_variables[updateIndex][9] = 0
            fieldArray[updateIndex] = "0"
        return fieldArray
        
    def getFormatDateTime(self, tab='date'):
        tooltip = 'If selected, datetime format directions in the file name, such as "%Y%m%d", are replaced with values of todays date.'
        tooltipUTC = 'If selected, date time format directions in the filename are replaced with the current UTC date. If not, local time is used.'
        return (
            ('FormatDateTime', '_'.join(('Format date', tab)), 'bool', ["1", "0"], "0", 0, 0, tooltip, self.toggle_utc_ctrl, 1),
            ('FormatDateTimeUTC', '_'.join(('UTC', tab)), 'bool', ["1", "0"], "0", 0, 0, tooltipUTC, None, 1),
            )
...
def setDateFormats(fileName, utc):
    directives = ('%%', '%a', '%A', '%w', 
                  ...
                  '%Z', '%j', '%U', '%W', 
                  '%c', '%x', '%X', '%d')
    now = datetime.datetime.utcnow() if utc is True else datetime.datetime.now()
    for d in directives:
