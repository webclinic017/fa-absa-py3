...
import acm
import FUxCore
from CVAUtils import CVAAttributeMapper, CVAStateChartConstants
import traceback
import sys
...
class CVAPaymentsDialog( FUxCore.LayoutDialog ):
    ...
    def CreateLayout( self ):
        ...
        b.      BeginHorzBox()
        b.              BeginVertBox()
        b.                      AddLabel('serverAmountLabel', 'CVA ( XYZ ) ')
        self.                   m_currBinding.BuildLayoutPart(b, 'Currency')
        b.                      AddLabel('clientAmountLabel', 'CVA ( XYZ ) ')
        b.              EndBox()
        b.              BeginVertBox()
    ...
    def HandleCreate( self, dlg, layout ):
        ...
        try:
            self.m_serverCurrName, self.m_serverAmountCalcValue = self._suggestCVA()
            
            self.m_dlg = dlg
            
            dlg.Caption( 'CVA Charge' )
    ...
    def UpdateControls(self):
        ...
        serverCurrLabel = 'CVA ( %s )' % self.m_serverCurrName
        ...
        clientCurrLabel = 'CVA ( %s )' % self.m_currObject.Name()
    ...
    def _suggestCVA( self ):
        cvaCurr = ''
        cvaAmount = None
                
        try:
            denomValue = self.m_mapper.GetIncrementalCVA( self.m_trade )
            cvaAmount = denomValue.Number()
            cvaCurr = str(denomValue.Unit())
        except Exception as e:
            print ('Exception:', e)
            cvaCurr = self.m_trade.CreditBalance().Currency().Name()
            cvaAmount = 0.0
        
        return (cvaCurr, cvaAmount)

    def ShowCVAPaymentsDialog( self, trade, notes='' ):
        self.m_trade      = trade
        self.m_notes      = notes
        self.m_mapper = CVAAttributeMapper()
...
class CVAReRequestPaymentsDialog( FUxCore.LayoutDialog ):
    def __init__(self, shell):
        self.m_shell    = shell
    ...
    def HandleCreate( self, dlg, layout ):
        dlg.Caption( 'Request CVA Charge' )
    ...
    def ShowCVAReRequestPaymentsDialog( self ):
        builder = self.CreateLayout()
        acm.UX().Dialogs().ShowCustomDialogModal( self.m_shell, builder, self )
