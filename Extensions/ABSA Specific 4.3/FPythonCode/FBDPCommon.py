...
def sendMail(TO, SUBJECT, MSG):
    ...
    FROM = "PRIME client %s <ABCapITRTBFrontArena@absa.africa>" % (ENVIRONMENT)
    BODY = string.join((
        "From: %s" % FROM,
        "To: %s" % TO,
        "Subject: %s" % SUBJECT,
...
def toDate(strDate, calOrCurr=None):
    ...
    if strDate and type(strDate) == type(''):
        ...
        elif strDate == '5 OPEN DAYS':
            return calOrCurr.AdjustBankingDays(acm.Time.DateToday(), -5)
        elif strDate == '15 OPEN DAYS':
            return calOrCurr.AdjustBankingDays(acm.Time.DateToday(), -15)
        elif strDate == '25 OPEN DAYS':
            return calOrCurr.AdjustBankingDays(acm.Time.DateToday(), -25)           
        else:
...
class FBDPQuerySelection:
    ...
    def Run(self, time_execution=False):
        ...
        def typeCast(value, result_type):
            if isinstance(result_type, types.TypeType):
