...
def _InitializeAllEsc ():
    ...
    _AllEsc.update({ six.u('.'): pyxb.utils.str.WildcardEsc })
    bs = six.chr(0x5c)
    for k, v in six.iteritems(pyxb.utils.str.SingleCharEsc):
        _AllEsc[bs + six.text_type(k)] = v
    for k, v in six.iteritems(pyxb.utils.str.MultiCharEsc):
        _AllEsc[bs + six.text_type(k)] = v
    for k, v in six.iteritems(pyxb.utils.str.catEsc):
        _AllEsc[bs + six.text_type(k)] = v
    for k, v in six.iteritems(pyxb.utils.str.complEsc):
        _AllEsc[bs + six.text_type(k)] = v
    for k, v in six.iteritems(pyxb.utils.str.IsBlockEsc):
        _AllEsc[bs + six.text_type(k)] = v
_InitializeAllEsc()
...
def _MatchPosCharGroup(text, position):
    ...
    while True:
        ...
        if ch == six.u('['):
            ...
            # End
            break
        elif ch == six.chr(0x5c): # backslash
    ...
    while position < len(pattern):
        ...
        if cg is None:
            ...
            if ch == six.u('^') or ch == six.u('$'):
                ...
                # be escaped.
                new_pattern_elts.append(six.chr(0x5c) + ch)
            else:
