...
'''----------------------------------------------------------------------------------------------------------
MODULE                  :       FC_HEART_BEAT_SYSTEM_01_ATS
PROJECT                 :       FX onto Front Arena
PURPOSE                 :       This module will retreive the system statistics where the Front Cache ATSs
                                will be running. Each Server/Machine which hosts Front Cache ATSs should have
                                one SYSTEM Heart Beat ATS running.
...
try:
    from FC_HEART_BEAT_SYSTEM_ATS_WORKER import FC_HEART_BEAT_SYSTEM_ATS_WORKER as HEART_BEAT_SYSTEM_ATS_WORKER
except ImportError as e:
...
def work():
    global worker
    if not worker:
        UTILS.ErrorHandler.processError(None, EXCEPTION('The worker variable in module %s is not instantiated. '
                                                        'The ATS will not be able to process any messages.' %__name__,\
                                                        traceback, 'CRITICAL', None), __name__)
    else:
        worker.work()
...
'''
def start():
    UTILS.Logger.flogger.info('Starting the ATS %s at %s' %(__name__, datetime.now()))
    global worker
    if not worker:
        worker = HEART_BEAT_SYSTEM_ATS_WORKER()
    worker.start()
...
def stop():
    ...
    if not worker:
        UTILS.ErrorHandler.processError(None, EXCEPTION('The worker variable in module %s is not instantiated. '
                                                            'The ATS will not be able to process stop gracefully.' %__name__,\
                                                            traceback, 'MEDIUM', None), __name__)
    else:
        worker.stop()
