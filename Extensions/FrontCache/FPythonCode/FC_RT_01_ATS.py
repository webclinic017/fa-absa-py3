...
PURPOSE                 :       This ATS is the starting point for the Real Time Event processing. This module
                                name FC_RT_01_ATS will be the module_name in the ATS installation. It will 
                                subscribe to real time events being posted by the Front Cache Real Time AMBAs
                                and convert these Front Arena AMBA messages to Event Request AMBA messages.
                                These Event Request AMBA messages will be placed onto the AMB for the Front
                                Cache Request Coordinating ATS to pick up.
...
try:
    from FC_RT_ATS_WORKER import FC_RT_ATS_WORKER as RT_ATS_WORKER
except ImportError, e:
...
'''----------------------------------------------------------------------------------------------------------
work function which the ATS will call once started. The method work kicks of the processing of messages from
the AMB.
----------------------------------------------------------------------------------------------------------'''
def work():
    global worker
...
'''----------------------------------------------------------------------------------------------------------
start function which the ATS will call when the ATS is starting. The method start creates all the AMB readers,
AMB writer handlers to ensure that the ATS is fully up and running before it starts working.
----------------------------------------------------------------------------------------------------------'''
def start():
    UTILS.Logger.flogger.info(UTILS.Constants.fcFloggerConstants.STARTING_ATS_S_AT_S %(__name__, datetime.now()))
    global worker
    if not worker:
        worker = RT_ATS_WORKER()
    worker.start()

'''----------------------------------------------------------------------------------------------------------
stop function which the ATS will call when the ATS is stopping. This method closes all AMB connections in the
