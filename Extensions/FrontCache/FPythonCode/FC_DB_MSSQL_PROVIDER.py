...
class FC_DB_MSSQL_PROVIDER:
    ...
    def getSqlConnection(self):
        ...
        while dbRetryCount < UTILS.Parameters.fcGenericParameters.DataBaseRetryThreshold:
            try:
                return odbc.connect(self.connectionString, autocommit=True)
            except Exception, e:
            ...
            try:
                ...
                for item in params:
                    if len(sql_b) == 0:
                        ...
                        sql_b += ",(" + str(self.BuildSqlString(item)) + ")"
                ...
                if controlMeasureResults is not None:
                    for item in controlMeasureResults:
                        if len(sql_d) == 0:
                            ...
                            sql_d += ",(" + str(self.BuildSqlString(item)) + ")"

                sql = "SET NOCOUNT ON;" + sql_a + sql_b + sql_c + sql_d + ";exec " + sqlParam + " @TradeTVP, @ControlMeasureTVP"
                #print sql
                result = cursor.execute(sql)
                return result
            except Exception, error:
