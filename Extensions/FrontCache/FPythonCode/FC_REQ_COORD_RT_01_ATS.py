...
'''----------------------------------------------------------------------------------------------------------
MODULE                  :       FC_REQ_COORD_01_ATS_ST
PROJECT                 :       FX onto Front Arena
PURPOSE                 :       This module is the entry point for the Front Cache Request Event Coordinator ATS for single trades.
                                This ATS will subscribe to a SINGLE TRADE REQUEST type messages from the AMB, log them
                                to a database, give the request to the SINGLE TRADE REQUEST ATS and send the RESPONSE
                                message to the AMB for subscribers.
...
try:
    from FC_REQ_COORD_RT_ATS_WORKER import FC_REQ_COORD_RT_ATS_WORKER as REQ_COORD_RT_ATS_WORKER
except ImportError, e:
...
def start():
    ...
    global worker
    if not worker:
        worker = REQ_COORD_RT_ATS_WORKER()
    worker.start()
