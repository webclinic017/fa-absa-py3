            ...
            for column_id, results in pos_key_dict.items():
                ...
                for dims, result in results.items():
                    ...
                    if isinstance(values, dict):
                        values = []
                        for horizons, value in result.items():
...
class MeasurementBaseSBAWriter(FRTBBaseWriter.Writer):
    ...
    def _getRows(self, header):
        ...
        rows = []
        for data_key, measurements in self._results.items():
...
class VegaVolatilitySBAWriter(MeasurementBaseSBAWriter):
    ...
    def _getRows(self, header):
        rows = MeasurementBaseSBAWriter._getRows(self, header)
        for data_key, measurements in self._results.items():
            measurements.clear()
            del measurements
            measurements = None #mra
    ...
    def write(self):
        retval = MeasurementBaseSBAWriter.write(self)
