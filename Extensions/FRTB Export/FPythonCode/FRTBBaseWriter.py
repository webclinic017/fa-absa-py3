...
class ResultsCollector(object):
    ...
    def getData(self):
        ...
        for pos_key, pos_key_dict in self._result_dictionary.items():
            ...
            for column_id, result in pos_key_dict.items():
...
class Writer(object):
    ...
    def _getDefaultTradeAttributes(self, trade_attrs):
        #print self._trade_attributes
        attrs = list(trade_attrs)
        #print attrs
        asql_port = self.PortfolioFromAttributes(
            self._trade_attributes, attrs)
        if asql_port:        
            asql_port_trades = asql_port.Trades()
            if asql_port_trades.Size() > 0:
                attrs.extend(self._GetAdditionalAttrValues(
                        asql_port_trades[0]))
