...
max_length = int(config_parameters.get_paramvalue('FREGULATORY_TRANSACTION_LIMIT'))
run_modular = config_parameters.get_paramvalue('FREGULATORY_MODULAR_FLAG')
...
class FRegulatoryInfoUpgrade(object):
    ...
    def rename_parent_add_info(self):
        ...
        if self.b_parent:
            for add_info in list(self.add_info_dict.keys()):
    ...
    def copy_parent_add_info_to_child(self, add_info_name, add_info_api, add_info_val, reg_info_api, reg_info):
        ...
        if str(add_info_val) != 'None':
            ...
            try:
                ...
                FRegulatoryLogger.DEBUG(logger, "Setting RegulatoryInfo attribute <%s> on <%s> of id/name <%s> with value <%s>."%(reg_info_api, self.object_type, self.object_id, add_info_log_val))
                if reg_info_api in log_name or reg_info_api in log_timestamps:
                    eval('reg_info.' + reg_info_api + "('" + add_info_log_val + "')")
                elif add_info_name in different_handling:
                ...
                FRegulatoryLogger.ERROR(logger, "Error occurred while copying <%s> from AddInfo <%s> on <%s> of id/name <%s> to the RegulatoryInfo instance. Error: <%s>"%(add_info_log_val, add_info_name, self.object_type, self.object_id, str(e)))
    ...
    def verify_parent_add_info_on_child(self, add_info_name, add_info_api, reg_info_api, b_reg = False):
        ...
        try:
            ...
            # this exception handling is added as CommodityProduct API is not available on 16.5 and below
            pass
        if add_info_name not in list(different_handling.keys()):
    ...
    def upgrade_reg_info(self, add_info_name, parent_object, reg_info, renamed = False):
        ...
        if self.b_parent:
            ...
            if not acm.FAdditionalInfoSpec[add_info_name]:
                if renamed and acm.FAdditionalInfoSpec[add_info_api]:
                    pass
                else:
                    return reg_info
            add_info_val = None
            ...
            reg_info = self.copy_parent_add_info_to_child(add_info_name, add_info_api, add_info_val, reg_info_api, reg_info)
            if not run_modular:
    ...
    def delete_addinfo_val(self, add_info_spec, acm_object):
    ...
    def delete_add_info_specs(self):
        ...
        for add_info_spec in list(self.add_info_dict.keys()):
            addInf = acm.FAdditionalInfoSpec[add_info_spec]
            if addInf:
                add_infos = acm.FAdditionalInfo.Select("addInf = %d"%addInf.Oid())
                if not add_infos:
...
class FTradeRegulatoryInfoUpgrade(FRegulatoryInfoUpgrade):
    ...
    def set_object_type_id(self):
        ...
        try:
            ...
            FRegulatoryLogger.ERROR(logger, "Error while attempting to get the object type that the upgrade script is being executed upon")
        ...
        if short_sell_indicator and self.integration_utils.get_acm_version_override() >= 2017.2:
            ...
            if acm_object.BusinessEventTradeLinks() and acm_object.BusinessEventTradeLinks()[0].BusinessEvent():
                if acm_object.BusinessEventTradeLinks()[0].BusinessEvent().EventType() == short_sell_indicator:
                    self.delete_addinfo_val('regShortSell', acm_object_add_info_on)
            else:
                FRegulatoryLogger.ERROR(logger, \
                    "There is a mismatch while migrating the ShortSellIndicator. Hence, AddInfo RegShortSell is not being deleted for trade <%d>"%acm_object.Oid())
    ...
    def OurInvestmentDecider(self, add_info_val, reg_info):
        ...
        if not contact_obj:
            contact_obj = self.get_contact(reg_info.OurOrg(), add_info_val)
        FRegulatoryLogger.DEBUG(logger, "Setting RegulatoryInfo OurInvestmentDecider with value <%s> on <%s>"%(contact_obj.Fullname(), reg_info.Trade().Oid()))
    ...
    def TheirInvestmentDecider(self, add_info_val, reg_info):
        ...
        FRegulatoryLogger.DEBUG(logger, "Setting RegulatoryInfo TheirInvestmentDecider with value <%s> on <%s>"%(contact_obj.Fullname(), reg_info.Trade().Oid()))
    ...
    def TheirTrader(self, add_info_val, reg_info):
        ...
        if not contact_obj:
            contact_obj = self.get_contact(reg_info.TheirOrg(), add_info_val)
        FRegulatoryLogger.DEBUG(logger, "Setting RegulatoryInfo TheirTrader with value <%s> on <%s>"%(contact_obj.Fullname(), reg_info.Trade().Oid()))
...
class FContactRegulatoryInfoUpgrade(FRegulatoryInfoUpgrade):
    ...
    def delete_contact_reg_info(self, contact):
        ...
        for add_info_spec in add_info_specs:
            ...
            self.delete_addinfo_val(add_info_spec, contact)
        ...
        try:
            ...
            if contact.Person() and national_id:
                if contact.Person().NationalId() != national_id:
                    FRegulatoryLogger.INFO(logger, "The contact <%s> with Oid <%d> on Party <%s> with NationalId <%s> is already linked to a Person with NationalId <%s>. Please check the linking."%(\
                    contact.Fullname(), contact.Oid(), contact.Party().Name(), national_id, contact.Person().NationalId()))
            elif national_id:
                ...
                if not person:
                    ...
                    person.Commit()
                    FRegulatoryLogger.INFO(logger, "Creating Person with NationalId <%s> and linking it to Contact  <%s> with Oid <%d> on Party <%s>"%(national_id, contact.Fullname(), contact.Oid(), contact.Party().Name()))
                    person_id = person.Oid()
                if person:
                    person_id = person.Oid()
                if person_id:
                    FRegulatoryLogger.INFO(logger, "Linking contact <%s> with Oid <%d> on Party <%s> with NationalId <%s> to Person with Oid <%d>."%(contact.Fullname(), contact.Oid(), contact.Party().Name(), national_id, person_id))
                    contact.Person(person)
                    contact.Commit()
            elif not contact.Person():
                FRegulatoryLogger.WARN(logger, "The NationalId details for contact <%s> with Oid <%d> on Party <%s> is not available. Hence, cannot link it to a Person object."%(contact.Fullname(), contact.Oid(), contact.Party().Name()))
        except:
            pass
...
def upgrade_trade_reg_info():
    ...
    reg_infos_to_commit = []
    trd_counter = 0
    query = """select trdnbr from Trade"""
    col, trades_info = ael.asql(query)
    total_trd_counter = len(trades_info[0])
    add_info_dict = get_reg_info_dict('trade')
    currentTrade = 0
    for trd in trades_info[0]:
        try:            
            currentTrade = trd[0]            
            trade = acm.FTrade[trd[0]]
            trade_reg_info = trade.RegulatoryInfo()
            ...
            for add_info in list(add_info_dict.keys()):
                trade_reg_info = upgrade_obj.upgrade_reg_info(add_info, trade, trade_reg_info)
            for add_info in list(add_info_dict.keys()):
                if add_info in different_handling:
                    trade_reg_info = upgrade_obj.upgrade_reg_info(add_info, trade, trade_reg_info)            
            ...
            if orig_add_info_spec_found:
                ...
                upgrade_obj.set_lookup_dict(trade_add_info_dict)
                for add_info in list(trade_add_info_dict.keys()):
                    trade_reg_info = upgrade_obj.upgrade_reg_info(add_info, trade, trade_reg_info, True)
                upgrade_obj.ShortSellIndicator(trade)
            ...
            reg_infos_to_commit.append(trade_reg_info)
            if len(reg_infos_to_commit) >= max_length:
                reg_infos_sub_lists = [reg_infos_to_commit[ins:ins + max_length] for ins in
                                       range(0, len(reg_infos_to_commit), max_length)]
                if reg_infos_sub_lists:
                    commit_reg_info(reg_infos_sub_lists)
                reg_infos_to_commit = []
        except Exception as ex:
            FRegulatoryLogger.ERROR(logger, "Trade <%d> had a problem. Error: %s"%(currentTrade, str(ex)))

    commit_reg_info([reg_infos_to_commit]) #Committing remaining list of instruments with length lesser than max_length
...
def upgrade_ins_reg_info():
    ...
    query = """select insid from Instrument"""
    col, instruments = ael.asql(query)
    total_ins_counter = len(instruments[0])
    ins_counter = 0
    add_info_dict = prune_add_info_specs(add_info_dict)
    for ins in instruments[0]:
        instrument = acm.FInstrument[ins[0]]
        ...
        for add_info in list(add_info_dict.keys()):
            reg_info = upgrade_obj.upgrade_reg_info(add_info, instrument, reg_info)
        upgrade_obj.delete_parent_add_info()
        global instrument_add_info_dict
        instrument_add_info_dict = get_rename_reg_info_dict('instrument', instrument_add_info_dict)
        instrument_add_info_dict = prune_add_info_specs(instrument_add_info_dict)
        ...
        if orig_add_info_spec_found:
            ...
            upgrade_obj.set_lookup_dict(instrument_add_info_dict)
            for add_info in list(instrument_add_info_dict.keys()):
                reg_info = upgrade_obj.upgrade_reg_info(add_info, instrument, reg_info, True)
            upgrade_obj.delete_parent_add_info(True)
        reg_infos_to_commit.append(reg_info)
        if len(reg_infos_to_commit) >= max_length:
            reg_infos_sub_lists = [reg_infos_to_commit[ins:ins + max_length] for ins in
                                   range(0, len(reg_infos_to_commit), max_length)]
            if reg_infos_sub_lists:
...
def upgrade_ins_reg(instrument):
    ins = acm.FInstrument[instrument]
    reg_info = ins.RegulatoryInfo()
    add_info_dict = get_reg_info_dict('instrument')
    upgrade_obj = FInstrumentRegulatoryInfoUpgrade()
    upgrade_obj.is_on_parent()
    upgrade_obj.set_lookup_dict(add_info_dict)
    ...
    if orig_add_info_spec_found:
        ...
        for add_info in list(instrument_add_info_dict.keys()):
            reg_info = upgrade_obj.upgrade_reg_info(add_info, instrument, reg_info, True)
        upgrade_obj.delete_parent_add_info(True)
    commit_reg_info([[reg_info]])
    add_info_del_dict = upgrade_obj.get_renamed_add_infos_list()
    add_info_del_dict = dict(add_info_del_dict, **add_info_dict)
    upgrade_obj.set_lookup_dict(add_info_del_dict)


def upgrade_trade_reg(trade):
    add_info_dict = get_reg_info_dict('trade')
    trade_info = acm.FTrade[trade]
    reg_info = trade_info.RegulatoryInfo()
    upgrade_obj = FTradeRegulatoryInfoUpgrade()
    upgrade_obj.is_on_parent()
    upgrade_obj.set_lookup_dict(add_info_dict)
    for add_info in list(add_info_dict.keys()):
        reg_info = upgrade_obj.upgrade_reg_info(add_info, trade, reg_info)
    upgrade_obj.ShortSellIndicator(trade)
    upgrade_obj.delete_parent_add_info()
    global trade_add_info_dict
    trade_add_info_dict = get_rename_reg_info_dict('trade', trade_add_info_dict)
    ...
    if orig_add_info_spec_found:
        ...
        upgrade_obj.set_lookup_dict(trade_add_info_dict)
        for add_info in list(trade_add_info_dict.keys()):
            reg_info = upgrade_obj.upgrade_reg_info(add_info, trade, reg_info, True)
        upgrade_obj.ShortSellIndicator(trade)
        upgrade_obj.delete_parent_add_info(True)
    commit_reg_info([[reg_info]])
    add_info_del_dict = upgrade_obj.get_renamed_add_infos_list()
...
def upgrade_contact_reg_info():
    ...
    try:
        upgrade_obj = FContactRegulatoryInfoUpgrade()
        query = """select seqnbr from Contact"""
        col, contacts = ael.asql(query)
        for cntct in contacts[0]:
            contact = acm.FContact[cntct[0]]
            upgrade_obj.link_contact_to_person(contact)
            upgrade_obj.upgrade_unique_name(contact)
            b_mismatch = upgrade_obj.verify_linked_contact_to_person(contact)
            if not b_mismatch:
...
def upgrade_regulatory_info():
    """api to be called to upgrade all Regulatory attributes across all classes"""
    upgrade_contact_reg_info()
    upgrade_ins_reg_info()
    upgrade_trade_reg_info()
    clean_up_obsolete_data()
    FRegulatoryLogger.INFO(logger, "Upgrade Complete!")

####################### Modular API's from here ##################
def prune_add_info_specs(add_info_dict):
    """get list of all applicable AddInfoSpecs"""
    obj = FInstrumentRegulatoryInfoUpgrade()
    pruned_add_info_dict = {}
    for add_info_name in list(add_info_dict.keys()):
        add_info_api = obj.get_add_info_name(add_info_name)
        if acm.FAdditionalInfoSpec[add_info_name]:
            pruned_add_info_dict[add_info_name] = add_info_dict[add_info_name]
    return pruned_add_info_dict

def copy_all_reginfos_to_reg_ins(ins_list):
    """Copy all RegInfos on Instrument to its relevant RegulatoryInfo attribute"""
    reg_infos_to_commit = []
    add_info_dict = get_reg_info_dict('instrument')
    add_info_dict = prune_add_info_specs(add_info_dict)
    upgrade_obj = None
    ...
    for instrument in ins_list:
        ...
        try:
            instr = acm.FInstrument[instrument]
            if not instr:
                continue
            ins = instr.StorageImage()
            reg_info = ins.RegulatoryInfo()
            upgrade_obj = FInstrumentRegulatoryInfoUpgrade()
            upgrade_obj.is_on_parent()
            upgrade_obj.set_lookup_dict(add_info_dict)
            for add_info in add_info_dict.keys():
                reg_info = upgrade_obj.upgrade_reg_info(add_info, ins, reg_info)
            instrument_add_info_dict = get_rename_reg_info_dict('instrument', instrument_add_info_dict)
            instrument_add_info_dict = prune_add_info_specs(instrument_add_info_dict)
            upgrade_obj.set_lookup_dict(instrument_add_info_dict)
            orig_add_info_spec_found = upgrade_obj.rename_parent_add_info()
            if orig_add_info_spec_found:
                ...
                upgrade_obj.set_lookup_dict(instrument_add_info_dict)
                for add_info in list(instrument_add_info_dict.keys()):
                    reg_info = upgrade_obj.upgrade_reg_info(add_info, ins, reg_info, True)
            reg_infos_to_commit.append(reg_info)
            if len(reg_infos_to_commit) >= max_length:
                reg_infos_sub_lists = [reg_infos_to_commit[ins:ins + max_length] for ins in range(0, len(reg_infos_to_commit), max_length)]
                if reg_infos_sub_lists:
                    commit_reg_info(reg_infos_sub_lists)
                reg_infos_to_commit = []
        except Exception as ex:
            FRegulatoryLogger.ERROR(logger, "Instrument <%s> had a problem. Error: <%s>"%(currentIns, str(ex)))
    if reg_infos_to_commit:
        commit_reg_info([reg_infos_to_commit])#Committing remaining list of instruments with length lesser than max_length
    return upgrade_obj


def verify_and_delete_all_addinfos_on_reg_ins( ins_list, upgrade_obj=None):
    """verify and delete all AddInfos on the Instrument that have been copied onto its corresponding Regulatory Attribute"""
    total_ins_counter = len(ins_list)
    add_info_dict = get_reg_info_dict('instrument')
    global instrument_add_info_dict
    ins_counter = 0
    for ins in ins_list:
        ...
        if not upgrade_obj:
            upgrade_obj = FInstrumentRegulatoryInfoUpgrade()
        upgrade_obj.acm_object = instrument
        upgrade_obj.set_lookup_dict(add_info_dict)
        for add_info in add_info_dict.keys():
            add_info_api = upgrade_obj.get_add_info_name(add_info)
            reg_info_api = upgrade_obj.get_reg_info_api_name(add_info)
            add_info_val = None
            b_mismatch = upgrade_obj.verify_parent_add_info_on_child(add_info, add_info_api, reg_info_api)
            try:
                add_info_val = eval('upgrade_obj.acm_object.AdditionalInfo().' + add_info_api + '()')               
            except:
                pass
            if (not b_mismatch) and str(add_info_val) != 'None':#it means, the AddInfo on the parent can be deleted
                upgrade_obj.delete_add_info[add_info] = add_info_val
        upgrade_obj.delete_parent_add_info()
        instrument_add_info_dict = get_rename_reg_info_dict('instrument', instrument_add_info_dict)
        upgrade_obj.set_lookup_dict(instrument_add_info_dict)
        orig_add_info_spec_found = upgrade_obj.rename_parent_add_info()
        if orig_add_info_spec_found:
            ...
            upgrade_obj.set_lookup_dict(instrument_add_info_dict)
            for add_info in list(instrument_add_info_dict.keys()):
                add_info_api = upgrade_obj.get_renamed_add_info(add_info)
                reg_info_api = upgrade_obj.get_reg_info_api_name(add_info)
                b_mismatch = upgrade_obj.verify_parent_add_info_on_child(add_info, add_info_api, reg_info_api)
                try:
                    add_info_val = eval('upgrade_obj.acm_object.AdditionalInfo().' + add_info_api + '()')
                    ...
                    pass
                if (not b_mismatch) and str(add_info_val) != 'None':#it means, the AddInfo on the parent can be deleted
                    upgrade_obj.delete_add_info[add_info_api] = add_info_val
            upgrade_obj.delete_parent_add_info(True)
        FRegulatoryLogger.INFO(logger, "Processing completed for instrument <%s>"%instrument.Name())
        if ins_counter % 1000 == 0:
            FRegulatoryLogger.INFO(logger, "Verification and Deletion of AddInfos completed for: <%d> instruments. Processing in progress for: <%d> instruments"%(ins_counter, total_ins_counter - ins_counter))

def copy_all_reg_infos_to_trade(trades_list):
    """upgrade all the Trade Regulatory attributes in AddInfos into their corresponding ADM columns"""
    reg_infos_to_commit = []
    add_info_dict = get_reg_info_dict('trade')
    add_info_dict = prune_add_info_specs(add_info_dict)
    upgrade_obj = None
    ...
    for trades in trades_list:
        try:
            currTrade = trades
            FRegulatoryLogger.DEBUG(logger, "Trade <%d> is being processed."%(currTrade))
            trd = acm.FTrade[trades]
            if not trd:
                continue
            trade = trd.StorageImage()
            trade_reg_info = trade.RegulatoryInfo()
            upgrade_obj = FTradeRegulatoryInfoUpgrade()
            upgrade_obj.is_on_parent()
            upgrade_obj.set_lookup_dict(add_info_dict)
            ...
            upgrade_obj.ShortSellIndicator(trade)
            trade_add_info_dict = get_rename_reg_info_dict('trade', trade_add_info_dict)
            trade_add_info_dict = prune_add_info_specs(trade_add_info_dict)
            upgrade_obj.set_lookup_dict(trade_add_info_dict)
            orig_add_info_spec_found = upgrade_obj.rename_parent_add_info()
            if orig_add_info_spec_found:
                return
            ...
            FRegulatoryLogger.ERROR(logger, "Trade <%d> had a problem. Error: <%s>"%(currTrade, str(ex)))
    if reg_infos_to_commit:
        commit_reg_info([reg_infos_to_commit]) #Committing remaining list of trades with length lesser than max_length
    return upgrade_obj

def commit_reg_info(reg_infos_sub_lists):
    """commit all the Regulatory attributes"""
    try:
        acm.BeginTransaction()
        for reg_infos in reg_infos_sub_lists:#reg_infos_to_commit is a list of lists.
            for reg_info in reg_infos:
                if reg_info.__class__.__name__ == "FInstrumentRegulatoryInfo":
                    FRegulatoryLogger.INFO(logger,"Committing RegulatoryInfo on Instrument <%s>."%(reg_info.Instrument().Name()))
                    reg_info.Instrument().Commit()
                else:
                    FRegulatoryLogger.INFO(logger,"Committing RegulatoryInfo on Trade <%s>."%(reg_info.Trade().Oid()))
                    reg_info.Trade().Commit()
                if reg_info.__class__.__name__ == "FInstrumentRegulatoryInfo":
                    FRegulatoryLogger.INFO(logger,"RegulatoryInfo on Instrument <%s> is committed successfully!." %(reg_info.Instrument().Name()))
                else:
                    FRegulatoryLogger.INFO(logger,"RegulatoryInfo on Trade <%s> is committed successfully!." %(reg_info.Trade().Oid()))
        acm.CommitTransaction()
    except Exception as e:
        for reg_infos in reg_infos_sub_lists:
            for reg_info in reg_infos:
                if reg_info.__class__.__name__ == "FInstrumentRegulatoryInfo":
                    FRegulatoryLogger.ERROR(logger,"Instrument <%s> cannot be committed. Exception :<%s>" % (
                            reg_info.Instrument().Name(), str(e)))
                else:
                    FRegulatoryLogger.ERROR(logger,"Trade <%s> is cannot be committed. Exception :<%s>" %(reg_info.Trade().Oid()), str(e))
            acm.AbortTransaction()
            FRegulatoryLogger.ERROR(logger, "Transaction Aborted!!")


def verify_and_delete_all_addinfos_on_reg_trade(trade_list, upgrade_obj = None):
    """verify and delete all AddInfos on the Trade that have been copied onto its corresponding Regulatory Attribute"""
    trd_counter = 0
    total_trd_counter = len(trade_list)
    add_info_dict = get_reg_info_dict('trade')
    global trade_add_info_dict
    for trades in trade_list:
        ...
        if orig_add_info_spec_found:
            ...
            upgrade_obj.delete_parent_add_info(True)
        if trd_counter % 1000 == 0:
            FRegulatoryLogger.WARN(logger, "Verification and Deletion of Add info complete for: <%d> trades. Processing in progress for: <%d> trades"%(trd_counter, total_trd_counter - trd_counter))

def update_set_of_instruments(ins_list):
    """Migrate the regulatory fields as AddInfos on the Instrument to its corresponding Regulatory fields"""
    FRegulatoryLogger.DEBUG(logger, "Updating RegulatoryInfo on Instruments.")
    FRegulatoryLogger.DEBUG(logger, "Calling copy_all_reginfos_to_reg_ins")
    ins_upgrd_obj = copy_all_reginfos_to_reg_ins(ins_list)
    FRegulatoryLogger.DEBUG(logger, "Calling verify_and_delete_all_addinfos_on_reg_ins")
    verify_and_delete_all_addinfos_on_reg_ins(ins_list, ins_upgrd_obj)

def update_set_of_trades(trd_list):
    """Migrate the regulatory fields as AddInfos on the Trade to its corresponding Regulatory fields"""
    FRegulatoryLogger.DEBUG(logger, "Updating RegulatoryInfo on Trades.")
    FRegulatoryLogger.DEBUG(logger, "Calling copy_all_reg_infos_to_trade")
    trd_upgrd_obj = copy_all_reg_infos_to_trade(trd_list)
    FRegulatoryLogger.DEBUG(logger, "Calling verify_and_delete_all_addinfos_on_reg_trade")
    verify_and_delete_all_addinfos_on_reg_trade( trd_list, trd_upgrd_obj)
    
def get_add_info_specs(addinfos, add_info_list):
    """returns the AddInfoSpecs list"""
    for addinfo in addinfos[0]:
        addinf = acm.FAdditionalInfoSpec[addinfo[0]]
        if addinf:
            add_info_list.append(addinf)
    return add_info_list

def get_add_info_specs_for_class(class_name):
    """get all relevant AddInfosSpec names for the given class"""
    queries = []
    if class_name == 'Instrument':
        queries = ["""select field_name from AdditionalInfoSpec where rec_type = 'Instrument' and field_name like 'Treg%' """, \
                """select field_name from AdditionalInfoSpec where rec_type = 'Instrument' and field_name like 'reg%' """]
    elif class_name == 'Trade':
        queries = ["""select field_name from AdditionalInfoSpec where rec_type = 'Trade' and field_name like 'Treg%' """, \
                """select field_name from AdditionalInfoSpec where rec_type = 'Trade' and field_name like 'reg%' """]
    add_info_list = []
    for query in queries:
        col, addinfos = ael.asql(query)
        add_info_list = get_add_info_specs(addinfos, add_info_list)
    return add_info_list

def delete_add_info_specs(class_name):
    """delete all the AddInfoSpec that have been migrated for the given class"""
    add_info_specs = get_add_info_specs_for_class(class_name)
    for add_info_spec in list(add_info_specs):
        if add_info_spec:
            FRegulatoryLogger.DEBUG(logger,
                        "Deleting AddInfoSpec <%s> on %s" % (add_info_spec.Name(), add_info_spec.RecType()))
            try:
                add_info_spec.Delete()
                FRegulatoryLogger.DEBUG(logger,
                        "Deleted AddInfoSpec <%s> on %s" % (add_info_spec.Name(), add_info_spec.RecType()))
            except Exception as e:
                FRegulatoryLogger.ERROR(logger, "Cannot delete AddInfoSpec <%s> on %s.Error : <%s>." % (
                add_info_spec.Name(), add_info_spec.RecType(), str(e)))
