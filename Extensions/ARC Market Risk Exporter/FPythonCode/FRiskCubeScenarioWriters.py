...
class scenarioValData(object):
    def __init__(self, dimValues, val):
        self.dimValues = dimValues
        self.val = val 

    def getVal(self):
        val = []
        shiftVal = self.dimValues.split(',')
        val.extend(shiftVal)
        val.append(self.val)
        return val
...
class ArtiQSpotVolWriter(SpotVolWriter):
    ...
    SHIFT_COLUMNNAME = {PRICE_BUMP: "Stress Scenario.Spot Shift",
        VOL_BUMP:"Stress Scenario.Vol Shift"}

    def _getCalculatedColumnHeaders(self):
        header = ["Stress Scenario.Stress Name "]
        for col in self.columnInfoTable.values():
            dimNames = col.scenarioInformation.dimensionNames
            shiftColName = [self.SHIFT_COLUMNNAME.get(name, 
                "Stress Scenario.{0}".format(name)) for name in dimNames]
            header.extend(shiftColName + [col.name])
        return header
    ...
    def _getCalculatedValues(self, posID):
        ...
        for colID, colInfo in self.columnInfoTable.items():
            ...
            # For the column descriptor in the data header, see above.
            calcVals = []
            colNamesVal = colInfo.columnPartNames
            for colval, valObj in zip(colNamesVal, scenarioVals):
                val = BaseWriter._getStringValue(
                    self._getAttributeValues(posID), colID, valObj.values, self._logger)
                calcVals.append(scenarioValData(colval, val))
        return calcVals
    ...
    def _writeDataRows(self, fp):
        ...
        for posID in sorted(positions):
            ...
            for c in calcs:
                line = self.separators[0].join([reportname] + rowID + attrs + bs + [self._stressName] + c.getVal())
                ...
                for posID in sorted(positions):
                    ...
                    if fType in ['InterestRate', 'Benchmark', 'Credit']:
                        for factorName in self.curveFactorNames:
                            deltaColName = fType + " Par Delta" if fType == 'Credit' else fType + ' Delta'
                        ...
                        if instrumentSpreadCurves:
                            deltaColName = "Instrument Spread Delta"
                            gammaColName = "Instrument Spread Gamma"
                            ...
                            dateStr, period = FMarketRiskExportUtil.ParseDateStrToPeriod(
                                                expiryDate, self._logger)
