...
class MarketDataWriter(object):
    ...
    def _getInstrumentSpreadPrice(self, yc):
        ...
        for inst in insts:
            ...
            if not math.isnan(price.Number()):
                ...
                if period.find('-') != -1:
                    continue
                dataStr = "{0}::::{1:.4f}".format(period, price.Number())
                outputData.append((inst.Name(), dataStr))
        return outputData
    ...
    def _getBenchmarkPrice(self, yc):
        ...
        for p in sortedPoints:
            ...
            pointStr = period + "::::" + '{0:.4f}'.format(mtmPrice)
    ...
    def _writeDataRows(self, fp):
        for yc in self.IRRateCurves:
            ...
            if ycType == "InstrumentSpread":
                results=self._getInstrumentSpreadPrice(yc)
            elif self.exportBenchmarkPrice:
                results = self._getBenchmarkPrice(yc)
            else:
                results = self._getYieldCurvePoints(yc)

            fTypeExportName = self.riskfactorExportMaps.get(ycType, ycType)
            if isinstance(results, str):
                line = ",".join(["Sensitivity Based", fTypeExportName, yc.Name(), "Whole", results])
                line = FRiskCubeWriterBase.BaseWriter._encodeData(line)
                print(line, file=fp)
            elif isinstance(results, list):
                for name, dataStr in results:
                    line = ",".join(["Sensitivity Based", fTypeExportName, '{0}-{1}'.format(yc.Name(), name), "Whole", dataStr])
                    line = FRiskCubeWriterBase.BaseWriter._encodeData(line)
                    print(line, file=fp)
