...
#__src_file__ = "extensions/BDExport/./etc/FExportProcess.py"
"""-------------------------------------------------------------------------------------------------------
MODULE
    FExportProcess
...
class FExportProcess():
    ...
    def __init__(self, integrationInstance, ACMTradeQueryIdList, additionalQueryDictionary=dict(),
        ...
        '''if additionalParameters != None:  Commented
            try:
        ...
        self._sheetTemplate = additionalParameters.tradingSheetTemplateName
        self._isheetTemplate = additionalParameters.Instrument_iSheetTemplateName
        self._ienableexport = additionalParameters.InstrumentExport
        
    def GenerateEmptyFile(self):
        return self._alwaysGenerateFile == 'true'

    def SingleExportsAsList(self):
        """Returns a list of all export flows formed during the load process."""
        list_export = self._singleExport.values()
        ins_file_name = self._additionalParameters.Instrument_iFileName
        i=0
        for key in self._singleExport.keys():
            if ins_file_name == self._singleExport[key]._filename:
    ...
    def LoadBusinessProcesses(self):
        ...
        if len(self._ExportableBusinessProcesses(self._integration.ChartId()))>0:
            ...
            sheetTemplateId = self._SheetTemplateId(self._sheetTemplate)
    ...
    def AddBusinessProcess(self, businessProcess, ACMQueryId, trade = None):
        ...
        """
        try:
            if self._additionalQueryDictionary['Instruments'] == ACMQueryId and self._ienableexport == 'true':
                sheetTemplateId = self._isheetTemplate
            else:
                sheetTemplateId = self._SheetTemplateId(self._sheetTemplate)
            trade = businessProcess.Subject() if not trade else trade
            if type(trade) == type(acm.FTrade()) or self._ienableexport == 'true':
    ...
    def _FindMatchingLinkedBP(self, trade, linkedfunction, stateChartId):
        for linkedBP in self._ExportableBusinessProcessesIns(stateChartId):
            if linkedBP.Subject() == linkedfunction(trade):
                return linkedBP
        return None
		
    def _ExportableBusinessProcessesIns(self, stateChartId):
        """
...
class FExportInsProcess(FExportProcess):
    ...
    def LoadBusinessProcesses(self):
        ...
        logger.debug('Now loading all the relevant trade business processes...')
        totalBPsLoaded = 0
        if len(self._ExportableBusinessProcesses(self._integration.InstrumentStateChart))>0:
            for insBP in self._ExportableBusinessProcesses(self._integration.InstrumentStateChart):
                ins = insBP.Subject()
                if not ins:
                    logger.debug('Business Process %d no longer refers to a valid trade or object -- ignoring it.', insBP.Oid())
                    continue
                queryId = FExportUtils.FindMatchingQueryId(ins, [self._additionalQueryDictionary['Instruments']])
                if queryId:
                    totalBPsLoaded += self.AddBusinessProcess(insBP, queryId)
                    #if self.Integration().LinkedExportObjects():
                    #    totalBPsLoaded += self.LoadLinkedObjects(trade)
...
class FExportTradeProcess(FExportProcess):
...
class FSingleExport():
    ...
    def ExportableBusinessProcesses(self):
        return [bp for bp in self.BusinessProcesses() if not bp.CurrentStep().IsInErrorState() and FBusinessProcessUtils.IsValidEvent(bp, 'Export Executed') and IsInstrumentExport(bp)]
...
def IsInstrumentExport(bp):
    try:
        if bp.Subject_type() == "Instrument":
            ins = bp.Subject()
            if not "InsStatus" in dir(ins.AdditionalInfo()):
                return True
            else:
                ins_status = ins.AdditionalInfo().InsStatus()
                for st in FIntegration.InstrumentTransitions():
                    if FBusinessProcessUtils.IsValidEvent(bp, st.EventId()) and ins_status == st.ToStatus() and st.EventId() == 'Export Executed':
                        return True
                return False
        else:
            return True
