...
#__src_file__ = "extensions/BDExport/./etc/FFileTransporter.py"
"""-------------------------------------------------------------------------------------------------------
MODULE
    ...
    def _SetBusinessProcessesExported(self, businessProcesses):
        ...
        for bp in businessProcesses:
            try:
                ...
                # Revalidate that the business process is still in the required state for an export
                elif bp.Subject_type() == "Instrument" and  ("InsStatus" in dir(bp.Subject().AdditionalInfo())):
                    ins = bp.Subject()
                    ...
                    for st in FIntegration.InstrumentTransitions():
                        if FBusinessProcessUtils.IsValidEvent(bp, st.EventId()) and ins_status == st.ToStatus():
                            ...
                            bp.Commit()		
                    if bp.CurrentStep().State().Name() == 'Sent':
                        if ins.AdditionalInfo().InsStatus() != 'Default':
