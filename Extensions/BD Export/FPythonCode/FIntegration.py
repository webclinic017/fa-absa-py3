...
#__src_file__ = "extensions/BDExport/./etc/FIntegration.py"
...
class FIntegration:
    ...
    def _ipostTransformationFunction(self, filepath):
        return
    ...
    def LinkedExportObjects(self, listOfLinkData = None):
        ...
        for func, chartId, _linkedExportObjectId in listOfLinkData:
            if not (acm.FStateChart[chartId]):
                return
    ...
    def iPostTransformationFunction(self, function = None):
        """
        Supply the function that transforms the file after creation
        """

        if function is None:
            return self._ipostTransformationFunction

        self._ValidateCallbackFunction(function, 1)
        self._ipostTransformationFunction = function
...
class FInstrumentStatusTransition(FTransition):
    """
    Class for modeling a trade status change event
    E.g. a trade that goes from Simulated into FO Confirmed
    But also from any state into some specific state.
...
def InstrumentTransitions():
    return (
        FInstrumentStatusTransition(
            eventId=FTransition.CREATE_EVENT_ID,
            toStatus='Default'),
        FInstrumentStatusTransition(
            eventId='Inst. Amended',
            toStatus='Amend'),
        FInstrumentStatusTransition(
            eventId='Export Executed',
            toStatus='Default'),
        FInstrumentStatusTransition(
            eventId='Void Instrument',
            toStatus='Void'),
    )