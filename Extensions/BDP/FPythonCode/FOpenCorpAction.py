...
#__src_file__ = "extensions/corp_actions/etc/FOpenCorpAction.py"
#----------------------------------------------------------------------------
...
def voluntary(valueString, typ=float, default=0):
    return typ(valueString.strip() or default)
...
def run_main(d, ca):
    ...
    if d['CashCurrency']:
        ca.CashCurrency = d['CashCurrency'][0]
    else:
        ca.CashCurrency = None
    ca.Instrument = d['Instrument'][0]
    if d['Market']:
        ca.Market = d['Market']

    if d['NewInstrument']:
        ca.NewInstrument = d['NewInstrument'][0]
    else:
        ca.NewInstrument = None
    if len(d['Text']) > 38:
        print("Description text too long, only the first 38 characters will "
                "be saved!")
        d['Text'] = d['Text'][:38]
    ca.Text = d['Text']
    ca.SpinoffCostFraction = voluntary(d['SpinoffCostFraction'])
    ca.OldQuantity = voluntary(d['OldQuantity'])
    ca.NewQuantity = voluntary(d['NewQuantity'])
    ca.CashAmount = voluntary(d['CashAmount'])
    ca.ExternalId = d['ExternalId']
    ca.IssuePerShare = voluntary(d.get('IssuePerShare', '0'))
    ca.RoundingType = ael.enum_from_string('RoundingRule',
                                                d['RoundingType'])
    ca.CaChoiceType = ael.enum_from_string('CorpActionChoiceType',
                                                d['CaChoiceType'])
    ca.AdjustQuantity = 1
    ca.AdjustTradePrice = 1
    linkedCorpAction = d['LinkedCorpActions']
    __setBusinessEvent(ca, linkedCorpAction)
