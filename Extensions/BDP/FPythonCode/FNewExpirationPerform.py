...
def perform(log, execParam):
    ...
    for instOrInstName in instrumentsList:
        ...
        try:
            ...
            if action in [ACTION_ARCHIVE,
                ACTION_DELETE_POSITIONS,
                ACTION_DELETE] \
                and execParam.get('preservePL') \
                and not FBDPInstrument.isExpired(acm.FInstrument[name]):
                msg = (
                    'Ignore the instrument %s, as we need to preservePL, '
                    'but the instrument has not expired yet.' % (
                     name))
                log.logWarning(msg)
                actionMsg = 'ARCHIVE' if \
                action in [ACTION_ARCHIVE] \
                else 'DELETE'
            ...
            if action == ACTION_ARCHIVE:
                _perform(log, execParam, 'archive', 'trade', inst)
                ...
                if not utility.isInstArchived(inst):
                    ...
                    _perform(log, execParam, 'archive delete', 'trade', inst)
...
def _perform(log, execParam, action, object_type, inst):
...
class _DearchiveTrades(_NewExpirationPerformBase):
    ...
    def perform(self, execParam):
        _NewExpirationPerformBase._perform(self, execParam)
        ...
        def process(opList):
            for opDetails in opList:
                if opDetails.opTyp == ACTION_DEARCHIVE:
                    utility.DeArchiveTrd(opDetails.oid,
                            self._getWorldRef())
...
class _ArchiveTrades(_NewExpirationPerformBase):
    ...
    def _doInspect(self, InspectList):
        ...
        for cpArcTrds in InspectList:
            ...
            try:
                for trd in arcTrds:
                    archiveOp = utility.GetOpsToArchiveTrd(trd,
                        self._getWorldRef())
...
class _ArchiveInstrument(_NewExpirationPerformBase):
    ...
    def _doInspect(self, InspectList):
        ...
        if len(InspectList) > 1:
            ...
            oid = InspectList[0]
            archiveInstOps = utility.GetOpsToArchiveInstrument(oid,
                        self._getWorldRef())
            ...
            for opDetails in inspectionReport:
                for deArcInst in opDetails:
                    if deArcInst.opTyp == ACTION_DEARCHIVE:
                        utility.DeArchiveInst(deArcInst.oid,
                            self._getWorldRef())
