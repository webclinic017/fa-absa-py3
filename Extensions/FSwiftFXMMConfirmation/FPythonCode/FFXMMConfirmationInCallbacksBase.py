...
class FFXMMConfirmationInCallbacksBase(object):
    def __init__(self):
        pass
    ...
    def condition_entry_state_amended(self, context):
        return FSwiftMLUtils.HasUserRights(FSwiftMLUtils.get_operation_name_from_bpr_context(context))
# ------------------------------------------------------------------------------
    ...
    def condition_exit_state_amended(self, context):
        return True
# ------------------------------------------------------------------------------
    ...
    def on_entry_state_unpaired(self, context):
        try:
            ...
            if conf_obj:
                ...
                # Do not remove the add info on conf object so it wont be eligible for pairing
                # After manually unpairing, it generated confirmation update and while processing this update conf will again be paired with
                # recetly unpaired BPR. To avoid this we just remove the reference of acm object but do not clear the additional info.
                #FSwiftMLUtils.unlink_acm_object(bpr)
                FSwiftMLUtils.FSwiftExternalObject.unlink_acm_object(bpr, 'Confirmation')
                # Hack to update the operation manager screen by updating add info on conf object
                # mt_type = bpr.AdditionalInfo().SwiftMessageType()
                # add_info = FSwiftMLUtils.get_add_info_name_for_mt_type(mt_type)
                # utils_obj = FIntegrationUtils.FIntegrationUtils()
                # utils_obj.set_additional_info(add_info, conf_obj, 'Paired')
                # utils_obj.set_additional_info(add_info, conf_obj, 'Unpaired')
        except Exception as error:
    ...
    def on_entry_state_amended(self, context):
        try:
            bpr = context.CurrentStep().BusinessProcess()
            conf_obj = FSwiftMLUtils.get_acm_object_from_bpr(bpr)
            ...
            if conf_obj:
                ...
                FSwiftMLUtils.unlink_references(context.CurrentStep().BusinessProcess())
        except Exception as error:
            notifier.ERROR("Error occurred in on_entry_state_amended: %s"%str(error))
            notifier.DEBUG(str(error), exc_info=1)
# ------------------------------------------------------------------------------
    ...
    def on_exit_state_amended(self, context):
        pass
