...
class FFXMMConfirmationOutCallbacksBase(object):
    def __init__(self):
        pass
...
# ------------------------------------------------------------------------------
    ...
    def on_entry_state_swiftmsggenerated(self, context):
        """ On entry state swiftmsggenerated"""
        msg_type = ''
        try:
            ...
            while counter <= max_retries:
                try:
                    FSwiftMLUtils.set_document_status_on_acm_object(acm_obj, 'GENERATED')
                    """if FSwiftMLUtils.get_acm_version() >= 2016.4:
                        if acm_obj.Status() <> ConfirmationStatus.RELEASED:
                            acm_obj_clone = acm_obj.Clone()
                            acm_obj_clone.Status(ConfirmationStatus.PENDING_APPROVAL)
                            if acm_obj_clone.IsSignOffed():
                                acm_obj_clone.Status(ConfirmationStatus.AUTHORISED)
                            acm_obj.Apply(acm_obj_clone)
                            acm_obj.Commit()
                    else:
                        if acm_obj.Status() <> 'Released':
                            acm_obj_clone = acm_obj.Clone()
                            acm_obj_clone.Status('Pending Approval')
                            if acm_obj_clone.IsSignOffed():
                                acm_obj_clone.Status('Authorised')
                            acm_obj.Apply(acm_obj_clone)
                            acm_obj.Commit()"""
                except Exception as e:
                    # raise Exception("Exception in on_entry_state_networkrulesvalidated : %s"%str(e))
                    if 'Update collision' in str(e):
                        counter = counter + 1
                        notifier.ERROR("{0} Retrying after 8 seconds due to update collision".format(msg_type))
                        time.sleep(8)
                        continue
                    notifier.ERROR('%s Exception in on_entry_state_swiftmsggenerated : %s' % (msg_type, str(e)))
                break
        except Exception as e:
            notifier.ERROR('%s Exception caught in on_entry_state_swiftmsggenerated : %s' % (msg_type, str(e)))
            ...
            while counter <= max_retries:
                ...
                if event == 'Nack':
                    try:
                        ...
                        """acm_obj_clone = acm_obj.Clone()
                        if FSwiftMLUtils.get_acm_version() >= 2016.4:
                            acm_obj_clone.Status(ConfirmationStatus.NOT_ACKNOWLEDGED)
                        else:
