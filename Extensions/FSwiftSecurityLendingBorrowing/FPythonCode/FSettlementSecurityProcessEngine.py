...
-----------------------------------------------------------------------------------------------------------------------------------------
01-05-2020   SBL          Jaysen Naicker          Gasant Thulsie                 Do not allow SBL settlements to cancel or correct
-----------------------------------------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------"""
...
from sbl_booking_utils import RETURN_TYPES
#-------------------------------------------------------------------------
...
class SecurityProcessEngine(object):
    ...
    def _PerformCancelOrCorrect(settlement):
        ...
        # SBL check. If partial return do not do Cancel or Correct on settlement
        if FSBLCalculator.is_sl_swift(settlement):
            if settlement.Trade().Oid() != settlement.Trade().ConnectedTrade().Oid():
                if settlement.Trade().ConnectedTrade().Text1() == RETURN_TYPES["PARTIAL"] or settlement.Trade().Text1() in (RETURN_TYPES["FULL"], RETURN_TYPES["PARTIAL"]):
                    # block cancellation of SBL settlements
