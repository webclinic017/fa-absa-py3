...
def GetExoticEventsOfKindForDate(derivative, eventType, dateToday, getHistorical):
    ...
    for event in allEventsOfType:
        if (event.Date() == dateToday or (getHistorical and event.Date() < dateToday)):
            if event.EventValue() == F_FUTURE_VALUE:
                events.append(event)
    return events
...
def ExoticEventCurrency(derivative, exoticEvent):
    ...
    if derivative.IsKindOf("FVolatilityFuture"):
        ...
        if derivative.QuantoOptionType() == 'Quanto':
            if not underlying.IsKindOf("FCurrency"):
...
def ExoticEventMtMPrice(derivative, exoticEvent): 
    ...
    try:
        ...
        msg = "Hook GetPriceForExoticFixings is not enabled because of: {}".format(str(e))
        ael.log(msg)
    ...
    if hook:
        try:
            price, handled = hook(exoticEvent, currency)
        except Exception as e:
            msg = 'Caught exception from GetPriceForExoticFixings hook: '.format(str(e))
            ael.log(msg)
        if handled:
            if price:
                if fixingSource:
                    msg = "MtM Price form fixing source {} for exotic event with Oid: {} has been retrieved".format(fixingSource.Name(), exoticEvent.Oid())
                else:
                    msg = "There is no fixing source for exotic event with Oid {}".format(exoticEvent.Oid())
                ael.log(msg)
            else:
                if fixingSource:
                    msg = "Failed to retrieve MtM Price form fixing source {} for exotic event with Oid: {}".format(fixingSource.Name(), exoticEvent.Oid())
                else:
                    msg = "There is no fixing source for exotic event with Oid {}".format(exoticEvent.Oid())
                ael.log(msg)
