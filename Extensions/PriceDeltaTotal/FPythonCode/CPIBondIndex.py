''' 
:Author: Andreas Bayer <Andreas.Bayer@absacapital.com; andreas.bayer.mannheim@googlemail.com>
:Version: 1.0 13/02/218
:Summary: Trade Detail Exporter for RoA client trades
'''

import acm, ael, csv, string, time
from at_time import acm_datetime, acm_date
...
calc_space = acm.Calculations().CreateCalculationSpace(acm.GetDefaultContext(), acm.FDealSheet)
...
def get_inflation_level_for_date(dt):
    sacpi = acm.FPriceIndex['SACPI']
    date_column = 'Forward Date Column'
    estimate_column = 'Forward Price Column'
    calc_space.Clear()
    calc = calc_space.CreateCalculation(sacpi, estimate_column)
    calc_space.SimulateValue(sacpi,
        date_column,
        dt)
...
def bond_index_calculations(ins, bond_weights, t, s, user_input='No', input_file='F:\\Albi Yields.csv'):
    ...
    bond_calcs = {'Bt':0.0, 'Ct':0.0, 'Cmit':{}, 'Cit':{}, 'Cpit':{}, 'Yit':{}, 'Pit':{}, 'gi':{}, 'Hit':{}, 'Dit':{}, 'Nit':{}, 'Nitp':{}, 'Xit':{}, 'Vit':{}, 'Rit':{}, 'base_cpi': {}, 'cpi_t': {}, 'cpi_s': {}, 't': {}, 's': {}, 'cpi_cit': {}}
    for b in bond_weights.keys():
        instr = acm.FInstrument[b]
        base_inflation = instr.Legs().At(0).InflationBaseValue()
        ...
        if user_input == 'Yes':
            ...
            instr = acm.FInstrument[b]
            bond_calcs['Yit'][b] = instr.Calculation().MarketPrice(calc_space_collection).Value().Number()
            ...
            bond_calcs['Pit'][b] = instr.Calculation().MarketPriceConverted(calc_space_collection, 'Pct of Nominal', 'RSA').Number()
        ...
        cpi_t = get_inflation_level_for_date(t)/base_inflation
        #print 'cpi_t', cpi_t, get_inflation_level_for_date(t), base_inflation
        cpi_s = get_inflation_level_for_date(s)/base_inflation
        #print cpi_s, todays_kfactor
        #print 'asdf', t, get_inflation_level_for_date(t)
        #print 'asdf2', s, get_inflation_level_for_date(s)
        bond_calcs['Dit'][b] = (1/(1 + bond_calcs['Yit'][b]/float(200)))**bond_calcs['Hit'][b] * cpi_t/cpi_s
        bond_calcs['Nit'][b] = todays_kfactor*bond_weights[b]
        
        bond_calcs['base_cpi'][b] = base_inflation
        bond_calcs['cpi_t'][b] = cpi_t
        bond_calcs['cpi_s'][b] = cpi_s
        bond_calcs['t'][b] = t
        bond_calcs['s'][b] = s
        ...
        cpi_c = get_inflation_level_for_date(bond_calcs['Cit'][b])/base_inflation
        cpi_s = get_inflation_level_for_date(s)/base_inflation
        bond_calcs['cpi_cit'][b] = cpi_c
        bond_calcs['Xit'][b] = bond_calcs['gi'][b]*bond_calcs['Nitp'][b]/200 * cpi_c * 100
        
        bond_calcs['Vit'][b] = bond_calcs['Xit'][b]*bond_calcs['Dit'][b]*(1/(1 + bond_calcs['Yit'][b]/float(200))) \
                **(max(float(s.days_between(bond_calcs['Cit'][b])), 0.0) / bond_calcs['Cmit'][b].days_between(bond_calcs['Cit'][b])) * cpi_s/cpi_c
        ...
        bond_calcs['Bt'] += 100*bond_calcs['Nit'][b]*bond_calcs['Pit'][b]*bond_calcs['Dit'][b]/100
        bond_calcs['Ct'] += bond_calcs['Vit'][b]
        
    return bond_calcs

def bond_index_calculations_theor(ins, bond_weights, t, s, theoretical_prices, converted_prices, user_input='No', input_file='F:\\Albi Yields.csv'):
    ''' Return a dictionary of dictionaries containing the values of all intermediate calculations as 
        specified by the BEASSA Total Return Indices 2000 documentation 
    '''
    ...
    for b in bond_weights.keys():
        ...
        if user_input == 'Yes':
            ...
            bond_calcs['Pit'][b] = converted_prices[b].Number()           
...
def index_value(bond_index, theoretical_prices, instruments, converted_prices):
    ...
    converted_values = {}
    for instrument, price in zip(instruments, converted_prices):
        converted_values[to_ael(instrument).insid] = price
    ...
    index_calcs = bond_index_calculations_theor(bond_index, weights, vd, sd, theoretical_values, converted_values)
...
def calculation_report(bond_index, user_input='No', input_file='F:\\Albi Yields.csv'):
    ...
    output = [['Bond', 'Weight', 'ExCouponDay', 'C_minus_it', 'C_it', 'C_plus_it', 'Y_it', 'H_it', 'D_it', 'N_it', 'N_itp', 'P_it', 'g_i', 'X_it', 'V_it', 'R_it', 'base_cpi', 'cpi_t', 'cpi_s', 't', 's', 'cpi_cit']]
    for b in weights.keys():
        row = [b, weights[b], ex_cpn_day(b, vd), index_calcs['Cmit'][b], index_calcs['Cit'][b], index_calcs['Cpit'][b], \
            ...
            index_calcs['Pit'][b], index_calcs['gi'][b], index_calcs['Xit'][b], index_calcs['Vit'][b], index_calcs['Rit'][b], index_calcs['base_cpi'][b], index_calcs['cpi_t'][b], index_calcs['cpi_s'][b], index_calcs['t'][b], index_calcs['s'][b], index_calcs['cpi_cit'][b]]
        output.append(row)
...
def test():
    tic = time.time()
    test_bond = ael.Instrument['ZAR/IGOV_Weights_TEST']
    ...
    calculation_report(test_bond, 'No')
