...
#__src_file__ = "extensions/cva/adaptiv_fva/./etc/FVAWorkflow.py"
import acm
from FVAUtils import FVAStateChartConstants, IsSwapDesk, IsFVADesk
import FVAHooksHelper as hooks
...
def condition_entry_state_ready(context):
    ...
    if subject.IsKindOf(acm.FTrade):
        return hooks.IsFVACandidate(subject) and IsSwapDesk()
...
def condition_entry_state_pending_fva(context):
    return IsSwapDesk()

def condition_exit_state_pending_fva(context):
    return IsFVADesk()
...
def on_entry_state_pending_fva(context):
    subject = context.Subject()
    subject.Changed()
    
def on_exit_state_pending_fva(context):
    subject = context.Subject()
    subject.Changed()
...
def on_exit_state_pending_confirmation(context):
    ...
    if event.Name() == EVENTS.FVA_RE_REQUEST:
