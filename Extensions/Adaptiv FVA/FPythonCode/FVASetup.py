...
#__src_file__ = "extensions/cva/adaptiv_fva/./etc/FVASetup.py"

import acm
from FVAUtils import FVAStateChartConstants

CHART_NAME = FVAStateChartConstants.CHART_NAME
STATES     = FVAStateChartConstants.STATES
EVENTS     = FVAStateChartConstants.EVENTS

def CreateFvaStateChart():
    ...
    if state_chart:
        ...
        state_chart.CreateState(STATES.PENDING_FVA)
        ...
        pending_incremental_fva_state = states[STATES.PENDING_FVA]
        pending_trade_confirmation_state = states[STATES.PENDING_CONFIRMATION]
        confirmed_state = states[STATES.END]

        trade_to_fva_desk_event = acm.FStateChartEvent(EVENTS.FVA_REQUESTED)
        create_payments_event = acm.FStateChartEvent(EVENTS.FVA_ASSIGNED)
        resend_trade_to_fva_desk_event = acm.FStateChartEvent(EVENTS.FVA_RE_REQUEST)
        ...
        ready_state.CreateTransition( trade_to_fva_desk_event, pending_incremental_fva_state )
        pending_incremental_fva_state.CreateTransition( create_payments_event, pending_trade_confirmation_state )
        pending_trade_confirmation_state.CreateTransition( resend_trade_to_fva_desk_event, pending_incremental_fva_state )
...
def Setup():
    ...
    # ADDITIONAL INFOS
    CreateAdditionalInfo("FVADocument", "Instrument", "RecordRef", "ChoiceList", "Standard Document", ["Credit Balance"] )
