...
#__src_file__ = "extensions/confirmation/etc/FConfirmationDocXMLSpecifier.py"
"""----------------------------------------------------------------------------
MODULE
    FConfirmationDocXMLSpecifier -
...
import ael
import FConfirmationFields
import FOperationsUtils as Utils


class ConfirmationDocumentXMLSpecifier(XmlSpecifierBase):
    ''' Class for describing how the XML for confirmation documents should be
        created. '''

    def __GetAelConfirmationFromMbf(self, mbfMessage):
        ''' Helpfunction for getting the ael confirmation out of the mbf-message. '''
        confirmation = None
        if mbfMessage:
    ...
    def __init__(self, mbfMessage):
        ''' Confstructor for this class. Takes a mbf-message (the amba message). '''

        # raise TypeException
        assert (str(type(mbfMessage)) == "<type 'mbf_object'>"), \
            'Constructor called without mbfMessage'

        # Call base class constructor
        XmlSpecifierBase.__init__(self, FConfirmationFields.field_dict)


        # Set the base ael entity
        confirmation = self.__GetAelConfirmationFromMbf(mbfMessage)
        if not confirmation:
            Utils.LogVerbose("ConfirmationDocumentXMLSpecifier could not get confirmation record")
