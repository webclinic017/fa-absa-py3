...
def first_weekday_of_month(month_date, day):
    ...
    first_day = acm.Time.FirstDayOfMonth(month_date)
    num_days = (week_days[day] - week_days[acm.Time.DayOfWeek(first_day)]) % 7
    
    return acm.Time.DateAddDelta(first_day, 0, 0, num_days)
    
def rebasing_day(input_date, method = 'Next'):
    ...
    ''' 
    first_day = acm.Time.FirstDayOfMonth(input_date)
    calendar = acm.FCurrency['ZAR'].Calendar()
    rebase_day = calendar.ModifyDate(None, None, first_weekday_of_month(first_day, 'Thursday'), 'Preceding')
    
    if input_date <= rebase_day and method == 'Next':
        return rebase_day
    elif input_date > rebase_day and method == 'Next':
        return calendar.ModifyDate(None, None, first_weekday_of_month(acm.Time.DateAddDelta(first_day, 0, 1, 0), 'Thursday'), 'Preceding')
...
def triple_to_date(t):
    ...
    '''
    return '-'.join([string.zfill(s, 2) for s in t])

def ncd(bnd, settlement_day):
    ''' Returns the next coupon date for a given bond and settlement date
    '''
    return min([cf.EndDate() for cf in acm.FInstrument[bnd].Legs()[0].CashFlows() if cf.CashFlowType() == 'Fixed Rate' and cf.EndDate() > settlement_day])
    
def ex_cpn_day(bnd, settlement_day):
    ''' Returns the ex coupon date for a given bond and settlement date
    '''
    myBnd = acm.FInstrument[bnd]
    next_coup = ncd(bnd, settlement_day)
    calendar = acm.FCurrency['ZAR'].Calendar()

    if myBnd.ExCouponMethod() == 'Calendar Days':
        return acm.Time.DateAdjustPeriod(next_coup, '-' + myBnd.ExCouponPeriod())
        
    elif myBnd.ExCouponMethod() == 'Business Days':
        return calendar.AdjustBankingDays(acm.Time.DateAdjustPeriod(next_coup, '-' + myBnd.ex_coup_period), 0)
        
    elif myBnd.ExCouponMethod() == 'AdditionalInfo':
        ...
        ex_day_list = [acm.Time.DateToYMD(myBnd.add_info('ExCoup1')), acm.Time.DateToYMD(myBnd.add_info('ExCoup2'))]
        ncd_trip = acm.Time.DateToYMD(next_coup)
...
def Cit_plus_inc(bnd, value_day, inc):
    ...
    '''
    cash_flows = acm.FInstrument[bnd].Legs()[0].CashFlows()
    date_list = [cf.EndDate() for cf in cash_flows if cf.CashFlowType() == 'Fixed Rate']
    date_list.sort()
...
def getKFactor(oid, t):
    ''' Return a KFactor for a specified instrument and date.  The KFactors are stored as a time series called KFactor which is linked to
        the instrument.
    '''
    time_series = acm.FTimeSeriesSpec['KFactor'].TimeSeries()
    for ts in time_series:
        if ts.Day() == t and ts.Recaddr() == oid:
            return ts.TimeValue()/KFactor_Multiplier
            return ts.TimeValue()/KFactor_Multiplier
            
def bond_index_calculations(ins_oid, bond_weights, previous_weights, t, s, theoretical_values = {}, theoretical_ytms = {}, user_input='No', input_file='F:\\Albi Yields.csv'):
    ...
    prev_weight_change = rebasing_day(t, 'Previous')
    todays_kfactor = getKFactor(ins_oid, t)
    ...
    calendar = acm.FCurrency['ZAR'].Calendar()
    t_prev = calendar.AdjustBankingDays(t, -1)
    s_prev = calendar.AdjustBankingDays(t_prev, 3)
    ...
    for b in bond_weights.Keys():
        if user_input == 'Yes':
            ...
            if theoretical_ytms:
                bond_calcs['Yit'][b] = theoretical_ytms[b].Number()
            else:
                bond_calcs['Yit'][b] = ael.Instrument[b].spot_price()
            if theoretical_values:
                bond_calcs['Pit'][b] = 100 * theoretical_values[b].Number()
            else:    
        ...
        bond_calcs['gi'][b] = acm.FInstrument[b].Legs()[0].FixedRate()
        print bond_calcs
        if bond_calcs['Cit'][b] >= s:
            bond_calcs['Hit'][b] = float(acm.Time.DateDifference(t, s)) / acm.Time.DateDifference(bond_calcs['Cmit'][b], bond_calcs['Cit'][b])
        else:
            bond_calcs['Hit'][b] = float(acm.Time.DateDifference(s, bond_calcs['Cit'][b])) / acm.Time.DateDifference(bond_calcs['Cpit'][b], bond_calcs['Cit'][b]) \
                    + float(acm.Time.DateDifference(bond_calcs['Cit'][b], t)) / acm.Time.DateDifference(bond_calcs['Cit'][b], bond_calcs['Cmit'][b])
        ...
        if (ex_coupon_day <= s <= bond_calcs['Cit'][b]) or (s_prev < bond_calcs['Cit'][b] and s >= bond_calcs['Cit'][b]):
            ...
            first_day = calendar.AdjustBankingDays(ex_coupon_day, -3)
            ...
            if first_day <= prev_weight_change <= bond_calcs['Cit'][b]:
                ...
                if b in previous_weights.keys():
                    bond_calcs['Nitp'][b] = getKFactor(ins_oid, first_day) * previous_weights[b]
                else:
                    bond_calcs['Nitp'][b] = 0.0
            else:
                print ins_oid, first_day
                bond_calcs['Nitp'][b] = getKFactor(ins_oid, first_day) * bond_weights[b]
        else:
            bond_calcs['Nitp'][b] = 0.0
        ...
        bond_calcs['Vit'][b] = bond_calcs['Xit'][b]*bond_calcs['Dit'][b]*(1/(1 + bond_calcs['Yit'][b]/float(200))) \
                **(max(float(acm.Time.DateDifference(s, bond_calcs['Cit'][b])), 0.0) / acm.Time.DateDifference(bond_calcs['Cmit'][b], bond_calcs['Cit'][b]))
...
def index_value(bond_index_oid, theoretical_values, theoretical_ytms, weights, previous_weights, valuation_date):
    ''' Calculate the value of a bond_index
    '''
    
    # value day and settlement day
    calendar = acm.FCurrency['ZAR'].Calendar()
    settlement_date = calendar.AdjustBankingDays(calendar.ModifyDate(None, None, valuation_date, 'Preceding'), 3)
    
    # Get all the intermediate calculations needed for calculating the final value of the index
    index_calcs = bond_index_calculations(bond_index_oid, weights, previous_weights, valuation_date, settlement_date, theoretical_values, theoretical_ytms)
    return (index_calcs['Bt'] + index_calcs['Ct'])
...
def calculate_kfactor(bond_index, user_input='No', input_file='F:\\Albi Yields.csv'):
    ...
    weight_change_date = rebasing_day(vd, 'Next')
...
def rebase_instruments(bond_index):
    ...
    if ael.date_today() == rebasing_day(ael.date_today(), 'Next'):     
...
def get_previous_index(bond_index):
    prev_index = acm.FInstrument[bond_index.add_info('Previous_Index')]
    return prev_index
...
def calculation_report(bond_index, user_input='No', input_file='F:\\Albi Yields.csv'):
    ...
    print 'Previous weight change date:', rebasing_day(vd, 'Previous')
    print 'Next weight change date:', rebasing_day(vd, 'Next')
