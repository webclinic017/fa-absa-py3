...
def getTBbasedOnPoints(points, logger, onPointDate):
    ...
    if onPointDate:
        ...
        buckets = [(b[1], ParseDateStrToPeriod(b[0], logger)[1]) for b in buckets if b[0].lower() != 'rest']
        cmp = functools.cmp_to_key(lambda a, b: Compare(a[1], b[1]))
        buckets.sort(key=cmp)
        buckets= ['{0}-{1}'.format(b[0], b[1]) for b in buckets]
    return buckets
