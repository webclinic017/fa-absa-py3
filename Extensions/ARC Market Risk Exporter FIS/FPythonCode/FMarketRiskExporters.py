...
class PLExplainExporter(FMarketRiskExporterBase):
    ...
    def getPosReferredYieldCurves(self, posObj, columnName):
        ...
        vals = calcSpace.CreateCalculation(selectedNode, columnName).Value()
        if type(vals) is str and vals !='':
            yieldCurves.append(vals)
        else:
            yieldCurves = [v.Name() for v in vals] if vals else []
        it = selectedNode.Iterator().FirstChild()
        while it:
            tree = it.Tree()
            vals = calcSpace.CreateCalculation(tree, columnName).Value()
            if type(vals) is str and vals != '' and vals not in yieldCurves:
                yieldCurves.append(vals)
            elif type(vals) is list:
                ycs = [v.Name() for v in vals if str(v) not in yieldCurves] if vals else []
                yieldCurves.extend(ycs)
            it = it.NextSibling()
