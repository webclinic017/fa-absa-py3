...
The hook can also be used for custom processing of any kind. This can be done
in the function ca_modify(..).
...
This hook is called from the Corporate Action script if Method does not equal
Adjust.  It provides the possibility to adjust the old derivative instrument.
old_ins is the clone of the old instrument in which modifications should be
made.

Adjustments made to the instrument will only be supported in the rollback
functionality if a list of changed attribute names are returned along with the
instrument in a tuple.  In the same way, adjustments made to children of the
entity will only be supported in the rollback functionality if a list of
tuples are returned as a third element, along with the new instrument and the
list of changed instrument attributes. These tuples must contain a child
entity, along with a list of changed attributes if it is an clone.
...
@return str The new instrument id (max 39 characters). Return False if the
standard naming should be used.
@example
def update_name_hook(ca):
    if ca.x.otc:
        ...
        if ca.new_insaddr:
            ...
            s = ca.x.suggest_id() + new_mod
            s = s[0:39] # Insid can be max 39 characters.
        return s
...
(max 39 characters).  Optionally a tuple with the Return False if the standard
