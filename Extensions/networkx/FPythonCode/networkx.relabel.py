...
def convert_node_labels_to_integers(G, first_label=0, ordering="default",
    ...
    if ordering == "default":
        mapping = dict(list(zip(G.nodes(), range(first_label, N))))
    elif ordering == "sorted":
        nlist = sorted(G.nodes())
        mapping = dict(list(zip(nlist, range(first_label, N))))
    elif ordering == "increasing degree":
        dv_pairs = [(d, n) for (n, d) in G.degree()]
        dv_pairs.sort()  # in-place sort from lowest to highest degree
        mapping = dict(list(zip([n for d, n in dv_pairs], range(first_label, N))))
