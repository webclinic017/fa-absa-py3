...
#__src_file__ = "extensions/SecurityLoanDealCustom/etc/SecurityLoan_DP.py"
import acm
...
class SecurityLoanLeg(LegDefinition):
    ...
    def SetInitialFX(self, value):
        ...
        self._lastInitialFX = value
        
    def IsCrossCurrencyAndFixingStatusMarket(self):
        return self.Instrument().Originator().StorageId() > 0 and self._lastInitialFX == 0.0 and self.indexRef and self.indexRef.Currency() != self.Leg().Instrument().Currency()
    ...
    def GetInitialFX(self):
        if self.indexRefFXFixingType == 'Explicit':
            ...
            if first:
                if first.IsFixed():
                    self._lastInitialFX = first.FixingValue()
        if self.IsCrossCurrencyAndFixingStatusMarket():
            self._lastInitialFX = self.CalculateInitialFxEstimate()
        return self._lastInitialFX
    ...
    def CalculateInitialFxEstimate(self):
        try:
            ...
            resetDate = calendarInfo.AdjustBankingDays(leg.StartDate(), -1)
            ...
            return leg.IndexRef().Currency().Calculation().FXRate(calcSpaceCollection, leg.Currency(), resetValueDay).Number()
        except:
            pass
    ...
    def Nominal(self, value = 'NoValue', **kwargs):
        if value == 'NoValue':
            ...
            if self.Instrument().Underlying():
                ...
                return self.Leg().Calculation().Nominal(space, self.Trade(), self.Trade().ValueDay()).Number() * payFactor * contractSize
            else:
                return 0.0
        else:
    ...
    def AttributeChanged(self, attributeName, oldValue, newValue, userInputAttributeName):
        if self.priceFixingStatus == 'Starting' and self._lastInitialFX:
            first = self.InitialFXFixing()
            if first and not first.IsFixed():
                first.FixFixingValue(self._lastInitialFX)
...
class SecurityLoanCashFlowInstrumentDefinition(CashFlowInstrumentDefinition):
    ...
    def AskBeforeSave(self, attributeName, saveConfig):
        ...
        if not self.cfgRegenerate and ins.IsStorageImage():
            ...
            resets = copy.FirstPayLeg().Resets()
            for reset in resets:
                if reset.Original() and reset.Original().IsFixed() and not reset.IsFixed():
                    reset.FixFixingValue = reset.Original().FixingValue()
                    reset.ReadTime = reset.Original().ReadTime()
    ...
    # Attribute override
    def AttributeOverrides(self, overrideAccumulator):
        overrideAccumulator({'cfgRegenerateAll'                         : dict(enabled=self.UniqueCallback('@CfgRegenerateAllEnabled'))})

    def CfgRegenerateAllEnabled(self, *args):
        trade = self.Owner().DealPackage().Trades().First()
        allowedStatus = True if trade.Status() == 'Simulated' else False
        return self.cfgRegenerate and allowedStatus
...
class SecurityLoanDefinition(DealDefinition):
    ...
    ins                 = SecurityLoanCashFlowInstrumentDefinition( instrument="Instrument" )
    ...
    def OnInit(self):
        ...
        self._dummyLeg.NominalScaling = 'Price'
    ...
    def AttributeOverrides(self, overrideAccumulator):
        overrideAccumulator(
            {'ins_currency': dict(onChanged='@OnInstrumentCurrencyChanged'),
             ...
             'ins_underlyingType': dict(label='Type',
                                                               maxWidth=500),
    ...
    def UpdateIndexRefFXFixingType(self):
        self.payLeg.UpdateIndexRefFXFixingType()
        if self.ReceiveLeg():
            self.receiveLeg.UpdateIndexRefFXFixingType()
    ...
    def OnUnderlyingChanged(self, attributeName, oldValue, newValue, *args):
        ...
        self.SetAttribute('receiveLeg_indexRef', newValue)
    ...
    def ReceiveLegVisibleAndNotOpenEnd(self, *args):
        visible = False
        if self.ReceiveLegVisible():
            if self.Leg().Instrument().OpenEnd() == 'Open End':
