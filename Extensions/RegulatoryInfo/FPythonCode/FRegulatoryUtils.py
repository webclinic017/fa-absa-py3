...
#__src_file__ = "extensions/RegulatoryInfo/../SwiftIntegration/RegulatoryInfo/General/FRegulatoryUtils.py"
"""------------------------------------------------------------------------
MODULE
    FRegulatoryUtils -
DESCRIPTION:
...
class WAIVER:
    NONE,SIZE,RFPT,PRIC,OILQ,NLIQ,ILQD = range(7)
class OTCPOSTTRADEINDICATOR:
    NONE,BENC,ACTX,LRGS,ILQD,SIZE,CANC,AMND,SDIV,RPRI,DUPL,TNCP,TPAC,XFPH = range(14)

def get_choicelist_pos(choicelist_val, choicelist_name):
    choicelist = eval(choicelist_name)
    reverse_dict = dict(list(zip(choicelist.values(), choicelist.keys())))
    if reverse_dict.has_key(choicelist_val):
    ...
    if query == '':#it means we have to pull out data for all
        ...
        if (not query_result) and ais in add_info_dict_16_4 and integration_utils.get_acm_version_override() >= 2016.4:
            ...
            try:
                ...
                print "Error", str(e)
                print "Expected datetime format not provided for <%s>"%ais
...
def DeleteField(acm_object, add_info):
    ...
    if integration_utils.get_acm_version_override() >= 2016.4 and add_info in add_info_dict_16_4:
        ...
        if reg_info and adm_attribute.has_key(add_info):
            reg_info = reg_info[0]
...
def get_log_message(reg_info, attribute, log_dict, log_val):
    if log_dict.has_key('NAME') and attribute in log_dict['NAME']:
        try:
            log_val = log_val + '\n' + attribute.strip() + " = " +  eval("reg_info." + attribute + "().Name()")
        except:
            log_val = log_val + '\n' + attribute.strip() + " = None"

    elif log_dict.has_key('OID') and attribute in log_dict['OID']:
        try:
            log_val = log_val + '\n' + attribute.strip() + " = " +  eval("reg_info." + attribute + "().Name()")
        except:
            log_val = log_val + '\n' + attribute.strip() + " = None"
    elif log_dict.has_key('FULLNAME') and attribute in log_dict['FULLNAME']:
        try:
...
def log_reg_attributes(reg_info):
    ...
    log_reg_functions = {
                ...
                'FTradeRegulatoryInfo' : ['ReportDeferToTime', 'RepositoryId', 'ClearingBroker', \
                        ...
                        'TimePrecisionInUTC', 'TradeTimeWithPrecision', 'OurTrader', 'Trade', 'OurOrg', 'TheirOrg']
                }
    log_val = ''
    log_dict = None
    class_name = None
    try:
        ...
        pass
    if log_list.has_key(class_name):
        log_dict = log_list[class_name]
    if log_reg_functions.has_key(class_name):
    ...
    if str(str_val):
        for item in str_val.split(","):
            try:
                ...
                print "valid values not provided for %s"%choice_decl
    return bitmask
...
def is_valid_instrument(instrument_name):
    valid_ins = False
    if acm.FInstrument[instrument_name]:
        valid_ins = True
    return valid_ins

def ins_valid_ins_with_isin(instrument_name):
    isin_present = False
