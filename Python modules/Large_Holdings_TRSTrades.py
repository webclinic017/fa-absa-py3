...
import ael, string, acm, FSQL_functions
...
def Write(t,FileDir,Filename,*rest): 
    ...
    for leg in ins.Legs():
        if (leg.LegType() == 'Total Return') and (leg.IndexRef().InsType() in ('Stock', 'EquityIndex')):
            indexRef = leg.IndexRef()
            
            TrdNbr          =       trade.Oid()    
            Instrument      =       trade.Instrument().Name()
            Price           =       leg.InitialIndexValue()
            Qty             =       trade.Quantity()
            ...
            try:
                Underlying      =       indexRef.Name()
            except:
                Underlying      =       ''

            Type            =       indexRef.InsType()    
            Date_and_Time   =       trade.TradeTime()
            Isin            =       ins.Isin()
            Strike          =       ''
            Exercise        =       ''
            ExpT            =       acm.Time().DateDifference(ins.ExpiryDate(), acm.Time().DateToday()) / 365.0
            ...
            if leg.PayLeg():
                EQDelta         =     -1 * trade.Nominal()
            else:
                EQDelta         =     trade.Nominal()
...
#OpenFile(0,'C:\\','Large_Holdings_TRSTrades.csv')
#trd = ael.Trade[8151952]
#Write(trd, 'C:\\','Large_Holdings_TRSTrades.csv')
