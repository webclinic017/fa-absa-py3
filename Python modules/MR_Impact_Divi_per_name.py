...
def SDividend():
    SDividend = []
    for d in ael.DividendStream:
        if str(1) not in d.name:
            SDividend.append(d.name)
    SDividend.sort()
    return SDividend

           
def divImpact_pershare(stf, outpath, percbump, share):
    ...
    pvC_impact = open(outpath + stf + '_' + share + '_' + str(percbump) +'_' + 'divImpact' + '_'+ dt_str + '.xls', 'w')
...
ael_variables = [('TrdFilter', 'TrdFilter', 'string', TrdFilter(), 'MO_EqRisk_Index', 1),
                 ...
                 ('Share', 'Share', 'string', SDividend(), 'ASA1', 1),] 
...
def ael_main(ael_dict):
    ...
    share = ael_dict['Share']
    
    divImpact_pershare(stf, outpath, percbump, share)
