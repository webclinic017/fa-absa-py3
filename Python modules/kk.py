...
def CopyBasisCurvesToSOB():
    crvs = ('ZAR-BASIS', 'ZAR-PRIME')
    ...
    for crv in crvs:
        ...
        sprd = {}
        for p in yc.points():
            ps = ael.YCSpread.select('point_seqnbr = ' + str(p.seqnbr))[0]
            sprd[p.date_period] = ps.spread
        
        # write spreads to sob curve
        yc = ael.YieldCurve[crv + '-SOB']
        for p in yc.points():
            ps = ael.YCSpread.select('point_seqnbr = ' + str(p.seqnbr))[0].clone()
            t = p.date_period
            ps.spread = sprd[t]
            ps.commit()
