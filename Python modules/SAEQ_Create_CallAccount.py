...
def update_leg(ins, dat):
    ...
    for l in ins.legs():
        ...
        lc.nominal_factor = 1
        if flag:
            lc.float_rate = ael.Instrument['ZAR-SAFEX-ON-DEP']
            if ins.insid.find('_L') > 0:
                lc.spread = -0.10
            else:
                lc.spread = 0.45
        else:
...
#getcl()
#creat_call(ael.Portfolio['47274_CFD_ZERO'],ael.date('2008-01-01'))
creat_call(ael.Portfolio['BARCAP_CFD'], ael.date('2008-09-08'), 1)
