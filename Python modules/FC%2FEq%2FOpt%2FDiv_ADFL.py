...
def getavgmindate(i):
    mindate = i.exp_day.add_days(1)
    ex = i.exotic_events()
    for e in ex:
        if e.type == 'Average strike':
            if mindate >= e.date:
                mindate = e.date
    return mindate
...
def SQL_ModelInputs2(i, input, FairForwardUnd, *rest):
    #print input, FairForwardUnd
    # retrieve d (dividend yield) from additional info (NACA format)
    try:
    	d = float(i.und_insaddr.add_info('DividendYield'))
    except:
...
# copy of FC/Eq/Opt/Div     added function below to return Vol in Trading Manager
#                           for Options with underlying Stock
#                           see Context : Customer
#                           ExtensionAttribute : volatilityInPerCent
#                           Line : 5
def SQL_ModelInputs_ADFL(i):
    #print i.insid, string.find(i.insid, 'RUT')
    
    if string.find(i.insid, 'RUT') == -1:
        return i.used_vol()
    else:
        return SQL_ModelInputs(i, 'sigma') * 100
...
# copy of FC/Eq/Opt/Div     added function below to return Vol in Trading Manager
#                           for Options with underlying EquityIndex
#                           see Context : Customer
#                           ExtensionAttribute : volatilityInPerCent
#                           Line : 5
def SQL_ModelInputs_ADFL2(i, FairForwardUnd):
    #print 'FF', FairForwardUnd
    if i.und_instype == 'Stock':
        if string.find(i.insid, 'RUT') == -1:
            return i.used_vol()
        else:
            return SQL_ModelInputs2(i, 'sigma', FairForwardUnd) * 100
    elif i.und_instype == 'EquityIndex':
        if string.find(i.insid, 'SAPY') != -1:
            return i.used_vol() 
        elif string.find(i.insid, 'AGM') != -1:
            return i.used_vol()
        elif string.find(i.insid, 'ALBI') != -1:
            return i.used_vol()    
        elif string.find(i.insid, 'XGD') != -1:
            return i.used_vol()
        elif string.find(i.insid, 'XGF') != -1:
            return i.used_vol()
        elif string.find(i.insid, 'XGH') != -1:
            return i.used_vol()
        elif string.find(i.und_insaddr.insid, 'BANK_BASKET_3') != -1:
            return i.used_vol()
        elif string.find(i.und_insaddr.insid, 'IPL_EQS_BASKET') != -1:
            return i.used_vol()
        elif string.find(i.und_insaddr.insid, 'ZAR/RES_BASKET') != -1:
            return i.used_vol()
        elif string.find(i.und_insaddr.insid, 'ZAR/BANK_BASKET_4') != -1:
            return i.used_vol()
        elif string.find(i.und_insaddr.insid, 'ZAR/XIY_Basket') != -1:
            return i.used_vol()
        else:
            return SQL_ModelInputs2(i, 'sigma', FairForwardUnd) * 100
    else:
...
#i = ael.Instrument['ZAR/EQ/ALSI/07DEC10/C/18342.10/ASIA/OTC']
#print SQL_ModelInputs_ADFL2(i, 32093)
    
#i = ael.Instrument['ZAR/SAPY/25JAN08/C/386.91/OTC/ASIA']
#print SQL_ModelInputs_ADFL2(i, 382.453210774043)
