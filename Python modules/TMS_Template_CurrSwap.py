...
Description:    TMS Template for CurrSwaps

---------------------------------------------------------------------------------'''

import ael, amb, time, datetime, TMS_Functions, SAGEN_Resets, SAGEN_Cashflows

def CurrSwap_Message(ent_op, entity_addr):
    ...
    for Leg in Legs:
            ...
            if cf.type != 'Fixed Amount':
                ...
                if l.type == 'Float':
                    if cf.start_day.to_string('%Y-%m-%d') <= DateToday:
                        ...
                        CF_msg.mbf_add_double("CFNominal", abs(TObject.quantity * IObject.contr_size * cf.nominal_factor * l.nominal_factor))
                else:
                    CF_msg.mbf_add_double("CFNominal", abs(TObject.quantity * IObject.contr_size * cf.nominal_factor * l.nominal_factor))
                ...
                #CF_msg.mbf_add_double("Projected_cf",cf.projected_cf())
        ...
        for cf in l.cash_flows():
            if cf.type == 'Fixed Amount':
                ...
                P_msg.mbf_add_string("P_Date", cf.pay_day.to_string('%Y-%m-%d'))
                P_msg.mbf_add_double("P_Nominal", cf.projected_cf() * TObject.quantity)
                P_msg.mbf_add_string("P_Type", 'Exchange')
                P_msg.mbf_end_list()
