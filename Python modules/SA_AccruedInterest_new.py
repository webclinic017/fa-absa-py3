    ...
    for l in legs:
		...
		if request == 'FDI' and end_day <= edate:
                    ...
                    if (start_day<=edate) and (pay_day>edate):		
                        ...
                        WholeCashflow = FixedRate*days/DaysInYear*t.quantity*ins.contr_size*nominal_factor
                        FixedIntPerDay = WholeCashflow/days
                        if edate >= end_day:
                            AccDays = (ddate.days_between(end_day, daycount_method))
                        else:
                            AccDays = (ddate.days_between(edate, daycount_method)+1)
        
                        AccruedCashflow = FixedRate*AccDays/DaysInYear*t.quantity*ins.contr_size*nominal_factor
                        DaysInPeriod = start_day.days_between(end_day, daycount_method)
        
                        if l.payleg == 1:
                            AccruedCashflow = AccruedCashflow * -1
                            FixedIntPerDay = FixedIntPerDay * -1
        
                        
        		 #   print 'days ', days, AccDays, DaysInYear, nominal_factor
        		 ...
        		 #   print
        		 #   print 'FixedIntPerDay ', FixedIntPerDay
            	    	 #   print
        		 ...
        		 #   print 'DaysInPeriod ', DaysInPeriod
                        ...
                        #print
        		#print 'Fixedaccint ', accint, cfwnbr
        ...
        if request == 'FDI':
            FDIFixedAccInt = (FDI_Nominal * FixedRate * FDI_days/36500) - FDI_SUMCF
        else:    
...
#		    edate = edate.add_days(-1)
                    ...
                    if (cf.start_day<=edate) and (cf.pay_day>edate):
#		        print cf.cfwnbr, edate, cf.start_day, cf.end_day   		
...
#	    	        FloatRate = (cf.period_rate(cf.start_day, cf.end_day) * cf.float_rate_factor) + cf.float_rate_offset
                        FloatRate = (cf.period_rate(cf.start_day, edate) * cf.float_rate_factor) + cf.float_rate_offset
		    
                        days = cf.start_day.days_between(cf.end_day, daycount_method) + 1
                        Nominal = t.quantity*ins.contr_size*cf.nominal_factor
                        WholeCashflow = FloatRate*days/DaysInYear*t.quantity*ins.contr_size*cf.nominal_factor
                        FloatIntPerDay = WholeCashflow/days
		    
                        if edate >= cf.end_day:
                            AccDays = (ddate.days_between(cf.end_day, daycount_method))
                        else:
                            AccDays = (ddate.days_between(edate, daycount_method)+1)
		    
                        #float cashflow
                        #AccruedCashflow = FloatRate*(ddate.days_between(edate,daycount_method)+1)/DaysInYear*t.quantity*ins.contr_size*cf.nominal_factor
		    
                        #fixed cashflow
                        AccruedCashflow = FloatRate*AccDays/DaysInYear*t.quantity*ins.contr_size*cf.nominal_factor
		    
                        DaysInPeriod = cf.start_day.days_between(cf.end_day, daycount_method) 
		    
                        if l.payleg == 1:
                            AccruedCashflow = AccruedCashflow * -1
                            FloatIntPerDay = FloatIntPerDay * -1
...
#		        print 'WholeCashflow ', WholeCashflow
#		        print
#		        print 'FloatIntPerDay ', FloatIntPerDay
#		        print
#		        print 'AccruedCashflow ', AccruedCashflow
#		        print 
#		        print 'DaysInPeriod ', DaysInPeriod
#   		        print 
#		        print 'Rate ', FloatRate

                        accint = accint + AccruedCashflow
		    
#		        print
#		        print 'Floataccint ', accint
...
#    	print '\n\n\n'
    if request == 'FixedIntON':
    	return FixedIntPerDay
    elif request == 'FloatIntON':	
    	return FloatIntPerDay
    elif request == 'IntON':
    	return FixedIntPerDay + FloatIntPerDay
    elif request == 'FixedRate':
    	return FixedRate
    elif request == 'FloatRate':
    	return FloatRate
    elif request == 'Rate':
	return FixedRate + FloatRate
    elif request == 'Float':
    	return FloatAccInt
    elif request == 'Fixed':
      	return FixedAccInt
...
#ac = accrued_interest(t, '2006-01-01', '2006-07-05', 'Fixed')
#ac = accrued_interest(t, '2005-01-01', '2005-12-21', '')
#print 'Final', ac
