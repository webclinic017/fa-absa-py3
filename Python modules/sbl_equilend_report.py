...
DESCRIPTION
    Date                : 2020-04-20
    Purpose             : Script generates Equilend downstream report
...
FILE_HEADERS = ["isin", "sedol", "cusip", "quick", "ticker", "secDesc",
                "borrowLen", "unitQty", "contractValue", "contractCurr", "margin",
                "rate", "fee", "dividendRate", "tradeType", "tradeDate", "termDate",
                "collType", "collCurr", "cptyName", "intRefId", "settlementDate", "divTradeInd",
                "Entity 1", "Entity 2", "Entity 3", "Entity 4", "reinvestmentRate", "CptyCode"]
...
class RowData(reporting_utils.ReportingData):
    ...
    def get_row_details(self):
        try:
            ...
            self.data["CptyCode"] = counterparty_code
...
def ael_main(dictionary):
    try:
        file_prefix = dictionary["file_name"]
        file_date = datetime.strptime(acm.Time.DateNow(), "%Y-%m-%d").strftime("%Y%m%d")
        output_filename = "{prefix}.{date}.CEF".format(prefix=file_prefix, date=file_date)
