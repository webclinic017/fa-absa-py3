...
2020-11-19     CHG0142194       Thando Mpalala     Andile Biyana      https://absa.atlassian.net/browse/CMRI-948
    ...
    for l in ins.Legs():
        if l.LegType() == 'Float':
            ...
            if (str(i.insaddr) + '_' + str(l.Oid())) not in InsL:
                ...
                #Base record
                BASFLAG             =       'BAS'
                HeaderName          =       'Basis Swap Float Leg'
                ...
                if i.non_deliverable:
                    ...
                    if (l.OriginalCurr().Name() != i.curr.insid):
                        ...
                        for fl in Legs:
                            ...
                            InitialStrCfProcXREF = l.original_curr.insid + i.curr.insid + '_PP_End'
                ...
                float_rate = l.FloatRateReference()
                if float_rate:
                    output = float_rate.Name()
                    UndrCrvIndXREF 	=	'SCI_' + output + '_' + str(l.ResetType()) + '_' + str(l.ResetPeriodCount()) + str(l.ResetPeriodUnit()) + '_' + str(l.ResetDayOffset()) + str(l.ResetDayMethod())
                else:
                    UndrCrvIndXREF  = ''
                ...
                BASFLAG             =       'rm_ro'
                HeaderName          =       'Basis Swap Float Leg : Structured CashFlow'
                        ...
                        if cf.CashFlowType() == 'Fixed Amount':
                                    ...
                                    if cf.PayDate() <> R.Day():
                                        ...
                                        if i.non_deliverable:
                                            ...
                                            if ((not l.IsLocked()) and (isScaling)):
                                                ...
                                                StrCfPayVAL     = PayRecSign * acm_to_ael(cf).nominal_amount() * l.NominalFactor()
                                                StrCfProcXREF = ''
                            ...
                            try:
                                for r in cf.Resets():
                                    ...
                                    StrCfFwdEndDATE     =       MR_MainFunctions.Datefix(str(cf.EndDate()))
                                    StrCfFwdStartDATE   =       MR_MainFunctions.Datefix(str(cf.StartDate())) 
                                    StrCfHidOddCpFLAG   =       'FALSE'
                                    StrCfInstrFctrVAL   =       '1'
                                    if cf.FloatRateFactor() == '0':
                                        StrCfPayFctrVAL   =       ''
                                    else:
                                        StrCfPayFctrVAL   = cf.FloatRateFactor() * l.NominalFactor()
                                    ...
                                    FXScaling = 0
                                    if i.non_deliverable:
                                        for ndsr in cf.Resets():
                                            if ndsr.ResetType() == 'Settlement FX':
                                                FXScaling = 1
                                        ...
                                        if ((not l.IsLocked()) and (isScaling)):
                                            if (MR_MainFunctions.Datefix(str(r.Day())) < MR_MainFunctions.Datefix(l.StartDate())) and (MR_MainFunctions.Datefix(l.StartDate())== MR_MainFunctions.Datefix(cf.StartDate())):
                                                ...
                                                legStartDate = MR_MainFunctions.Datefix(l.StartDate())
                                                resetFixingDate = MR_MainFunctions.Datefix(r.ReadTime()[0:10])
                                                if (resetFixingDate > defaultFixingDate) and (today > tradeDate and today < legStartDate):# there is fixing at inception
                                                    StrCfProcXREF =  ''  
                                                elif (r.FixingValue() == l.InitialIndexValue()): #if there is no fixing at inception but initialindex = fixing rate                                                     
                                                    StrCfProcXREF =  ''
                                                else:
                                                    StrCfProcXREF  = Locked + Reset + '_PP' # if there is no fixing at inception
                                            elif (r.FixingValue() == l.InitialIndexValue()):# initial_index value = fixing rate 
                                                StrCfProcXREF =  ''
                                            else:
                                                StrCfProcXREF  = Locked + Reset + '_PP'
                                            if cf.Calculation().NominalFactor(calcSpace) == 0:
                                                ...
                                                StrCfPayVAL =  i.contr_size * cf.NominalFactor() * l.NominalFactor()
                                        else:
                                            StrCfPayVAL  = PayRecSign * acm_to_ael(cf).nominal_amount()
                                            StrCfProcXREF =  ''
                                    ...
                                    StrCfRstDateDATE    =       MR_MainFunctions.Datefix(str(cf.StartDate()))
