...
def CheckCFD(temp, insid, prfid, ddate, *rest):
    ...
    for t in trds:
    	...
    	if t.time == ddt.to_time():
            if t.prfnbr == ael.Portfolio[prfid + '_CFD_ZERO']:
...
def CreateCFD(temp, insid, prfid, qty, vwap, fee, premium, Shedge, cfddate,  *rest):
    ...
    port = ael.Portfolio[prfid + '_CFD_ZERO']
    t_new.prfnbr = port
    ...
    t_new.value_day = cfddate.add_banking_day(ccy, 5)
    #t_new.acquire_day = ael.date_today()
    t_new.acquire_day = cfddate.add_banking_day(ccy, 5)
