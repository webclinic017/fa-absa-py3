...
"""----------------------------------------------------------------------------
MODULE
        agg_equity - aggregation of equities and equity derivatives

    (c) Copyright 2000 by Front Capital Systems AB. All rights reserved.

DESCRIPTION
        
        This module is used to perform various aggregation tasks on equity like
        ...
        ael <name of config file> agg_equity.py -f <name of inifile>
    ...
    -verbose,       <0 or 1, where 1 enables printouts>
        ...
        The script can be used to aggregate positions in stocks, and options,
        warrants and future/forwards on stocks.  The instype argument can be
        used if only positions in a specific instrument type should be
        aggregated.
...
if __name__ == "__main__":
    ...
    try:
        ...
        print msg
        print 'Usage: agg_equity.py -f inifilename [-h help]',
        sys.exit(2)
    ...
    if perform_agg_and_arch(M, 'eq') == -1:
    ...
    (get_valid_instypes('eq', 0), get_valid_instypes('eq', 1),
     ael.archived_mode())
    ...
    q = '''select insid from instrument where instype in %s and
    archive_status <= %d''' % \
    (get_valid_instypes('eq', 1), ael.archived_mode())
    ...
    [('date', 'Date', 'date', [], str(ael.date_today()), 0),
     ...
     ('instype', 'Instype', 'string', ['Stock', 'Option', 'Future/Forward',
                                       'Warrant'], None, 0, 1),
    ...
    def ael_main(dictionary):
        ...
        result = 'finished'
        if store_variables(M, dictionary) == -1 or perform_agg_and_arch(M, 'eq') == -1:
            result = 'failed'
