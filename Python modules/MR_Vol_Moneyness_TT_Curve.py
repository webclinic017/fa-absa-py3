'''
Purpose                 :[Market Risk feed files,Added string "nan" for exception handling],[Added CurveFUNC to BAS 'Volatility - Term/TermSPEC']
Department and Desk     :[IT],[MR]
Requester:              :[Natalie Austin],[Susan Kruger]
Developer               :[Douglas Finkel],[Willie van der Bank]
CR Number               :[264536, 686048],[2012-08-10, 380391]
'''
...
def Write(v,FileDir,Filename,*rest):    
    ...
    if (v.seqnbr) not in InsL:
        ...
        for points in v.points():
            ...
            if points.insaddr:
                ...
                if points.insaddr.strike_type in ('Rel Frw', 'Rel Spot'):
                    ...
                    if str(Strike) in ('1.#QNAN', 'NaN', 'nan'):
                        Strike          =       round(points.strike)
        ...
        for Strike in volsurfacedict.keys():
            ...
            IDENTIFIER	        =       v.vol_name + '_TT_' + str(Strike/100)
            NAME                =       v.vol_name + '_TT_' + str(Strike/100)
            ...
            outfile.write('%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n'%(BAS, HeaderName, OBJECT, TYPE, IDENTIFIER, NAME, ActiveFLAG, CurveUnitCAL, CurveUnitDAYC, CurveUnitPERD, CurveUnitUNIT, DatumDATE, OriginOffsetNB, RelativeCurveFLAG, StateProcFUNC, TimeEvolutionFUNC, FunctionIdFLAG, GenVolSTSfExt0FLAG, GenVolSTSfExt1FLAG, GenVolStrTrmSf0SIN, GenVolStrTrmSf1SIN, CurveFUNC))
        ...
        for Strike in sorted(volsurfacedict.keys()):
            ...
            ATTRIBUTE               =       'Procedure Parameter'
            OBJECT                  =       'Volatility - Moneyness/Term/TermSPEC'
            ProcedureParamXREF      =       v.vol_name + '_TT_' + str(Strike/100)
