...
def ael_main(dict):
    ...
    try:
        ...
        outgoingAMBAMessages = []
        fundingInsTypes = ['CL', 'FDC', 'FDE', 'FDI', 'CL Pre Payable', 'FRD Pre Payable 1M', 'CD', 'FTL', 'FRD']
        ...
        while fromDate <= toDate:
            for portfolio in physicalPortfolioTradesList:
                for trade in portfolio.Trades():
                    if trade.Status() == 'Void' or trade.Status() == 'Simulated': continue
                    if trade.AdditionalInfo().Funding_Instype() not in fundingInsTypes: continue
                    if trade.Instrument().IsExpired() == True: continue
                    outgoingMessage = createOutgoingMessageObjectRequest('0', '0', '0',
                                                                         fromDate,
                                                                         termReportDateTime,
                                                                         'INTRADAY_TRADE', '', __name__,
