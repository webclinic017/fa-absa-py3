...
--------------------------------------------------------------------------------
2016-01-27  Faize Adams                 ABITFA-4034: Added trade time field in output.
'''
import ael
...
def get_outstanding(outfile, trade_filter):
    ...
    head = ['Area', 'Trade', 'Days', 'Portfolio', 'Portfolio Code', 'Type',
            ...
            'EconoAmendOPSConf', 'Acquirer', 'Trade type', 'Trade Date']
    ...
    for trade in trades:
        ...
        #Days
        data.append(ael.date_today().bankingdays_between(ael.date(datetime.fromtimestamp(trade.execution_time).strftime('%Y-%m-%d')), ael.Instrument['ZAR']))
        ...
        #Trade time
        data.append(datetime.fromtimestamp(trade.time).strftime('%Y-%m-%d %H:%M:%S'))
...
ael_variables = [
    ...
    ['FileName', 'File Name', 'string', None, 'FO_Outstandings.xls', 0],
