...
class ZarSwapExtract():
    ...
    def rate_data(self):
        ...
        #calculate the curve and simulate to enable ral time updates
        ycc = curve.Clone()
        ycc.Calculate()
        curve.Apply(ycc)
    ...
    def run_zar_swap_extractor(self, path, file_name):
        ...
        write_to_location = os.path.join(path, file_name + ".csv")
        self.write_data_to_file(result, write_to_location)
        self.log("result written to: "+write_to_location )

ael_variables = [
                    ['OutputDirectory', 'Output Directory', 'string', None, 1, 0,\
                        'The directory where the extracts(s) will be generated.', \
                    None, 1],
                    ['FileName', 'Output File Name', 'string', None, 1, 0,\
                        'The file name for the extract(s).', None, 1]
                ]

def ael_main(dict):
    path = dict["OutputDirectory"]
    file_name =dict["FileName"]
    
    runner = ZarSwapExtract()
    runner.run_zar_swap_extractor(path, file_name)
