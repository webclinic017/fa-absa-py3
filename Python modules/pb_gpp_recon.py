...
==========      =========       ======================  ===================================================
2017-03-28      4634552         Ondrej Bahounek         ABITFA-4916: Initial implementation
"""
...
from pb_gpp_general import (DATE_LIST,
                            DATE_KEYS,
                            get_trading_portf,
                            get_gpp_cp,
                            get_account_alias,
                            get_fa_instype_alias,
                            get_gpp_type_from_fa_type,
                            add_common_aelvars,
                            set_general_input)
...
TRADING_PORTF = get_trading_portf()

# Dictionary of funds and their instypes currently currently beeing swept.
# Taken from 'pb_gpp_sweeping_<alias>_<ins_short>_SERVER' tasks.
...
class AbstractChecker(object):
    ...
    def __init__(self, for_date, email_recipients=None):
        ...
        self.email_subject = "GPP recon %s (%s): %s" % (for_date,
...
class Checker_NewTrading(AbstractChecker):
    ...
    class NewTradingProcessor(SimpleCSVFeedProcessor):
        ...
        def _process_record(self, record, dry_run):
            ...
            if not alias:
                LOGGER.warning("Alias '%s' not recognised. Skipping row #%d...",
                    record_data[self._col_account_num], row_nbr)
                return

            ins_shortcut = get_fa_instype_alias(record_data[self._col_ins_type])
            if not ins_shortcut:
                LOGGER.warning("Instrument type '%s' not recognised. Skipping row #%d...",
    ...
    def _create_fund_ins_sch_dict(self):
        ...
        tasks = acm.FAelTask.Select('name like "pb_gpp__sweeping_*_SERVER"')
        for task in tasks:
        ...
        for fund in input_data_dict:
            for ins_shortcut in input_data_dict[fund]:
                msg += "<b>{0}</b> : <b>{1} ({2})</b><br />".format(fund,
                    get_gpp_type_from_fa_type(ins_shortcut), ins_shortcut)
        msg += "<br /><br />"
...
ael_variables.add("file_dir",
                  ...
                  default=r"y:\Jhb\FAReports\AtlasEndOfDay\PrimeClients\GPP\${DATE}",
...
add_common_aelvars(ael_variables, "General")
