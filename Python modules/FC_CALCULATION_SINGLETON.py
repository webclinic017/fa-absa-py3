...
class FC_CALCULATION_SINGLETON:
    ...
    def __init__(self):
        if not FC_CALCULATION_SINGLETON.instance is None:
            ...
            #except Exception, e:
            #    raise Exception(UTILS.Constants.fcExceptionConstants.CALCULATION_SINGLETON_S % str(e))
    ...
    def LoadWorkbookSheetColumns(self):
        ...
        if len(workbookNames) == 0:
            raise UTILS.Constants.fcExceptionConstants.DYNAMIC_CALCULATIONS
        context = acm.GetDefaultContext()
        for workbookName in workbookNames:
            #load the workbook
            workbook = self.GetWorkBook(workbookName)
            if workbook==None:
                raise UTILS.Constants.fcExceptionConstants.NOT_LOAD_WORKBOOK_S % workbookName
            ...
            for worksheet in workbook.Sheets():
                ...
                if self.worksheets.HasKey(sheetName):
                    raise UTILS.Constants.fcExceptionConstants.WORKBOOKS_MUST_BE_UNIQUE % sheetName
                ...
                if self.worksheetColumns.HasKey(sheetName):
                    raise UTILS.Constants.fcExceptionConstants.WORKSHEETS_MUST_BE_UNIQUE % sheetName
                ...
                if self.worksheetCalcSpaces.HasKey(sheetName):
                    raise UTILS.Constants.fcExceptionConstants.WORKSPACE_WORKSHEETS_MUST_BE_UNIQUE % sheetName
    ...
    def getWorksheetColumns(self, sheet):
        try:
            ...
            if columns and len(columns)>0:
                ...
                print UTILS.Constants.fcExceptionConstants.CONTAINS_NO_COLUMNS % sheet.SheetName()
    ...
    def formatColumValue(self, value, column, label):
        ...
        if type(floatValueTest) == float or (str(type(floatValueTest)) == "<type 'FDenominatedValue'>"):
            formattedValue = FC_UTILS.formatNumber(floatValueTest)
        ...
        if (columnFormatter and columnFormatter.IsKindOf(UTILS.Constants.fcGenericConstants.F_NUM_FORMATTER)) or \
                                                            (label and label in UTILS.Parameters.fcGenericParameters.NumberFormatLabelOverrides):
            if type(formattedValue) != float:
    ...
    def calcWorksheetColumnValues(self, sheetName, acmFObject, acmTreeProxy):
        try:
            ...
            if not columns:
                raise UTILS.Constants.fcExceptionConstants.COLUMNS_FOUND_FOR_SHEET % sheetName
                
            #Fetch the calculation space for the sheet from the cached dictionary
            calcSpace = self.worksheetCalcSpaces[sheetName]
            if not calcSpace:
                raise UTILS.Constants.fcExceptionConstants.SPACE_FOUND_FOR_SHEET % sheetName
