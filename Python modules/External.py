...
def ExternalTrades(Trd):
    try:
        ...
        for i in  ael.Instrument.select('instype=Swap'):
            for l in i.legs():  
                for t in i.trades():
                    #if ( abs(i.exp_day.days_between(ExpiryDate)) <= 31):  or 
                    if (( ael.date_today().days_between(i.exp_day) > (6 * 365) and abs(i.exp_day.days_between(ExpiryDate)) <=  182 )) and  i.exp_day != None: 
#                        print " +- month from internal expiry date"
                        print i.insid, t.trdnbr, t.nominal_amount(), i.exp_day, t.acquire_day
                        Trades.append([i.insid, t.trdnbr, t.nominal_amount(), i.exp_day, t.acquire_day]) 
        return Trades
    except: 
            print  Trd.trdnbr, ExpiryDate, TradeDate, StartDate, Nominal, Rate 
            
'''                    if abs(l.fixed_rate - Rate)/100 <= 0.01 and 
                    t.nominal_amount() == Nominal and 
                    TradeDate > t.acquire_day and
                    abs(l.start_day.days_between(StartDate)) <= 31 and
                    l.type == 'Fixed' and 
                    i.exp_day != None and
                    
                    t.status in ('FO Confirmed', 'BO Confirmed', 'BO-BO Confirmed'):                                                 
