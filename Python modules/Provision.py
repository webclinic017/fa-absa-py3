...
2018-0523               Tibor Reiss     ABITFA-5413: check for start and end
                                        date separately in _reset_period.
-------------------------------------------------------------------------------
"""
        ...
        if (common_object.IsKindOf(acm.FCashFlow) or
            ...
            if common_object.IsKindOf(acm.FReset):
                if not start_date:
                    ...
                    LOGGER.debug("Reset '%s' has not start date. Using Cashflow values.", common_object.Oid())
                if not end_date:
                    end_date = common_object.CashFlow().EndDate()
                    LOGGER.debug("Reset '%s' has not end date. Using Cashflow values.", common_object.Oid())
