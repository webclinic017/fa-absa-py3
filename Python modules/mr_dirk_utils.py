import ael, time, acm
from dirk_utils import mightBeUsedPrice, update_cpi
...
def basis_delta(sprd, stf, syc, sftsize, *rest):
    t0 = time.time()
    ...
    sc = sprd.clone()
    sc.spread = sprd.spread + sftsize/100   
    sc.apply()            
...
def bm_delta(ins, stf, syc, lvl, shft, *rest): 
    t0 = time.time()
...
# Shift all benchmarks by same amount
    ...
    bm = ael.YieldCurve[syc].benchmarks()
    for b in bm:
 ...
 #End
    ...
    for p in prc:
        ...
        if mightBeUsedPrice(p) or p.ptynbr.ptyid == 'SPOT':
            ...
            pc = p.clone()
            log(1, 'changing point from', pc.settle, 'to', pc.settle + shft)
            
            pc.settle = p.settle + shft
            pc.last = p.last + shft
            pc.bid = p.bid + shft
            pc.ask = p.ask + shft
            pc.apply()   
...
def Bond_BM_delta_acm(ins, stf, *rest):    
    ...
    #MK4 tag            = acm.CreateEBTag()
    context             = 'Standard'    
    portfolio           = acm.FTradeSelection[stf]
    #MK4 groupers       = acm.GetAllPortfolioGroupers()
    #MK4 grouper        = groupers.At("Default")
    #MK4 fiat           = acm.CreateInstrumentAndTradesTree(portfolio, grouper, None, 0, 0)
    #MK4 adfl           = 'object:*"valPLEnd"[isCashGrouping=0]' 
    #MK4 eval           = acm.GetCalculatedValueFromString(fiat, context, adfl, tag) 
    #MK4 pv_base        = eval.Value()    
    calc_space          = acm.Calculations().CreateCalculationSpace(context, 'FPortfolioSheet')
    formatted_value     = calc_space.CalculateValue(portfolio, 'Portfolio Value End')
...
def Bond_BM_delta_ins_acm(ins, *rest):    
    ...
    dt = ael.date_today()
    yc = fins.MappedDiscountCurve().Parameter()
    repo = fins.MappedRepoCurve().Parameter()
    spot = fins.GetSpotDay(dt, None)
    
    pv_base = fins.PresentUnitValue(yc, repo, fins.mtm_value_fo(), dt, dt, spot)     
    ...
    pv_up = fins.PresentUnitValue(yc, repo, ins.mtm_value_fo(), dt, dt, spot)     
...
def pv_tf_acm(stf):
    ...
    #MK4 portfolio   = acm.FTradeSelection[stf]
    #MK4 groupers    = acm.GetAllPortfolioGroupers()
    #MK4 grouper     = groupers.At("Default")
    #MK4 fiat        = acm.CreateInstrumentAndTradesTree(portfolio, grouper, None, 0, 0)
    #MK4 tag         = acm.CreateEBTag()
    #MK4 context     = 'Standard'        
    #MK4 adfl        = 'object:*"valPLEnd"[isCashGrouping=0]' 
    #MK4 eval        = acm.GetCalculatedValueFromString(fiat, context, adfl, tag) 
    #MK4 pv          = eval.Value()    

    #MK4 We may need to simulate [isCashGrouping=0]
    context             = 'Standard'    
    portfolio           = acm.FTradeSelection[stf]
    calc_space          = acm.Calculations().CreateCalculationSpace(context, 'FPortfolioSheet')
...
def pv_trd_acm(trd):
    ...
    #MK4 tag         = acm.CreateEBTag()
    #MK4 context     = 'Standard'    
    #MK4 portfolio   = acm.FTradeRow[trd]
    #MK4 groupers    = acm.GetAllPortfolioGroupers()
    #MK4 grouper     = groupers.At("Default")
    #MK4 fiat        = acm.CreateInstrumentAndTradesTree(portfolio, grouper, None, 0, 0)
    #MK4 adfl        = 'object:*"valPLEnd"[isCashGrouping=0]' 
