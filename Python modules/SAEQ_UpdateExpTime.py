...
trades = ael.TradeFilter["SAEQ_Time_Change"].trades()
p = 0
histList = []
...
for t in trades:
    ...
    if ((t.insaddr.instype in ("Future/Forward")) and (t.insaddr.insid not in histList)):
...
for t in trades:
    ...
    if ((t.insaddr.instype in ("Option")) and (t.insaddr.insid not in histList)):
        eto = t.insaddr.exp_time
        et = t.insaddr.exp_day.to_time() + updateTime
