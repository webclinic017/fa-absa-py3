...
#__src_file__ = "extensions/vps/etc/FVPSHistoricYieldCurve.py"
"""----------------------------------------------------------------------------
    (c) Copyright 2017 SunGard Front Arena. All rights reserved.
----------------------------------------------------------------------------"""
"""----------------------------------------------------------------------------
MODULE
    FVPSHistoricYieldCurve - Stores historical yield curves
...
class _YcCloneMngr(FBDPWorld.WorldInterface):
    ...
    def __init__(self, world, strIsoDateToday):
        super(_YcCloneMngr, self).__init__(world)
...
def _duplicateOrigYc(world, process_date, ycToStoreList, memberCurves,
                    realtimeUpdatedUnchanged = False):
    ycCloneMngr = _YcCloneMngr(world, ael.date(process_date).to_string(ael.DATE_ISO))
    for origYc in ycToStoreList:
        try:
            ...
            lastHistYcOid = _findLastHistYcOid(origYcOid, process_date)
            if lastHistYcOid:
                continue
            else:
                cloneYc = _createAndPrepCloneYc(world, ycCloneMngr,
                        origYc, process_date, realtimeUpdatedUnchanged)
            if not cloneYc:
    ...
    for origYcOid, cloneMember in origYcOidAndYcClonePairs.items():
        if origYcOid in memberCurvesOids:
            try:
                ...
                failMsg = ("Unable to update yield curve '{0}': "
                    "{1}".format(cloneMember.Name(), _get_exception()))
                world.logError(failMsg)
...
def _recalculateYcClones(world, origYcOidAndYcClonePairs, recalculationOrder):
    ...
    for origYcOid in recalculationOrder:
        ...
        if rtn or not resultErrorMsgs:
            ...
            recalcYcInfoList.append(_RecalcYcInfo(origYcOid, cloneYc,
                isRecalculated=False, cloneYcName=cloneYcName))
            errMsg = ('Skipping the historical yield curve '
                    '\'{0}\'. Calculation Failed. Reasons: {1}. '.format(
...
def update_all_yield_curves(world, params, process_date):
    """
    Store historical yield curves according to the settings in FVPSVariables.
    Main function.
    """
    world.logInfo('Storing yield curves for date %s' % process_date)
    selOrigYcNameList = _findSelectedOrigYcNamesToStore(world,
            inclAllYcByDefault=params.yield_curve_base,
            ycNameInclList=params.yield_curves_incl,
            ycNameExclList=params.yield_curves_excl)
    world.logInfo('    Searching and include based curves.')
    selAndBasedOrigYcNameList = (FBDPYieldCurveLib.
            findTransitivelyBasedOriginalYieldCurveNameList(
    ...
    origYcOidAndYcClonePairs = _duplicateOrigYc(world, process_date,
              ycToStoreList, memberCurves, False)
    return
