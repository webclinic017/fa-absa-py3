...
----------------------------------------------------------------------------------------------------
Angelique Macnabe    Kevin Kistan          C889542  (2014-09-22)   Refactoring of valuation
                                                                   for Bond Instrument
----------------------------------------------------------------------------------------------------
"""
        ...
        for t in self.trade_list:
            ...
            if legs:
                if len(legs) == 2:
                    ...
                    currency_1 = legs[0].Currency().Name()
                    nominal_1 = t.Nominal()
                    ...
                    for cashflow in legs[0].CashFlows():
                        ...
                        nominal_1 = cashflow.Calculation().Nominal(self.std_calc_space,
                            t, currency_1).Number()
            else:
                currency_1 = t.Currency().Name()
                nominal_1 = t.Nominal()
            ...
            if t.Instrument().Underlying():
                isin = t.Instrument().Underlying().Isin()
            else:
                isin = t.Instrument().Isin()
            
            
            #Price of the bond as at the valuation date
            price = "{0:.4f}".format(t.Instrument().used_price())
...
def BuildValuation(mtm_ccy, ctpy, date, trade_list, output_dir, file_format):
    ...
    filename = "BOND_Client_Valuation-{0}-{1}".format(
...
ael_variables = [
    ...
    ['path', 'Output Folder:', 'string', None,
        r'Y:\Jhb\Arena\Data\PCG-Client-Valuations\Bonds', 1],
]
