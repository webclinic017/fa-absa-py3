    ...
    for k in keys:        
        if dat[k] <> 0.0 and k[0] == 'FWD':
            ...
            if ins == None or ins.instype <> ins_template_type:
                ins = ael.Instrument['FX_TEMPLATE'].new()
                ins.insid = insid                    
                ins.curr = curr_ins
                ins.commit() 
