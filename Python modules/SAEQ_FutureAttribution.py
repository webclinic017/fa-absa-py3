...
ael_variables = [('StartDate', 'StartDate', 'date', None, ael.date_today().add_banking_day(ael.Instrument['ZAR'], 0), 1),
                 ('EndDate', 'EndDate', 'date', None, ael.date('2008-03-20'), 1),
...
def Simulator(temp,Indexs,StartDate,EndDate,tel,Server,*rest):
    ...
    outfile = Server  + 'FutureForwardVariables_' + str(tel) + '_' + StartDate.to_string('%y%m%d') + '.csv'
    print outfile
    ...
    print'Success'
    print 'The file has been saved at:' + Server + 'FutureForwardVariables_' + str(tel) + '_' + StartDate.to_string('%y%m%d') + '.csv'
    return
