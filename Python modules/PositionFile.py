...
def CreatePosition(t, filename):
    ...
    if t.insaddr.instype == 'Portfolio Swap' and t.insaddr.legs(): 
        pswap_first_leg = t.insaddr.legs()[0]
        if (pswap_first_leg.type in ('Total Return', 'Position Total Return')
                and pswap_first_leg.payleg):
            PositionUnitsVAL = -1*t.quantity
    ...
    if t.insaddr.non_deliverable:
        InstrumentTYPE              = t.insaddr.instype + '_NDS'
    else:
...
def CreatePositionCash(temp,FileDir,Filename,PositionName,Portfolio,curr,cash,*rest):
    ...
    TradeAggregationNAME = str(curr) + '_Cash_' + str(Portfolio)
...
def CreatePositionStock(temp,FileDir,Filename,PositionName,InsName,PortName,CounterParty,Position,*rest):
    ...
    TradeAggregationNAME        =       str(Instrument.insid) + '_' + str(PortName) + '_' + str(CounterParty)
...
def CreatePositionPremium(t, filename):
    ...
    OBJECT	        =	'PositionSPEC'
    TYPE	        =	'Position'
    IDENTIFIER	        =	'trdnbr_'+str(t.trdnbr)+'_Premium'
    PositionUnitsCAL	=	''
    PositionUnitsDAYC	=	''
    PositionUnitsFUNC	=	''
    PositionUnitsPERD	=	''
    PositionUnitsSTRG	=	''
    PositionUnitsUNIT	=	''
    PositionUnitsVAL	=	-1 * t.quantity / abs(t.quantity)
    
    InstrumentXREF	=	str(t.trdnbr)+'_Premium' 
...
def CreatePositionStockPremium(temp,FileDir,Filename,PositionName,InsName,PortName,CounterParty,AcquireDay,Position,*rest):
    ...
    IDENTIFIER                  =       str(Instrument.insid) + '_' + str(PortName) + '_' + str(CounterParty) + '_' + str(AcquireDay)+'_Premium'
