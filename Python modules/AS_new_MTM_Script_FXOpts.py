...
def UpdateExoticEventValueWithMtMPrice(exoticEvent):
    ...
    if price:
        ...
        ael.log_all(underlying.Name() + ": ERROR: No MtM price in exists in " + currency + \
...
def GetBarrierInstruments(instrumentList, dateToday, getHistorical):
    barrierInstruments = []
    for instrument in instrumentList: 
        if instrument.IsBarrier() and instrument.ExpiryDate() > dateToday and \
    ...
    for instrument in barrierInstruments:
        ...
        if len(eventList) > 0:
            ...
            if underlyingPrice:
                ...
                ael.log(instrument.Name() + ": No MtM price exists.")
...
def UpdateVarianceSwapInstruments(varianceSwapList, dateToday, updateHistorical, *rest):
    from VarSwapInitialFixingFunctions import checkPriceFixingForVarianceSwapInstrument  #CR: 212697 - Rohan van der Walt - External Module to check for initial fixing of VarSwaps
...
def AllExoticUpdateMethods(instrumentCollection):
    ...
    """
    This would be part of the Default module:
