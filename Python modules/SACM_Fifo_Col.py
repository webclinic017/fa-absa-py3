'''SACM_Fifo_Col: last updated on Wed Jun 02 14:32:37 2004. Extracted by Stowaway on 2004-06-04.'''
'''ACMB_Fifo_pp_cfp: last updated on Sat Apr 03 13:11:17 2004. Extracted by Stowaway on 2004-04-05.'''
...
def calc_avg_cost_fifo(trades,tsb,ACnew=0.0,QTYnew=0.0):
    ...
    Q=P=RPL=0.0
    Qnew=trades[0].quantity
    ACnew=trades[0].price
    open_trades=[(trades[0].price, trades[0].quantity, trades[0])] # List of open trades values
    
    Qleft=Pleft=0.0        
    for t in trades[1:]:
        if do_log_avg: print "\n\ncalc_avg_cost_fifo:: Q(%.2lf) A(%.2lf) RPL(%.2lf)" % (Qnew, ACnew, RPL)
        if do_log_avg:
            print "N(%d) P(%.2lf) Q(%.lf) T(%s) VD(%s) TD(%s)" % (t.trdnbr, t.price, t.quantity, ttos(t.time), t.value_day, TTTOD(t))
        Q=t.quantity
        P=t.price
        Qold=Qnew
        ACold=ACnew
		...
		print T1.trdnbr, t.trdnbr, 'line304'
		if tsb == 1:
		    ClosingFIFOTrade(T1, t)
		    ...
		    for ot in open_trades:
		    	(op1, oq1, ot1) = ot
			print ot1.trdnbr, t.trdnbr, 'line318'
			if tsb == 1 and ot1.time < t.time:
			    ClosingFIFOTrade(ot1, t)
	    ...
	    while open_trades:
	        (P1, Q1, T1)=first(open_trades) # pops!
    	    	...
    	    	print T1.trdnbr, t.trdnbr, 'line334'
		if tsb == 1:
		    ClosingFIFOTrade(T1, t)
    ...
    for alltrds in trades:
    	...
    	elif alltrds not in openlist:
    	    pop_FIFO(alltrds, 0.0)
    ael.poll()
    for openfifo in open_trades:
    	print openfifo[2], openfifo[1], 'line357'
...
#    print t.trdnbr, value, 'pop_FIFO function'
    saved = 0
    value = value * t.insaddr.contr_size
    value_str = str(value)
    for addinfo in t.additional_infos():
    	
    	if addinfo.addinf_specnbr.field_name == 'FIFO_POS':
    	    print '*ADDINFO**** ', addinfo.value
		...
		addinfoclone.value= value
	    	addinfoclone.commit()
		saved = 1 
	    elif  addinfo.value and value_str and addinfo.value != value_str and addinfo.value != '0.0':
	    	#added the addinfo.value != 0    04/05/17
	    	print '****ADDINFO != 0****** ', addinfo.value
...
def ClosingFIFOTrade(t, ct):
    
    
    if t.insaddr == ct.insaddr:
#	if t.insaddr.insid == 'ZAR/R194': print ct.trdnbr, t.trdnbr, '392'
	saved = 0
	max_run = 0 
	value = ct.trdnbr

	x=ael.TimeSeries.new()
	x.recaddr = t.trdnbr
	x.value= value
	x.day = ct.value_day
	x.ts_specnbr=ael.TimeSeriesSpec['ClosingFIFOTrade']

	exists = 0

	for ts in t.time_series():
    	    if ts.ts_specnbr == x.ts_specnbr and ts.day == x.day:
		max_run = max(max_run, ts.run_no)
	    x.run_no=max_run+1 	
    	    if ts.recaddr == x.recaddr and ts.value == x.value: exists = 1    

	if exists == 0: x.commit()
	ael.poll()
...
def avg_price_fifo(ft,args,ctsb,*rest):
...
#    tft = args.trades()
    print 'TIMESERIES = ', ctsb
    if ctsb == 'Yes':
    	tsb = 1
    else:
    	tsb = 0
    tf = ael.TradeFilter.read('fltid = "%s"' %args)
    tft = tf.trades() # ael.TradeFilter.read('fltid = "%s"' %args[0][0]).trades()
    cut_off_day=ael.date_today()
    
#    if not args: 
#    	raise 'No arguments passed'
#    elif args.record_type != 'TradeFilter':
#    	raise 'Argument must be a trade filter'
	
#    args.get_query()
    pfolios={}
#    for x in args.get_query():
#	if x[2] == 'Portfolio':
#    	    name = x[4]
#	    pfolios[name]=[]

    for trds in tft:
    	if ttod(trds.time) <= cut_off_day:
	    pf = trds.prfnbr.prfid + trds.insaddr.insid
	    pfolios[pf]= []
    ...
    for trds in tft:
    	if ttod(trds.time) <= cut_off_day:
	    pf = trds.prfnbr.prfid + trds.insaddr.insid
	    pfolios[pf].append(trds)

    a = 0
    for pf in pfolios.keys():
    	print pf
	for abc in pfolios[pf]:
	    print abc.trdnbr
	if len(pfolios[pf])> 1:
	    print 'start fifo'
	    calc_avg_cost_fifo(pfolios[pf], tsb)
	    a = a+ 1
    	else:
	    print 'There is only one trade in portfolio %s' % pf
    return 'Done'	    


def FIFOPricePosition(fdt,fgt,*rest):

#    print 'enter FPP', fdt.trdnbr, fgt.trdnbr
    ts = ael.TimeSeries.select('ts_specnbr.field_name = "ClosingFIFOTrade"')
    ffgtrades = []
    ffdtrades = []
...
#	else: 
#	    continue
#    trades.sort(lambda x,y: x.time - y.time)


    for x in ts:
    	ffdtrade = ael.Trade.read('trdnbr ="%s"' %x.recaddr)
	val = str(int(x.value))
    	ffgtrade = ael.Trade.read('trdnbr ="%s"' %val)
    	if ffgtrade.time <= fgt.time and ffdtrade.time <= fdt.time and ffgtrade.prfnbr == fgt.prfnbr and ffgtrade.insaddr == fgt.insaddr:   
	    if (ffdtrade, ffgtrade) not in fifopairs: 
	    	fifopairs.append((ffdtrade, ffgtrade))
    
#    fifopairs.sort(lambda x, y: x[0].time - y[0].time)
    fifopairs.sort(sortFn)
    ffdnoms = {}
    ffgnoms = {}
    ...
    for ff in fifopairs:
    	if ff[0].trdnbr not in ffdnoms.keys() : ffdnoms[ff[0].trdnbr] = ff[0].nominal_amount()
    	if ff[0].trdnbr not in ffgnoms.keys() : ffgnoms[ff[1].trdnbr] = ff[1].nominal_amount()

    
    ret = 0
    for ff in fifopairs:
#    	print ff[0].trdnbr, ff[1].trdnbr
#    	print ffdnoms, ffgnoms, ffdnoms[ff[0].trdnbr], ffgnoms[ff[1].trdnbr]
    	if abs(ffdnoms[ff[0].trdnbr]) <= abs(ffgnoms[ff[1].trdnbr]):
	    ret = ffdnoms[ff[0].trdnbr] * -1
	    new = ffdnoms[ff[0].trdnbr] + ffgnoms[ff[1].trdnbr]
	    ffdnoms[ff[0].trdnbr] = 0
	    ffgnoms[ff[1].trdnbr] = new
	else:
	    ret = ffgnoms[ff[1].trdnbr]
	    ffdnoms[ff[0].trdnbr] = ffdnoms[ff[0].trdnbr] + ffgnoms[ff[1].trdnbr]
	    ffgnoms[ff[1].trdnbr] = 0
    return ret
    	    
def sortFn(a, b):
    if a[0].time > b[0].time: return 1
    elif a[0].time < b[0].time: return -1
    else:
    	if a[1].time > b[1].time: return 1
	elif a[1].time < b[1].time: return -1
	else: return 0
