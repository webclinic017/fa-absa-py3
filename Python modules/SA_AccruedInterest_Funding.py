import ael, string, FYieldConverter
...
#		if (start_day<=edate) and (end_day>edate):
		...
		if request == 'FDI':# and end_day <= edate:
                    ...
                    if (start_day<=edate) and (pay_day>edate):		
                        ...
                        yld = 'Yield'
                        #print 'rolling_period', l.rolling_period
                        if l.rolling_period == '0d':
                            inCon = 1
                            yld = 'Simple'
                        elif l.rolling_period == '6m':
                            inCon = 2
                        elif l.rolling_period == '3m':
                            inCon = 4
                        elif l.rolling_period == '1m':
                            inCon = 12
                        elif l.rolling_period == '7d':
                            inCon = 52
                        elif l.rolling_period == '1d':
                            inCon = 365
                            ...
                            inCon = days
                        rate = FYieldConverter.yield_converter(1, inCon, yld, AccDays, l.fixed_rate, 'NACD')
                            
                        #print "RATE", rate, TradeRate
                        ...
                        WholeCashflow = (pow((1 + rate/36500), days)-1) *t.quantity*ins.contr_size*nominal_factor
                        InterestPerDay = WholeCashflow/days
                        #AccruedCashflow = rate*AccDays/DaysInYear*t.quantity*ins.contr_size*nominal_factor
                        AccruedCashflow = (pow((1 + rate/36500), AccDays)-1) *t.quantity*ins.contr_size*nominal_factor
    ...
    #                    print 'DaysInPeriod ', DaysInPeriod
        ...
        if request == 'FDI':
            FDIFixedAccInt = (FDI_Nominal * TradeRate * FDI_days/36500) - FDI_SUMCF
        else:    
...
#	    	    rate = (cf.period_rate(cf.start_day, cf.end_day) * cf.float_rate_factor) + cf.float_rate_offset
                    ...
                    TradeRate = (cf.period_rate(cf.start_day, edate)) + cf.float_rate_offset
                    ...
                    if l.rolling_period == '0d':
                        inCon = 1
                    elif days >= 360 and days <= 370:
                        inCon = 1
                    elif days >= 177 and days <= 187:
                        inCon = 2
                    elif days >= 86 and days <= 96:
                        inCon = 4
                    elif days >= 25 and days <= 35:
                        inCon = 12
                        ...
                        inCon = days
                    '''
                    rate = FYieldConverter.yield_converter(1, inCon, yld, (ddate.days_between(edate, daycount_method)+1), TradeRate, 'NACD')
		    ...
		    Nominal = t.quantity*ins.contr_size*cf.nominal_factor
	    	    ...
	    	    WholeCashflow = (pow((1 + rate/36500), days)-1) *t.quantity*ins.contr_size*cf.nominal_factor
	    	    InterestPerDay = WholeCashflow/days
	    	    #AccruedCashflow = rate*(ddate.days_between(edate,daycount_method)+1)/DaysInYear*t.quantity*ins.contr_size*cf.nominal_factor
	    	    AccruedCashflow = (pow((1 + rate/36500), (ddate.days_between(edate, daycount_method)+1))-1) *t.quantity*ins.contr_size*cf.nominal_factor
...
#   print '\n\n\n'
    ...
    elif request == 'TradeRate':
        #print 'XXXXX', TradeRate
    	return TradeRate
    elif request == 'FundingRate':
        #print 'YYYYY', rate
    	return rate
...
#t = ael.Trade[935617]
#print FYieldConverter.yield_converter(1, 1, 'Yield', t.value_day.days_between(ael.date_today(),'Act/365'),11.0, 'NACD')

#print accrued_interest(t,'2007-01-01',ael.date_today(), 'TradeRate')
#print accrued_interest(t,'2007-01-01',ael.date_today(), 'FundingRate')

#print '\n\n'
#t = ael.Trade[941565]
#print FYieldConverter.yield_converter(1, 1, 'Yield', t.value_day.days_between(ael.date_today(),'Act/365'),11.0, 'NACD')

#print accrued_interest(t,'2007-01-01',ael.date_today(), '')
